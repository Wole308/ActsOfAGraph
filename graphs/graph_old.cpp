...
graph::graph(unsigned int datasetid, vertex_t vbatchsz){
	...
	// children = new vector<vertex_t>[KVDATA_RANGE];
	// parents = new vertex_t[KVDATA_RANGE];
	// _1stdescendants = new vector<vertex_t>[KVDATA_RANGE];
	// friends = new vector<vertex_t>[KVDATA_RANGE];
	
	// children = new vector<vertex_t>[vbatchsz];
	// parents = new vertex_t[vbatchsz];
	// _1stdescendants = new vector<vertex_t>[vbatchsz];
	// friends = new vector<vertex_t>[vbatchsz];
}
graph::~graph(){}
...
void graph::savevertexisactiveinfostofile(int fd, unsigned int offset, unsigned int * buffer, vertex_t bufferoffset, vertex_t size, unsigned int graph_iterationidx){
	...
	#ifdef _DEBUGMODE_HOSTPRINTS
	utilityobj->printbits(fd, fileoffset, 4);
	// exit(EXIT_SUCCESS);
	#endif
	return;
}
