...
struct Dispatch<FLAG, GraphT, InKeyT, OutKeyT, true> {
	// util::PrintMsg("[### I seen you: /net/bigtemp/oj2zf/gunrock_wole/gunrock/oprtr/AE_advance/kernel.cuh/Launch 3]", true);
  typedef typename GraphT::VertexT VertexT;
  typedef typename GraphT::SizeT SizeT;
  ...
  static __device__ __forceinline__ void Write_Global_Output( // this is the bottleneck function?: has random accesses?
...
__launch_bounds__(
    ...
    void Kernel(const bool queue_reset,
                ...
                AdvanceOpT advance_op) {
	// util::PrintMsg("[### I seen you: /net/bigtemp/oj2zf/gunrock_wole/gunrock/oprtr/AE_advance/kernel.cuh/Launch 2]", true);
  typedef typename GraphT::SizeT SizeT;
  typedef typename GraphT::VertexT VertexT;
...
cudaError_t Launch(const GraphT &graph, const FrontierInT *frontier_in,
	...
	// util::PrintMsg("[### I seen you: /net/bigtemp/oj2zf/gunrock_wole/gunrock/oprtr/AE_advance/kernel.cuh/Launch 1]", true);
  typedef typename GraphT::SizeT SizeT;
  typedef typename GraphT::ValueT ValueT;
  ...
  ///////////////////////////////////////////////////////
	...
	util::PrintMsg(">>> AE_advance Kernel time elapsed (/net/bigtemp/oj2zf/gunrock_wole/gunrock/oprtr/AE_advance/Launch): " + std::to_string(cpu_timer.ElapsedMillis()) + " milliseconds", true); 
	float totalsize = graph.edges;
	float totalsize_gbytes = (totalsize * sizeof(InKeyT)) / 1000000000;
	float totaltime_secs = cpu_timer.ElapsedMillis() / 1000;
	util::PrintMsg(">>> [ADVANCE] Total size (edges):: " + std::to_string(		totalsize		) + " edges", true);
	util::PrintMsg(">>> [ADVANCE] Total size (giga bytes):: " + std::to_string(		totalsize_gbytes		) + " giga bytes", true);
	util::PrintMsg(">>> [ADVANCE] Total time (ms):: " + std::to_string(		(totaltime_secs * 1000)		) + " milliseconds", true);
	util::PrintMsg(">>> [ADVANCE] Total time (s):: " + std::to_string(		totaltime_secs		) + " seconds", true);
	util::PrintMsg(">>> [ADVANCE] Effective Bandwidth (GB/s):: " + std::to_string(		(totalsize_gbytes / totaltime_secs)		) + " Gigabytes per sec", true);
	util::PrintMsg(">>> [ADVANCE] Number of running threads:: " + std::to_string(KernelPolicyT::THREADS) + " threads", true);
	util::PrintMsg("", true);
	#endif 
	///////////////////////////////////////////////////////
