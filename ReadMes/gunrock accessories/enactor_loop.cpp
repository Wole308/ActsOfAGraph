...
void Iteration_Loop(ThreadSlice &thread_data, IterationT &iteration) {
	util::PrintMsg("Iteration_Loop started. (/net/bigtemp/oj2zf/gunrock_wole/gunrock/app/enactor_loop.cuh/Iteration_Loop)", true);
  ...
  util::PrintMsg("Iteration entered (/net/bigtemp/oj2zf/gunrock_wole/gunrock/app/enactor_loop.cuh/Iteration_Loop)", enactor.flag & Debug);
  while (!iteration.Stop_Condition(gpu_num)) {
	  
	util::PrintMsg(">>> Iteration #" + std::to_string(enactor_slices[0].enactor_stats.iteration), true);
    ...
    while (mgpu_slice.wait_counter < num_gpus * 2 &&
      ...
      for (int peer__ = 0; peer__ < num_gpus * 2; peer__++) {
		...
		// util::PrintMsg(">>> Iteration #" + std::to_string(iteration_num), true);
		// util::PrintMsg(">>> Iteration #" + std::to_string(enactor_stats.iteration), true);
    ...
    if (!iteration.Stop_Condition(gpu_num)) {
      ...
      if (IterationT::FLAG & Use_FullQ) {
		...
		util::CpuTimer cpu_timer8;
		cpu_timer8.Start();
		  if (retval =
                  util::GRError(cudaStreamSynchronize(stream),
                                "FullQueue_Core failed.", __FILE__, __LINE__)){ break; } // REMOVEME.
		  iteration.Core(peer_); 
		  if (retval =
                  util::GRError(cudaStreamSynchronize(stream),
                                "FullQueue_Core failed.", __FILE__, __LINE__)){ break; } // REMOVEME.
		  cpu_timer8.Stop(); // stop timer
			util::PrintMsg(">>> Core time elapsed (t8) (/net/bigtemp/oj2zf/gunrock_wole/gunrock/app/enactor_loop.cuh/Iteration_Loop): " + std::to_string(cpu_timer8.ElapsedMillis()) + " milliseconds", true); 
			util::PrintMsg("", true);
			#endif 
			///////////////////////////////////////////////////////
          if (retval) break;
        ...
        if (enactor.flag & Debug) {
          util::PrintMsg(std::to_string(gpu_num) + "\t " +
                         std::to_string(enactor_stats0.iteration) +
                         "\t \t Fullqueue finished (/net/bigtemp/oj2zf/gunrock_wole/gunrock/app/enactor_loop.cuh/Iteration_Loop). Total_Length= " +
                         std::to_string(total_length));
        }
