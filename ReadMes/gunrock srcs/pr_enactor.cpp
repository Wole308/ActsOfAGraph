...
struct PRIterationLoop : public IterationLoopBase<EnactorT, Use_FullQ | Push> {
  ...
  cudaError_t Core(int peer_ = 0) {
	...
	util::PrintMsg("Core started. (/net/bigtemp/oj2zf/gunrock_wole/gunrock/app/pr/pr_enactor/Core)", true);
    ...
    if (iteration != 0) {
		...
		cpu_timer.Start();
		util::GRError(cudaStreamSynchronize(oprtr_parameters.stream),
      ...
      if (enactor.flag & Debug)
		...
		util::PrintMsg(">>> FilterOp time elapsed (/net/bigtemp/oj2zf/gunrock_wole/gunrock/app/pr/pr_enactor.cuh/Core): " + std::to_string(cpu_timer.ElapsedMillis()) + " milliseconds", true); 
		util::PrintMsg("", true);
		#endif 
	  ///////////////////////////////////////////////////////
    ...
    if (data_slice.pull) {
      ...
      if (enactor.flag & Debug)
        util::cpu_mt::PrintMessage("Advance start. (/net/bigtemp/oj2zf/gunrock_wole/gunrock/app/pr/pr_enactor.cuh/Core)", gpu_num, iteration, peer_);
      ...
      GUARD_CU(oprtr::Advance<oprtr::OprtrType_V2V>(
		...
		util::GRError(cudaStreamSynchronize(oprtr_parameters.stream),
                                "FullQueue_Core failed.", __FILE__, __LINE__);
	  cpu_timer.Stop(); // stop timer
	  util::PrintMsg(">>> AdvanceOp time elapsed (/net/bigtemp/oj2zf/gunrock_wole/gunrock/app/pr/pr_enactor.cuh/Core): " + std::to_string(cpu_timer.ElapsedMillis()) + " milliseconds", true); 
	  util::PrintMsg("", true);
	  #endif 
	  ///////////////////////////////////////////////////////
    }
...
class Enactor : public EnactorBase<typename _Problem::GraphT,
 ...
 public:
  ...
  cudaError_t Run(ThreadSlice &thread_data) {
	...
	// cpu_timer.Stop();
	// util::PrintMsg("Iteration_Loop Done. (/net/bigtemp/oj2zf/gunrock_wole/gunrock/app/pr/pr_enactor.cuh): " + std::to_string(cpu_timer.ElapsedMillis()) + " milliseconds", true);
	
	cpu_timer.Stop(); // stop timer
	util::PrintMsg(">>> Iteration_Loop time elapsed (/net/bigtemp/oj2zf/gunrock_wole/gunrock/app/pr/pr_enactor.cuh/Run): " + std::to_string(cpu_timer.ElapsedMillis()) + " milliseconds", true);
	util::PrintMsg("", true);
	util::PrintMsg("", true);
  ...
  cudaError_t Enact(VertexT src) {
    ...
    // util::PrintMsg("GPU PageRank Done. (/net/bigtemp/oj2zf/gunrock_wole/gunrock/app/pr/pr_enactor.cuh)", this->flag & Debug);
	
	cpu_timer.Stop(); // stop timer
	util::PrintMsg("GPU PageRank Done. (/net/bigtemp/oj2zf/gunrock_wole/gunrock/app/pr/pr_enactor.cuh): " + std::to_string(cpu_timer.ElapsedMillis()) + " milliseconds", true);
	util::PrintMsg("", true);
	util::PrintMsg("", true);
