...
cudaError_t RunTests(util::Parameters &parameters, GraphT &graph,
  ...
  for (int run_num = 0; run_num < num_runs; ++run_num) {
    ...
    util::PrintMsg("__________________________", !quiet); // CHANGEHERE.
	util::PrintMsg(
        "\nRun " + std::to_string(run_num) + // CHANGEHERE.
            " num_runs: " + std::to_string(num_runs) +
            ", num_runs = " + std::to_string(num_runs) + ", num_runs = " +
            std::to_string(num_runs),
        !quiet);

    cpu_timer.Start();
	util::PrintMsg("Launch Enactor. (/net/bigtemp/oj2zf/gunrock_wole/gunrock/app/pr/pr_app.cu/RunTests)", true);
    GUARD_CU(enactor.Enact(src));
    cpu_timer.Stop();
	util::PrintMsg(">>> Enactor time elapsed. (/net/bigtemp/oj2zf/gunrock_wole/gunrock/app/pr/pr_app.cu/RunTests): " + std::to_string(cpu_timer.ElapsedMillis()) + " milliseconds", true);			
