...
#ifdef KKK
unsigned int loadedges_sequential::getglobalpartition(keyvalue_t keyvalue, vertex_t upperlimit, unsigned int batch_range_pow, unsigned int treedepth){
	unsigned int partition = ((keyvalue.key - upperlimit) >> (BATCH_RANGE_POW - (NUM_PARTITIONS_POW * treedepth)));
	
	#ifdef _DEBUGMODE_CHECKS3
	utilityobj->checkoutofbounds("loadedges_sequential::getglobalpartition", partition, (1 << (NUM_PARTITIONS_POW * treedepth)), keyvalue.key, upperlimit, NAp);
	#endif
	return partition;
}
globalparams_TWOt loadedges_sequential::loadoffsetmarkers(vptr_type * vptrs[NUMSUBCPUTHREADS], edge_type * edges[NUMSUBCPUTHREADS], keyvalue_t * stats[NUMSUBCPUTHREADS], vector<edge3_type> (&edges_temp)[NUMSUBCPUTHREADS], container_t * container, globalparams_TWOt globalparams){
	#ifdef _DEBUGMODE_HOSTPRINTS3
	cout<<"loadedges_sequential::loadoffsetmarkers:: loading offset markers... "<<endl;
	#endif
	...
	for(unsigned int i = 0; i < NUMSUBCPUTHREADS; i++){
		#ifdef _DEBUGMODE_HOSTPRINTS3
		cout<<"loadedges_sequential::loadoffsetmarkers:: edgessize["<<i<<"]: "<<container->edgessize[i]<<endl;
		#endif 
		...
		for(unsigned int k=0; k<container->edgessize[i]; k++){
			#ifdef _DEBUGMODE_HOSTPRINTS
			if(k % 100000 == 0){ cout<<"### loadedges_sequential::loadoffsetmarkers:: k: "<<k<<endl; }
			#endif
			...
			if(k % VECTOR2_SIZE == 0){ if(k%1000000 == 0){ cout<<"loadedges_sequential::loadoffsetmarkers: src1: "<<src1<<", src2: "<<src2<<", k: "<<k<<", i: "<<i<<endl; }}
			#endif 
			...
			for(unsigned int CLOP=1; CLOP<=TREE_DEPTH; CLOP++){
				...
				#ifdef _DEBUGMODE_CHECKS
				utilityobj->checkoutofbounds("loadedges_sequential::loadoffsetmarkers.partitionCLOP.1", partitionCLOP, pow(NUM_PARTITIONS, TREE_DEPTH), edges_temp[i][k].dstvid, edges_temp[i][k].status, container->edgessize[i]);
				utilityobj->checkoutofbounds("loadedges_sequential::loadoffsetmarkers.partitionCLOP.2", offset + partitionCLOP, _KVSTATSDRAMSZ, keyvalue.key, container->edgessize[i], NAp);
				#endif
				...
				#ifdef _DEBUGMODE_CHECKS
				utilityobj->checkoutofbounds("loadedges_sequential::loadoffsetmarkers.p.1", p, _NUM_EDGECHUNKS_IN_A_BUFFER, keyvalue.key, vptrs[i][TWOO*_BASEOFFSET_VERTEXPTR + keyvalue.key].key, NAp);
				#endif
		...
		for(unsigned int CLOP=1; CLOP<=TREE_DEPTH; CLOP++){
			...
			utilityobj->checkoutofbounds("loadedges_sequential::loadoffsetmarkers.offset", offset, KVSTATSDRAMSZ, NAp, NAp, NAp);
...
void loadedges_sequential::accumstats(uint512_vec_dt * kvbuffer[NUMSUBCPUTHREADS], uint512_vec_dt * stats[NUMSUBCPUTHREADS], globalparams_TWOt globalparams){
	#ifdef _DEBUGMODE_HOSTPRINTS2
	cout<<"loadedges_sequential::accumstats:: accumulating stats to base... "<<endl;
	#endif
...
globalparams_TWOt loadedges_sequential::generatevmaskdata(vector<vertex_t> &activevertices, uint512_vec_dt * kvbuffer[NUMSUBCPUTHREADS], uint512_vec_dt * vdram, globalparams_TWOt globalparams){ 
	#ifdef _DEBUGMODE_HOSTPRINTS2
	cout<<"loadedges_sequential::generatevmaskdata:: generating vmask... "<<endl;
	#endif
...
void loadedges_sequential::savevdata(uint512_vec_dt * kvdram, unsigned int buffer[VDATA_PACKINGSIZE][REDUCEPARTITIONSZ_KVS2], batch_type vbaseoffset_kvs, batch_type voffset_kvs){
	#ifdef _DEBUGMODE_HOSTPRINTS
	cout<<"loadedges_sequential::savemasks:: saving vdata: voffset_kvs: "<<voffset_kvs<<endl;
	#endif 
...
void loadedges_sequential::setrootvid(unsigned int Algo, uint512_vec_dt * kvbuffer, vector<vertex_t> &activevertices, globalparams_t globalparams){
	#ifdef _DEBUGMODE_HOSTPRINTS
		...
		for(unsigned int partition=0; partition<NUMREDUCEPARTITIONS; partition++){
			for(unsigned int k=0; k<REDUCEPARTITIONSZ_KVS2; k++){
				for(unsigned int v=0; v<VDATA_PACKINGSIZE; v++){
					...
					if(vid == 1){
						#ifdef _DEBUGMODE_HOSTPRINTS
						cout<<"loadedges_sequential::setrootvid::(1) vid == 1 seen. i: "<<i<<", partition: "<<partition<<", k: "<<k<<", v: "<<v<<", lvid: "<<lvid<<", vid: "<<vid<<endl;
						#endif 
