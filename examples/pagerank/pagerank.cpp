...
runsummary_t pagerank::run(){
	cout<<"pagerank::run:: advance algorithm started. "<<endl;
	...
	for(unsigned int groupid = 0; groupid < 1; groupid++){
		...
		for(unsigned int graph_iterationidx=0; graph_iterationidx<1; graph_iterationidx++){
			cout<< TIMINGRESULTSCOLOR <<">>> pagerank::run: graph iteration "<<graph_iterationidx<<" of advance started"<< RESET <<endl;
...
void pagerank::loadgraphdata(unsigned int col, graph * graphobj, value_t * vertexdatabuffer, container_t * container){
...
void pagerank::loadgraphdata(unsigned int col, unsigned int threadid, unsigned int subthreadid, graph * graphobj, value_t * vertexdatabuffer, container_t * container){						
