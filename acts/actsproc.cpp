...
tradreduceandbuffer(bool_type enable, uint512_dt * kvdram, keyvalue_buffer_t buffer[VECTOR_SIZE][SOURCEBLOCKRAM_SIZE], buffer_type chunk_size, globalparams_t globalparams){				
	...
	#pragma HLS PIPELINE II=16 // NEWCHANGE.
		BASICREDUCEANDBUFFER_LOOP1B: for(unsigned int v=0; v<VECTOR_SIZE; v++){
			...
			// cout<<"tradreduceandbuffer: kv2.key: "<<kv2.key<<", kv2.value: "<<kv2.value<<""<<endl;
			if(kv2.key != GETV(INVALIDDATA) && kv2.value != GETV(INVALIDDATA)){
				...
				value_t temp;
				if(kv2.key % 2 == 0){ temp = kvdram[globalparams.BASEOFFSETKVS_VERTICESDATA + (kv2.key / VECTOR2_SIZE)].data[kv2.key % VECTOR_SIZE].key; }
				else { temp = kvdram[globalparams.BASEOFFSETKVS_VERTICESDATA + (kv2.key / VECTOR2_SIZE)].data[kv2.key % VECTOR_SIZE].value; }
				...
				if(kv2.key % 2 == 0){ kvdram[globalparams.BASEOFFSETKVS_VERTICESDATA + (kv2.key / VECTOR2_SIZE)].data[kv2.key % VECTOR_SIZE].key = rettemp; }
				else { kvdram[globalparams.BASEOFFSETKVS_VERTICESDATA + (kv2.key / VECTOR2_SIZE)].data[kv2.key % VECTOR_SIZE].value = rettemp; }
			}
		}
	}
	return;
