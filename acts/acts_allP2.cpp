...
void acts_all::UTILP2_writetoglobalvar(bool write){
	#ifdef SW____NOTUSED
...
void acts_all::UTILP2_ReadDatas(uint512_dt * kvdram, unsigned int offset_kvs, keyvalue_t datas[VECTOR_SIZE]){
	#pragma HLS INLINE
	#ifdef _WIDEWORD 
	datas[0].key = kvdram[offset_kvs].range(31, 0); 
	datas[0].value = kvdram[offset_kvs].range(63, 32); 
...
unsigned int acts_all::UTILP2_ReadData(uint512_dt * kvdram, unsigned int offset_kvs, unsigned int index){
 ...
 if(col == 0){
	...
	actsutilityobj->checkoutofbounds("UTILP2_ReadData 21", offset_kvs + (loc / VECTOR_SIZE), (((1 << 28) / 4) / 16), NAp, MESSAGES_ACTSPARAMS_MAXHBMCAPACITY_KVS, NAp);
	#endif
	...
	#ifdef SW
	actsutilityobj->checkoutofbounds("UTILP2_ReadData 22", globalvar_actsinstid, 64, NAp, MESSAGES_ACTSPARAMS_MAXHBMCAPACITY_KVS, NAp);
	UTILP2_writetoglobalvar(0);
	#endif
	return data;
}
...
keyvalue_t acts_all::UTILP2_ReadKV(uint512_dt * kvdram, unsigned int offset_kvs, unsigned int index){
	#pragma HLS INLINE
	keyvalue_t data;
	#ifdef _WIDEWORD
...
bool acts_all::UTILP2__processit__i_am_first__(globalposition_t globalposition){
	#pragma HLS INLINE
...
bool acts_all::UTILP2__processit__i_am_last__(globalposition_t globalposition){
	#pragma HLS INLINE
...
bool acts_all::UTILP2__reduceit__i_am_last__(globalposition_t globalposition, sweepparams_t sweepparams, globalparams_t globalparams){
	#pragma HLS INLINE
	bool __i_am_last__ = false;
...
// checks 
bool acts_all::UTILP2_isbufferused(unsigned int id){
...
void acts_all::MEMACCESSP2_read(bool_type enable, uint512_dt * kvdram, unsigned int buffer[VECTOR2_SIZE][BLOCKRAM_SIZE], batch_type baseoffset_kvs, batch_type offset_kvs, batch_type bufferoffset_kvs, buffer_type size_kvs){
...
void acts_all::MEMACCESSP2_readV(bool_type enable, uint512_dt * kvdram, keyvalue_vbuffer_t buffer[VECTOR2_SIZE][MAX_BLOCKRAM_VDESTDATA_SIZE], batch_type baseoffset_kvs, batch_type offset_kvs, batch_type bufferoffset_kvs, buffer_type size_kvs){
	if(enable == OFF){ return; } 
	...
	#pragma HLS PIPELINE II=1
		...
		#pragma HLS UNROLL 
			#ifdef _DEBUGMODE_CHECKS3
			actsutilityobj->checkoutofbounds("MEMACCESSP2_readV:: ERROR 21d", bufferoffset_kvs + i, MAX_BLOCKRAM_VDESTDATA_SIZE, bufferoffset_kvs, size_kvs, NAp);
			#endif
...
keyvaluemask_t acts_all::process_edge(unsigned int mode, bool enx, unsigned int v, unsigned int loc, keyvalue_t edge_data, keyvalue_vbuffer_t vbuffer[MAX_BLOCKRAM_VDESTDATA_SIZE], globalparams_t globalparams){				
	...
	if(mask == 1 && mode == ACTSPROCESSMODE){ cout<<">>> PROCESS VECTOR:: PROCESS EDGE SEEN @ v: "<<v<<", loc: "<<loc<<", edge_data.key: "<<edge_data.key<<", edge_data.value: "<<edge_data.value<<", udata: "<<udata<<", mask: "<<mask<<", srcvid: "<<(edge_data.value * EDGEDATA_PACKINGSIZE) + v<<", dstvid*: "<<UTILP2_GETREALVID(edge_data.key, globalparams.ACTSPARAMS_INSTID)<<", ldstvid: "<<edge_data.key<<endl; }
	#endif
	
	// cout<<">>> PROCESS VECTOR:: PROCESS EDGE SEEN @ v: "<<v<<", loc: "<<loc<<", edge_data.key: "<<edge_data.key<<", edge_data.value: "<<edge_data.value<<", udata: "<<udata<<", mask: "<<mask<<", srcvid: "<<(edge_data.value * EDGEDATA_PACKINGSIZE) + v<<", dstvid*: "<<UTILP2_GETREALVID(edge_data.key, globalparams.ACTSPARAMS_INSTID)<<", ldstvid: "<<edge_data.key<<endl;
...
unsigned int acts_all::MEMACCESSP2_process_and_buffer(unsigned int chunk_id, unsigned int mode, unsigned int llp_set, uint512_dt *in, uint512_evec_dt out[BLOCKRAM_SIZE], keyvalue_vbuffer_t vbuffer_source[VDATA_PACKINGSIZE][MAX_BLOCKRAM_VSRCDATA_SIZE], stats_t stats[STATS_PACKINGSIZE][BLOCKRAM_SIZE], workload_t workload_kvs, collection_t collections[COLLECTIONS_BUFFERSZ], globalparams_t globalparamsK){
	...
	#pragma HLS PIPELINE II=1
		// read
		UTILP2_ReadDatas(in, workload_kvs.offset_srcbase + (chunk_id * BLOCKRAM_SIZE) + i, datas);
...
void acts_all::store(unsigned int chunk_id, uint512_dt *out, uint512_evec_dt in[VDATA_PACKINGSIZE], workload_t workload_kvs, unsigned int offsetkvs, unsigned int sizekvs, collection_t collections[COLLECTIONS_BUFFERSZ], globalparams_t globalparamsK){
	...
	#pragma HLS PIPELINE II=1
		...
		UTILP2_WriteDatas(out, workload_kvs.offset_dstbase + offsetkvs + i, datas);
	}
	
}
 
unsigned int acts_all::MEMACCESSP2_write__process_base(unsigned int mode, unsigned int llp_set, uint512_dt * kvdram, keyvalue_vbuffer_t vbuffer_source[VDATA_PACKINGSIZE][MAX_BLOCKRAM_VSRCDATA_SIZE], stats_t stats[STATS_PACKINGSIZE][BLOCKRAM_SIZE], workload_t workload_kvs, collection_t collections[COLLECTIONS_BUFFERSZ], globalparams_t globalparamsK){
	...
	for (int chunk_id = 0; chunk_id < (workload_kvs.size + (BLOCKRAM_SIZE - 1)) / BLOCKRAM_SIZE; chunk_id++){
		unsigned int sizekvs = MEMACCESSP2_process_and_buffer(chunk_id, mode, llp_set, kvdram, c1, vbuffer_source, stats, workload_kvs, collections, globalparamsK);
...
void acts_all::reduce_update(unsigned int mode, bool enx, unsigned int v, unsigned int loc, keyvalue_t update_data, keyvalue_vbuffer_t vbuffer[MAX_BLOCKRAM_VDESTDATA_SIZE], unsigned int stats[BLOCKRAM_SIZE], unsigned int memory[1], globalparams_t globalparams){
	...
	if(en == true){ // REMOVEME.
		...
		if(mode == ACTSREDUCEMODE){ cout<<">>> REDUCE VECTOR:: REDUCE UPDATE SEEN @: v: "<<v<<", loc: "<<loc<<", mask: "<<mask<<", update_data.key: "<<update_data.key<<", update_data.value: "<<update_data.value<<", new_vprop: "<<new_vprop<<", new combo: "<<((new_vprop << 1) | 0x1)<<", dstvid: "<<UTILP2_GETREALVID(update_data.key, globalparams.ACTSPARAMS_INSTID)<<", vdata_tmp: "<<vdata_tmp<<endl; }
		#endif
...
void acts_all::MEMACCESSP2_read__reduce(unsigned int mode, uint512_dt * kvdram, keyvalue_vbuffer_t vbuffer_dest[VDATA_PACKINGSIZE][MAX_BLOCKRAM_VDESTDATA_SIZE], stats_t stats[STATS_PACKINGSIZE][BLOCKRAM_SIZE],
	...
	#pragma HLS dependence variable=vbuffer_dest inter false
		...
		// read 
		UTILP2_ReadDatas(kvdram, workload_kvs.offset_srcbase + offset_kvs, datas);
...
unsigned int acts_all::MEMACCESSP2_get_updateblock_workload(bool en, unsigned int reduce_partition, unsigned int * stats_data, unsigned int * stats_metadata, globalparams_t globalparams, workload_t xload_kvs[BLOCKRAM_SIZE], unsigned int buffer_offsets[BLOCKRAM_SIZE]){
	// #pragma HLS INLINE
...
unsigned int acts_all::MEMACCESSP2_get_upropblock_workload(bool en, unsigned int process_partition, uint512_dt * dram, keyvalue_vbuffer_t buffer[VDATA_PACKINGSIZE][MAX_BLOCKRAM_VDESTDATA_SIZE], stats_t stats[STATS_PACKINGSIZE][BLOCKRAM_SIZE], unsigned int num_active_upropblocks, globalparams_t globalparams, workload_t xload_kvs[BLOCKRAM_SIZE], unsigned int buffer_offsets[BLOCKRAM_SIZE], unsigned int graphiterationid){
	...
	if(sparse_readu == true){
		...
		for(unsigned int n=0; n<num_its; n++){
			...
			#endif
			upropblock_ids[n] = UTILP2_ReadData(dram, globalparams.BASEOFFSETKVS_ACTIVEUPROPBLOCKS + upropblockoffset_vs, n); // CRITICAL OPTIMIZEME. use wide word buffer
			#ifdef _DEBUGMODE_CHECKS3
...
unsigned int acts_all::MEMACCESSP2_get_edgeblock_workload(bool en, uint512_dt * dram, unsigned int process_partition, unsigned int llp_set, unsigned int vertexblock_ids[VDATA_PACKINGSIZE][BLOCKRAM_SIZE], stats_t stats[STATS_PACKINGSIZE][BLOCKRAM_SIZE], unsigned int num_active_edgeblocks, globalparams_t globalparamsK, globalparams_t globalparamsE, workload_t xload_kvs[BLOCKRAM_SIZE], unsigned int graphiterationid){
	...
	READ_PROCESS_PARTITION_WRITE_GETWORKLOADSTATS_LOOP: for(unsigned int n=0; n<num_its; n++){
		...
		if(sparse_process == true){
			...
			#endif
			workload_kvs.offset_begin = UTILP2_ReadData(dram, globalparamsE.BASEOFFSETKVS_VERTEXPTR, vertexblock_id) / EDGEDATA_PACKINGSIZE; 
			workload_kvs.offset_end = UTILP2_ReadData(dram, globalparamsE.BASEOFFSETKVS_VERTEXPTR, vertexblock_id + 1) / EDGEDATA_PACKINGSIZE; 
...
void acts_all::MEMACCESSP2_readglobalstats(bool_type enable, uint512_dt * kvdram, keyvalue_t globalstatsbuffer[BLOCKRAM_GLOBALSTATS_SIZE], batch_type offset_kvs, globalparams_t globalparams){ 
	...
	#pragma HLS PIPELINE II=1
		...
		// UTILP2_ReadDatas(kvdram, offset_kvs + i, datas); // NEWCHANGE.
...
void acts_all::MEMACCESSP2_readhelperstats(uint512_dt * vdram, pmask_dt pmask[BLOCKRAM_CURRPMASK_SIZE], batch_type offset_kvs, batch_type size_kvs, unsigned int GraphIter, globalparams_t globalparams){
...
void acts_all::MEMACCESSP2_savehelperstats(uint512_dt * vdram, pmask_dt pmask[BLOCKRAM_CURRPMASK_SIZE], batch_type offset_kvs, batch_type size_kvs, unsigned int GraphIter, globalparams_t globalparams){
	...
	#pragma HLS PIPELINE II=1
		...
		UTILP2_WriteDatas(vdram, offset_kvs + i, datas);
...
void acts_all::mergeP2_trace(uint512_dt * kvdram, unsigned int reduce_partition, unsigned int k, unsigned int offset_kvs, globalparams_t globalparams){
	#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3
	for(unsigned int v=0; v<VDATA_PACKINGSIZE; v++){
		...
		#ifdef _DEBUGMODE_KERNELPRINTS4
		if(mask == 1){ cout<<">>> MERGE:: MERGE VERTICES SEEN @ reduce_partition: "<<reduce_partition<<", k: "<<k<<", v: "<<v<<", vdata: "<<vdata<<", mask: "<<mask<<", vid: "<<UTILP2_GETREALVID(lvid, globalparams.ACTSPARAMS_INSTID)<<", combo: "<<combo<<", globalparams.SIZE_DESTVERTICESDATA: "<<globalparams.SIZE_DESTVERTICESDATA<<endl; }	
		#endif 
	}
	#endif 
}
...
unsigned int acts_all::MERGEP2_mergeVs(uint512_dt * kvdram, uint512_dt * vdram){
	#ifdef _DEBUGMODE_KERNELPRINTS3
	...
	MERGEP2_MERGEVSLOOP2: for(unsigned int reduce_partition=0; reduce_partition<globalparams.NUM_REDUCEPARTITIONS; reduce_partition++){
		...
		if(globalparams.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE){
			...
			num_its = MEMACCESSP2_get_updateblock_workload(true, reduce_partition, stats[0], stats[1], globalparams, xload_kvs, buffer_offsets); 
			// num_its = MEMACCESSP2_get_updateblock_workload(false, reduce_partition, stats[0], stats[1], globalparams, xload_kvs, buffer_offsets); 
		}
		
		MERGEP2_MERGEVSLOOP2B: for(unsigned int n=0; n<num_its; n++){
			...
			MERGEP2_MERGEVSLOOP2C: for(unsigned int k=xload_kvs[n].offset_begin; k<xload_kvs[n].offset_begin + xload_kvs[n].size; k++){ // globalparams.SIZE_DESTVERTICESDATA / VECTOR2_SIZE, globalparams.SIZEKVS2_REDUCEPARTITION
			#pragma HLS PIPELINE II=1
				#ifdef _DEBUGMODE_CHECKS3
				...
				#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3
				mergeP2_trace(kvdram, reduce_partition, k, globalparams.BASEOFFSETKVS_DESTVERTICESDATA + voffseti_kvs2 + k, globalparams); total_vertices_merged += VDATA_PACKINGSIZE;
				#endif
			}
...
void acts_all::MERGEP2_broadcastVs(uint512_dt * vdram, uint512_dt * kvdram, unsigned int offset1_kvs, unsigned int offset2_kvs, unsigned int size_kvs){
	MERGEP2_BROADCASTVS_LOOP: for(unsigned int k=0; k<size_kvs; k++){
	#pragma HLS PIPELINE II=1
...
void acts_all::MERGEP2_broadcastVs1(uint512_dt * vdram, uint512_dt * kvdram0, unsigned int offsetSRC_kvs, unsigned int offsetDST_kvs, unsigned int size_kvs){
	value_t datas[VECTOR2_SIZE]; 
	#pragma HLS ARRAY_PARTITION variable=datas complete
	
	MERGEP2_BROADCASTVS_LOOP: for(unsigned int k=0; k<size_kvs; k++){ // workload_kvs.offset_begin
	#pragma HLS PIPELINE II=1
		#ifdef _DEBUGMODE_CHECKS3
		...
		UTILP2_ReadDatas(vdram, offsetSRC_kvs + k, datas);	
		
		UTILP2_WriteDatas(kvdram0, offsetDST_kvs + k, datas);
	}
	return;
}
void acts_all::MERGEP2_broadcastVs2(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1, unsigned int offsetSRC_kvs, unsigned int offsetDST_kvs, unsigned int size_kvs){
	value_t datas[VECTOR2_SIZE]; 
	#pragma HLS ARRAY_PARTITION variable=datas complete
	...
	#pragma HLS PIPELINE II=1
		...
		UTILP2_WriteDatas(kvdram1, offsetDST_kvs + k, datas);
	}
	return;
}
void acts_all::MERGEP2_broadcastVs3(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2, unsigned int offsetSRC_kvs, unsigned int offsetDST_kvs, unsigned int size_kvs){
	value_t datas[VECTOR2_SIZE]; 
	#pragma HLS ARRAY_PARTITION variable=datas complete
	...
	#pragma HLS PIPELINE II=1
		...
		UTILP2_WriteDatas(kvdram2, offsetDST_kvs + k, datas);
	}
	return;
}
void acts_all::MERGEP2_broadcastVs4(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2,uint512_dt * kvdram3, unsigned int offsetSRC_kvs, unsigned int offsetDST_kvs, unsigned int size_kvs){
	value_t datas[VECTOR2_SIZE]; 
	#pragma HLS ARRAY_PARTITION variable=datas complete
	...
	#pragma HLS PIPELINE II=1
		...
		UTILP2_WriteDatas(kvdram3, offsetDST_kvs + k, datas);
	}
	return;
}
void acts_all::MERGEP2_broadcastVs5(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2,uint512_dt * kvdram3,uint512_dt * kvdram4, unsigned int offsetSRC_kvs, unsigned int offsetDST_kvs, unsigned int size_kvs){
	value_t datas[VECTOR2_SIZE]; 
	#pragma HLS ARRAY_PARTITION variable=datas complete
	...
	#pragma HLS PIPELINE II=1
		...
		UTILP2_WriteDatas(kvdram4, offsetDST_kvs + k, datas);
	}
	return;
}
void acts_all::MERGEP2_broadcastVs6(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2,uint512_dt * kvdram3,uint512_dt * kvdram4,uint512_dt * kvdram5, unsigned int offsetSRC_kvs, unsigned int offsetDST_kvs, unsigned int size_kvs){
	value_t datas[VECTOR2_SIZE]; 
	#pragma HLS ARRAY_PARTITION variable=datas complete
	...
	#pragma HLS PIPELINE II=1
		...
		UTILP2_WriteDatas(kvdram5, offsetDST_kvs + k, datas);
	}
	return;
}
void acts_all::MERGEP2_broadcastVs7(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2,uint512_dt * kvdram3,uint512_dt * kvdram4,uint512_dt * kvdram5,uint512_dt * kvdram6, unsigned int offsetSRC_kvs, unsigned int offsetDST_kvs, unsigned int size_kvs){
	value_t datas[VECTOR2_SIZE]; 
	#pragma HLS ARRAY_PARTITION variable=datas complete
	...
	#pragma HLS PIPELINE II=1
		...
		UTILP2_WriteDatas(kvdram6, offsetDST_kvs + k, datas);
	}
	return;
}
void acts_all::MERGEP2_broadcastVs8(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2,uint512_dt * kvdram3,uint512_dt * kvdram4,uint512_dt * kvdram5,uint512_dt * kvdram6,uint512_dt * kvdram7, unsigned int offsetSRC_kvs, unsigned int offsetDST_kvs, unsigned int size_kvs){
	value_t datas[VECTOR2_SIZE]; 
	#pragma HLS ARRAY_PARTITION variable=datas complete
	...
	#pragma HLS PIPELINE II=1
		...
		UTILP2_WriteDatas(kvdram7, offsetDST_kvs + k, datas);
	}
	return;
}
void acts_all::MERGEP2_broadcastVs9(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2,uint512_dt * kvdram3,uint512_dt * kvdram4,uint512_dt * kvdram5,uint512_dt * kvdram6,uint512_dt * kvdram7,uint512_dt * kvdram8, unsigned int offsetSRC_kvs, unsigned int offsetDST_kvs, unsigned int size_kvs){
	value_t datas[VECTOR2_SIZE]; 
	#pragma HLS ARRAY_PARTITION variable=datas complete
...
void acts_all::MERGEP2_broadcastVs12(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2,uint512_dt * kvdram3,uint512_dt * kvdram4,uint512_dt * kvdram5,uint512_dt * kvdram6,uint512_dt * kvdram7,uint512_dt * kvdram8,uint512_dt * kvdram9,uint512_dt * kvdram10,uint512_dt * kvdram11, unsigned int offsetSRC_kvs, unsigned int offsetDST_kvs, unsigned int size_kvs){
	...
	#pragma HLS PIPELINE II=1
		...
		UTILP2_WriteDatas(kvdram11, offsetDST_kvs + k, datas);
	}
	return;
}

unsigned int acts_all::MERGEP2_copy(unsigned int idx, unsigned int reduce_partition, uint512_dt * vdramSRC, uint512_dt * vdramDST1, uint512_dt * vdramDST2, uint512_dt * vdramREF, unsigned int stats[2][BLOCKRAM_SIZE], unsigned int voffset_kvs, unsigned int firstinstance_id, unsigned int num_compute_units, 
	...
	EXCHANGEP2_COPY_LOOP1: for(unsigned int i=0; i<num_compute_units; i++){
		...
		// unsigned int num_its = mergeP2_get_updateblock_workload(reduce_partition, stats[0], stats[1], globalparamsSRC, xload_kvs);
		...
		for(unsigned int n=0; n<num_its; n++){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP2_ReadDatas(vdramSRC, globalparamsSRC.BASEOFFSETKVS_SRCVERTICESDATA + voffset_kvs + v2offset_kvs + k, datas);
				UTILP2_WriteDatas(vdramDST1, globalparamsDST1.BASEOFFSETKVS_SRCVERTICESDATA + voffset_kvs + v2offset_kvs + k, datas); // NEWCHANGE.
				UTILP2_WriteDatas(vdramDST2, globalparamsDST2.BASEOFFSETKVS_SRCVERTICESDATA + voffset_kvs + v2offset_kvs + k, datas);
				
				for(unsigned int v=0; v<VDATA_PACKINGSIZE; v++){
				#pragma HLS UNROLL 
					value_t vdata = datas[v] >> 1; mask[v] = datas[v] & 0x1;
					unsigned int lvid = lvid_first + (k * VDATA_PACKINGSIZE) + v; vid[0] = UTILP2_GETREALVID(lvid, firstinstance_id + i); // FIXME. MAKE SIMPLER. // vid[0] = vbegin + (i * VDATA_PACKINGSIZE) + k;
					#ifdef _DEBUGMODE_CHECKS3
					unsigned int check_lvid = lvid_first + (k * VDATA_PACKINGSIZE) + v; unsigned int check_v0 = UTILP2_GETREALVID(check_lvid, firstinstance_id + i); // FIXME. MAKE SIMPLER.
...
void acts_all::MERGEP2_exchange(uint512_dt * vdramA, uint512_dt * vdramB, uint512_dt * vdramC){
	...
	EXCHANGEP2_LOOP1: for(unsigned int reduce_partition=0; reduce_partition<globalparamsvA.NUM_REDUCEPARTITIONS; reduce_partition++){
		total_sync[0] = MERGEP2_copy(0, reduce_partition, vdramA, vdramB, vdramC, vdramA, statsA, voffsetA_kvs, 0, NUMCOMPUTEUNITS_SLR2, vpartition_stats, upropblock_stats, globalparamsvA, globalparamsvB, globalparamsvC);
		total_sync[1] = MERGEP2_copy(1, reduce_partition, vdramB, vdramA, vdramC, vdramA, statsB, voffsetB_kvs, NUMCOMPUTEUNITS_SLR2, NUMCOMPUTEUNITS_SLR1, vpartition_stats, upropblock_stats, globalparamsvB, globalparamsvA, globalparamsvC);
		total_sync[2] = MERGEP2_copy(2, reduce_partition, vdramC, vdramA, vdramB, vdramA, statsC, voffsetC_kvs, NUMCOMPUTEUNITS_SLR2 + NUMCOMPUTEUNITS_SLR1, NUMCOMPUTEUNITS_SLR0, vpartition_stats, upropblock_stats, globalparamsvC, globalparamsvA, globalparamsvB);
	}
	...
	MEMACCESSP2_readhelperstats(vdramA, pmask_curr, globalparamsvA.BASEOFFSETKVS_VERTICESPARTITIONMASK, BLOCKRAM_CURRPMASK_SIZE, globalparamsvA.ALGORITHMINFO_GRAPHITERATIONID, globalparamsvA);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
unsigned int acts_all::MERGEP2__broadcastACTVVs(uint512_dt * vdram, uint512_dt * kvdram0, unsigned int source_partition, globalparams_t globalparamsK, globalparams_t globalparamsV){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		unsigned int v_offset = (((s * globalparamsV.NUM_REDUCEPARTITIONS) + source_partition) * NUM_ACTVVSPARTITIONS_PER_VPARTITION) * VDATA_PACKING_PERCHANNEL;
		unsigned int offset = s * VDATA_PACKING_PERCHANNEL;
		
		UTILP2_ReadDatas(vdram, globalparamsV.BASEOFFSETKVS_ACTIVEVERTICESDATA + v_offset + offset, datas);
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				UTILP2_ReadDatas(vdram, globalparamsV.BASEOFFSETKVS_ACTIVEVERTICESDATA + v_offset + offset + t, datas2);
	
				UTILP2_WriteDatas(kvdram0, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
				...
				if(t>0){ for(unsigned int v=0; v<VECTOR2_SIZE; v++){ 
					...
					cout<<">>> BROADCAST:ACTVV BUFFER:: ACTVV SEEN @: v: "<<v<<", lvid: "<<lvid<<", vdata: "<<vdata<<", num items: "<<datas[v]<<endl; 
				}}
				#endif
			}
		}
...
unsigned int acts_all::MERGEP2__broadcastACTVVs(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1, unsigned int source_partition, globalparams_t globalparamsK, globalparams_t globalparamsV){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	cout<< TIMINGRESULTSCOLOR << ">>> broadcastACTVVs:: broadcasting frontier vertices..."<< RESET <<endl; 
	#endif
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP2_WriteDatas(kvdram1, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
...
unsigned int acts_all::MERGEP2__broadcastACTVVs(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2, unsigned int source_partition, globalparams_t globalparamsK, globalparams_t globalparamsV){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	cout<< TIMINGRESULTSCOLOR << ">>> broadcastACTVVs:: broadcasting frontier vertices..."<< RESET <<endl; 
	#endif
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP2_WriteDatas(kvdram2, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
...
unsigned int acts_all::MERGEP2__broadcastACTVVs(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2,uint512_dt * kvdram3, unsigned int source_partition, globalparams_t globalparamsK, globalparams_t globalparamsV){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	cout<< TIMINGRESULTSCOLOR << ">>> broadcastACTVVs:: broadcasting frontier vertices..."<< RESET <<endl; 
	#endif
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP2_WriteDatas(kvdram3, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
...
unsigned int acts_all::MERGEP2__broadcastACTVVs(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2,uint512_dt * kvdram3,uint512_dt * kvdram4, unsigned int source_partition, globalparams_t globalparamsK, globalparams_t globalparamsV){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	cout<< TIMINGRESULTSCOLOR << ">>> broadcastACTVVs:: broadcasting frontier vertices..."<< RESET <<endl; 
	#endif
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP2_WriteDatas(kvdram4, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
...
unsigned int acts_all::MERGEP2__broadcastACTVVs(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2,uint512_dt * kvdram3,uint512_dt * kvdram4,uint512_dt * kvdram5, unsigned int source_partition, globalparams_t globalparamsK, globalparams_t globalparamsV){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	cout<< TIMINGRESULTSCOLOR << ">>> broadcastACTVVs:: broadcasting frontier vertices..."<< RESET <<endl; 
	#endif
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP2_WriteDatas(kvdram5, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
...
unsigned int acts_all::MERGEP2__broadcastACTVVs(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2,uint512_dt * kvdram3,uint512_dt * kvdram4,uint512_dt * kvdram5,uint512_dt * kvdram6, unsigned int source_partition, globalparams_t globalparamsK, globalparams_t globalparamsV){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	cout<< TIMINGRESULTSCOLOR << ">>> broadcastACTVVs:: broadcasting frontier vertices..."<< RESET <<endl; 
	#endif
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP2_WriteDatas(kvdram6, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
...
unsigned int acts_all::MERGEP2__broadcastACTVVs(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2,uint512_dt * kvdram3,uint512_dt * kvdram4,uint512_dt * kvdram5,uint512_dt * kvdram6,uint512_dt * kvdram7, unsigned int source_partition, globalparams_t globalparamsK, globalparams_t globalparamsV){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	cout<< TIMINGRESULTSCOLOR << ">>> broadcastACTVVs:: broadcasting frontier vertices..."<< RESET <<endl; 
	#endif
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP2_WriteDatas(kvdram7, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP2_WriteDatas(kvdram8, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP2_WriteDatas(kvdram9, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP2_WriteDatas(kvdram10, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP2_WriteDatas(kvdram11, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
...
unsigned int acts_all::MERGEP2_mergeACTVVs(uint512_dt * kvdram, uint512_dt * vdram){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	...
	MERGEP2_MERGEVSLOOP2: for(unsigned int reduce_partition=0; reduce_partition<globalparams.NUM_REDUCEPARTITIONS; reduce_partition++){
		unsigned int v_offset = (globalparams.ACTSPARAMS_INSTID * globalparams.NUM_REDUCEPARTITIONS * NUM_ACTVVSPARTITIONS_PER_VPARTITION) * VDATA_PACKING_PERCHANNEL;
		
		for(unsigned int p_=0; p_<NUM_ACTVVSPARTITIONS_PER_VPARTITION; p_++){
			unsigned int offset = ((reduce_partition * NUM_ACTVVSPARTITIONS_PER_VPARTITION) + p_) * VDATA_PACKING_PERCHANNEL;

			UTILP2_ReadDatas(kvdram, globalparams.BASEOFFSETKVS_ACTIVEVERTICESDATA + offset, datas);
			
			unsigned int max = 0; for(unsigned int v=0; v<VECTOR2_SIZE; v++){ if(datas[v]>max){ max = datas[v]; } }
			if(max > VDATA_PACKING_PERCHANNEL || offset >= globalparams.SIZE_BATCHRANGE){ max = 0; }
			#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3 
			...
			if(max > 0){
				for (buffer_type t=0; t<max+1; t++){
				#pragma HLS PIPELINE II=1
					UTILP2_ReadDatas(kvdram, globalparams.BASEOFFSETKVS_ACTIVEVERTICESDATA + offset + t, datas2);
					UTILP2_WriteDatas(vdram, globalparamsv.BASEOFFSETKVS_ACTIVEVERTICESDATA + v_offset + offset + t, datas2);
					transfsz_kvs += 1;
					
					#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3 
					if(t>0){ for(unsigned int v=0; v<VECTOR2_SIZE; v++){ 
						value_t combo = datas2[v]; value_t lvid = combo >> ACTVVATA_BITSZ; value_t vdata = combo & 0xFFFF;
						cout<<">>> MERGE:ACTVV BUFFER:: ACTVV SEEN @: v: "<<v<<", lvid: "<<lvid<<", vdata: "<<vdata<<", num items: "<<datas[v]<<endl; 
					}}
					#endif
				}
			}
...
unsigned int acts_all::MERGEP2_copyACTVVs(uint512_dt * vdramA, uint512_dt * vdramB, uint512_dt * vdramC, unsigned int begin_computeunitid, unsigned int num_compute_units){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	...
	globalparams_t globalparamsv = UTILP2_getglobalparams(vdramA, 0);
	#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3 
	...
	for(unsigned int i=begin_computeunitid; i<(begin_computeunitid + num_compute_units); i++){
		for(unsigned int reduce_partition=0; reduce_partition<globalparamsv.NUM_REDUCEPARTITIONS; reduce_partition++){
			unsigned int v_offset = (i * globalparamsv.NUM_REDUCEPARTITIONS * NUM_ACTVVSPARTITIONS_PER_VPARTITION) * VDATA_PACKING_PERCHANNEL;
			
			for(unsigned int p_=0; p_<NUM_ACTVVSPARTITIONS_PER_VPARTITION; p_++){
				unsigned int offset = ((reduce_partition * NUM_ACTVVSPARTITIONS_PER_VPARTITION) + p_) * VDATA_PACKING_PERCHANNEL;

				UTILP2_ReadDatas(vdramA, globalparamsv.BASEOFFSETKVS_ACTIVEVERTICESDATA + v_offset + offset, datas);
			
				unsigned int max = 0; for(unsigned int v=0; v<VECTOR2_SIZE; v++){ if(datas[v]>max){ max = datas[v]; } }
				if(max > VDATA_PACKING_PERCHANNEL || offset >= globalparamsv.SIZE_BATCHRANGE){ max = 0; }
				#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3 
				...
				if(max > 0){
					for (buffer_type t=0; t<max+1; t++){
					#pragma HLS PIPELINE II=1
						UTILP2_ReadDatas(vdramA, globalparamsv.BASEOFFSETKVS_ACTIVEVERTICESDATA + v_offset + offset + t, datas2);
						UTILP2_WriteDatas(vdramB, globalparamsv.BASEOFFSETKVS_ACTIVEVERTICESDATA + v_offset + offset + t, datas2);
						UTILP2_WriteDatas(vdramC, globalparamsv.BASEOFFSETKVS_ACTIVEVERTICESDATA + v_offset + offset + t, datas2);
						transfsz_kvs += 1;
							
						#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3 
						if(t>0){ for(unsigned int v=0; v<VECTOR2_SIZE; v++){ 
							value_t combo = datas2[v]; value_t lvid = combo >> ACTVVATA_BITSZ; value_t vdata = combo & 0xFFFF;
							unsigned int actvvid = UTILP2_GETREALVID(((offset + t - 1) * VECTOR2_SIZE) + v, i);
							cout<<">>> SYNC:ACTVV BUFFER:: ACTVV SEEN @: v: "<<v<<", lvid: "<<lvid<<", vdata: "<<vdata<<", num items: "<<datas[v]<<", lvid: "<<((offset + t - 1) * VECTOR2_SIZE) + v<<", actvvid: "<<UTILP2_GETREALVID(((offset + t - 1) * VECTOR2_SIZE) + v, i)<<endl; 
							if(t-1<datas[v]){ vbufferINT32[actvvid] = vdata; }
							// vbufferINT32[actvvid] = actvvid;
						}}
						#endif
					}
				}
...
unsigned int acts_all::MERGEP2_exchangeACTVVs(uint512_dt * vdramA, uint512_dt * vdramB, uint512_dt * vdramC){
	#ifdef _DEBUGMODE_KERNELPRINTS4//3
	cout<< TIMINGRESULTSCOLOR << ">>> exchangeVs:: exchanging vertices across different SLRs..." << RESET <<endl; 
	#endif

	unsigned int transfsz_kvs = 0;
	transfsz_kvs += MERGEP2_copyACTVVs(vdramA, vdramB, vdramC, 0, NUMCOMPUTEUNITS_SLR2);
	transfsz_kvs += MERGEP2_copyACTVVs(vdramB, vdramA, vdramC, NUMCOMPUTEUNITS_SLR2, NUMCOMPUTEUNITS_SLR1);
	transfsz_kvs += MERGEP2_copyACTVVs(vdramC, vdramA, vdramB, NUMCOMPUTEUNITS_SLR2 + NUMCOMPUTEUNITS_SLR1, NUMCOMPUTEUNITS_SLR0);
	return transfsz_kvs;
}
...
#pragma HLS DATA_PACK variable = vdramC
	...
	if(globalparamsvA.ENABLE_EXCHANGECOMMAND == ON){
		MERGEP2_exchangeACTVVs(vdramA, vdramB, vdramC);
	}
	#endif 
	return;
}
...
void acts_all::ACTSP2_actit(bool_type enable, unsigned int mode,
	...
	if(globalparamsK.ENABLE_RUNKERNELCOMMAND == ON && en == true){
		...
		if(mode == ACTSPROCESSMODE || mode == ACTSPARTITIONMODE){
			...
			MEMACCESSP2_get_vertexblock_ids(ON, globalposition.source_partition, kvdram, vbuffer, vertexblock_ids, stats, collections_tmp, globalposition.num_active_edgeblocks, globalparamsK, globalparamsE);
			
			#ifdef ACTS_ENABLE_READUPROPS______________XXX
			unsigned int num_its = MEMACCESSP2_get_upropblock_workload(true, sweepparams.source_partition, kvdram, vbuffer, stats, globalposition.num_active_edgeblocks, globalparamsK, xload_kvs, buffer_offsets, globalparamsK.ALGORITHMINFO_GRAPHITERATIONID);
			...
			if(sparse_readu == true){
				for(unsigned int n=0; n<num_its; n++){		
					MEMACCESSP2_readV(ON, kvdram, vbuffer, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA, xload_kvs[n].offset_begin, xload_kvs[n].offset_begin, xload_kvs[n].size);
					#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
				...
				MEMACCESSP2_readV(ON, kvdram, vbuffer, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA, 0, 0, globalparamsK.SIZEKVS2_PROCESSEDGESPARTITION);
				#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
			...
			for(unsigned int s=0; s<NUM_PEs; s++){
				...
				UTILP2_ReadDatas(kvdram, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset, datas);
				unsigned int max = 0; for(unsigned int v=0; v<VECTOR2_SIZE; v++){ if(datas[v]>max){ max = datas[v]; } }
				if(max > VDATA_PACKING_PERCHANNEL){ max = 0; }
				#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3 
				...
				if(max > 0){
					...
					#pragma HLS PIPELINE II=1
						UTILP2_ReadDatas(kvdram, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + 1 + t, datas2);
			...
			ACTIT_MAINLOOP1: for(unsigned int llp_set=0; llp_set<num_LLPset_; llp_set++){ 
				...
				unsigned int num_its = MEMACCESSP2_get_edgeblock_workload(true, kvdram, sweepparams.source_partition, llp_set, vertexblock_ids, stats, globalposition.num_active_edgeblocks, globalparamsK, globalparamsE, xload_kvs, globalparamsK.ALGORITHMINFO_GRAPHITERATIONID);
				...
				READ_PROCESS_PARTITION_STORE_LOOP1B: for(unsigned int n=0; n<num_its; n++){
					...
					unsigned int activetransfsz_kvs = MEMACCESSP2_write__process_base(mode, llp_set, kvdram, vbuffer, stats, workload_kvs, collections_tmp, globalparamsK);
					#ifdef _DEBUGMODE_KERNELPRINTS
		...
		if(mode == ACTSREDUCEMODE){		
			...
			unsigned int num_its = MEMACCESSP2_get_updateblock_workload(true, sweepparams.source_partition, stats[16], stats[17], globalparamsK, xload_kvs, buffer_offsets);
			...
			for(unsigned int n=0; n<num_its; n++){
				...
				#pragma HLS PIPELINE II=1
					UTILP2_ReadDatas(kvdram, globalparamsK.BASEOFFSETKVS_DESTVERTICESDATA + voffset_kvs + xload_kvs[n].offset_begin + i, datas);
					for(unsigned int v=0; v<VECTOR2_SIZE; v++){
					#pragma HLS UNROLL 
			...
			MEMACCESSP2_read__reduce(ACTSREDUCEMODE, kvdram, vbuffer, stats, workload_kvs, collections_tmp, globalparamsK);
			#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
			...
			for(unsigned int n=0; n<num_its; n++){
				...
				#pragma HLS PIPELINE II=1
					...
					UTILP2_WriteDatas(kvdram, globalparamsK.BASEOFFSETKVS_DESTVERTICESDATA + voffset_kvs + xload_kvs[n].offset_begin + i, datas);
				}
				#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
				collections_tmp[REDUCEPHASE_TRANSFSZ_COLLECTIONID].data1 += (xload_kvs[n].size + REPORT__DRAM_ACCESS_LATENCY) * VECTOR2_SIZE; 
				collections_tmp[NUMVERTICESPROCESSED_COLLECTIONID].data1 += (xload_kvs[n].size + REPORT__DRAM_ACCESS_LATENCY) * VECTOR2_SIZE;
			...
			for(unsigned int n=0; n<num_its; n++){
				...
				#pragma HLS PIPELINE II=1
					...
					#pragma HLS UNROLL 
						...
						if(mask == 1){
							...
							#endif
							#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3  // REMOVEME.
							cout<<">>> ACTIT:ACTVV BUFFER:: ACTVV SEEN @: v: "<<v<<", lvid: "<<index_offset + lvid<<", vdata: "<<vdata<<", mask: "<<mask<<", dstvid: "<<UTILP2_GETREALVID((index_offset + lvid), globalparamsK.ACTSPARAMS_INSTID)<<", new combo: "<<(((index_offset + lvid) << ACTVVATA_BITSZ) | vdata)<<endl; 
							#endif
						}
						#ifdef _DEBUGMODE_CHECKS3
				...
				for(unsigned int p_=0; p_<NUM_ACTVVSPARTITIONS_PER_VPARTITION; p_++){
					...
					UTILP2_WriteDatas(kvdram, globalparamsK.BASEOFFSETKVS_ACTIVEVERTICESDATA + offset, datas);
					for (buffer_type t=0; t<max; t++){
						for(unsigned int v=0; v<VECTOR2_SIZE; v++){
						#pragma HLS UNROLL 
							datas[v] = actvvbuffer[v][t].data;
						}
						UTILP2_WriteDatas(kvdram, globalparamsK.BASEOFFSETKVS_ACTIVEVERTICESDATA + offset + 1 + t, datas);
						#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3  // REMOVEME.
						for(unsigned int v=0; v<VECTOR2_SIZE; v++){ 
	...
	if(mode == ACTSPROCESSMODE && UTILP2__processit__i_am_last__(globalposition) == true){
		#ifdef _DEBUGMODE_KERNELPRINTS
		...
		#pragma HLS PIPELINE II=1	
			datas[0] = stats[16][t];
			datas[1] = stats[17][t];
			UTILP2_WriteDatas(kvdram, globalparamsK.BASEOFFSETKVS_ACTIVEUPDATEBLOCKS + t, datas);
		}
		// exit(EXIT_SUCCESS);
	}
	#endif
...
#endif 			
void acts_all::TOPP2_processit(uint512_dt * kvdram, keyvalue_vbuffer_t vbuffer[VDATA_PACKINGSIZE][MAX_BLOCKRAM_VDESTDATA_SIZE], workload_t edgestats[EDGESTATSRAM_SIZE], stats_t stats[STATS_PACKINGSIZE][BLOCKRAM_SIZE], keyvalue_t globalcapsule[BLOCKRAM_GLOBALSTATS_SIZE], globalparams_t globalparamsE, globalparams_t globalparamsK, globalposition_t globalposition,							
...
void acts_all::TOPP2_partitionit(uint512_dt * kvdram, keyvalue_vbuffer_t vbuffer[VDATA_PACKINGSIZE][MAX_BLOCKRAM_VDESTDATA_SIZE], workload_t edgestats[EDGESTATSRAM_SIZE], stats_t stats[STATS_PACKINGSIZE][BLOCKRAM_SIZE], globalparams_t globalparamsE, globalparams_t globalparamsK, unsigned int edgebankID, collection_t collections[COLLECTIONS_BUFFERSZ]){
	#pragma HLS INLINE
	...
	#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_numllops avg=analysis_numllops	
		...
		#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_numsourcepartitions avg=analysis_numsourcepartitions
			...
			// partition
			#ifdef _DEBUGMODE_KERNELPRINTS2
			if((config.enablepartition == ON) && (currentLOP >= 1) && (currentLOP <= globalparamsK.ACTSPARAMS_TREEDEPTH)){ actsutilityobj->print7("### TOPP2_partitionit:: source_p", "upperlimit", "begin", "end", "size", "dest range", "currentLOP", sweepparams.source_partition, sweepparams.upperlimit, NAp, NAp, NAp, BATCH_RANGE / (1 << (globalparamsK.ACTSPARAMS_POW_PARTITIONS * sweepparams.currentLOP)), sweepparams.currentLOP); }	
			#endif
...
void acts_all::TOPP2_reduceit(uint512_dt * kvdram, keyvalue_vbuffer_t vbuffer[VDATA_PACKINGSIZE][MAX_BLOCKRAM_VDESTDATA_SIZE], workload_t edgestats[EDGESTATSRAM_SIZE], stats_t stats[STATS_PACKINGSIZE][BLOCKRAM_SIZE], keyvalue_t globalcapsule[BLOCKRAM_GLOBALSTATS_SIZE], batch_type sourcestatsmarker, batch_type source_partition, globalparams_t globalparamsE, globalparams_t globalparamsK, unsigned int edgebankID, collection_t collections[COLLECTIONS_BUFFERSZ]){	
	#pragma HLS INLINE
	analysis_type analysis_numllops = 1;
	analysis_type analysis_numsourcepartitions = 1;
...
void acts_all::TOPP2_dispatch(bool_type en_process, bool_type en_partition, bool_type en_reduce,  uint512_dt * kvdram, keyvalue_vbuffer_t vbuffer[VDATA_PACKINGSIZE][MAX_BLOCKRAM_VDESTDATA_SIZE], workload_t edgestats[EDGESTATSRAM_SIZE], stats_t stats[STATS_PACKINGSIZE][BLOCKRAM_SIZE], keyvalue_t globalcapsule[BLOCKRAM_GLOBALSTATS_SIZE], 
	...
	if(en_process == ON){ TOPP2_processit(kvdram, vbuffer, edgestats, stats, globalcapsule, globalparamsE, globalparamsK, globalposition, v_chunkids, v_chunkid, edgebankID, collections); } 
	#endif 
	#ifdef CONFIG_ENABLEPARTITIONMODULE
	if(en_partition == ON){ TOPP2_partitionit(kvdram, vbuffer, edgestats, stats, globalparamsE, globalparamsK, NAp, collections); } 
	#endif 
	#ifdef CONFIG_ENABLEREDUCEMODULE
	if(en_reduce == ON){ TOPP2_reduceit(kvdram, vbuffer, edgestats, stats, globalcapsule, sourcestatsmarker, source_partition, globalparamsE, globalparamsK, NAp, collections); } 
	#endif 
	return;
} 

void acts_all::TOPP2_dispatch_reduce(uint512_dt * kvdram, keyvalue_vbuffer_t vbuffer[VDATA_PACKINGSIZE][MAX_BLOCKRAM_VDESTDATA_SIZE], workload_t edgestats[EDGESTATSRAM_SIZE], stats_t stats[STATS_PACKINGSIZE][BLOCKRAM_SIZE], keyvalue_t globalcapsule[BLOCKRAM_GLOBALSTATS_SIZE], globalparams_t globalparamsE, globalparams_t globalparamsK, globalparams_t globalparamsV, globalposition_t globalposition,	
					unsigned int v_chunkids[EDGESSTATSDRAMSZ], unsigned int v_chunkid, unsigned int edgebankID, collection_t collections[COLLECTIONS_BUFFERSZ]){
	#pragma HLS INLINE
	...
	DISPATCHREDUCEP2_MAINLOOP: for(batch_type source_partition=0; source_partition<num_source_partitions; source_partition+=1){
		#ifdef _DEBUGMODE_KERNELPRINTS3
		actsutilityobj->print4("### TOPP2_dispatch_reduce:: source_partition", "currentLOP", "num_source_partitions", "voffset_kvs", source_partition, currentLOP, num_source_partitions, voffset_kvs); 							
		#endif
		
		TOPP2_dispatch(OFF, OFF, enablereduce, kvdram, vbuffer, edgestats, stats, globalcapsule, sourcestatsmarker, source_partition, globalparamsE, globalparamsK, globalposition, v_chunkids, v_chunkid, NAp, collections);
...
void acts_all::TOPP2_topkernelproc_embedded(unsigned int GraphIter, unsigned int globalid, unsigned int localid, unsigned int en_process, unsigned int en_partition, unsigned int en_reduce, uint512_dt * kvdram, keyvalue_vbuffer_t vbuffer[VDATA_PACKINGSIZE][MAX_BLOCKRAM_VSRCDATA_SIZE], workload_t edgestats[EDGESTATSRAM_SIZE], stats_t stats[STATS_PACKINGSIZE][BLOCKRAM_SIZE], keyvalue_t globalcapsule[BLOCKRAM_GLOBALSTATS_SIZE], globalposition_t globalposition, globalparams_t globalparamsV, collection_t collections[COLLECTIONS_BUFFERSZ]){									
	#ifdef _DEBUGMODE_KERNELPRINTS
	actsutilityobj->printparameters();
	#endif
	...
	#ifdef TESTKERNEL 
	if(UTILP2_isbufferused(globalid) == false){ return; }
	#endif 
	#ifdef SW
	...
	if(__processit__i_am_first__ == true){ UTILP2_resetvalues(globalcapsule, globalparamsK.NUM_REDUCEPARTITIONS, 0); } 
	if(__processit__i_am_first__ == true){ UTILP2_resetkvstatvalues(kvdram, globalparamsK); } // NEWCHANGE
	if(__processit__i_am_first__ == true){ MEMACCESSP2_readglobalstats(ON, kvdram, globalcapsule, globalparamsK.BASEOFFSETKVS_STATSDRAM, globalparamsK); }
	...
	if(globalparamsK.ENABLE_PROCESSCOMMAND == ON && en_process == ON){
		...
		#endif
		TOPP2_dispatch(globalposition.EN_PROCESS, OFF, OFF, kvdram, vbuffer, edgestats, stats, globalcapsule, NAp, NAp, _globalparamsE, globalparamsK, globalposition, PARTITIONP2_CHKPT, globalposition.v_chunkid, globalposition.edgebankID, collections); 
	}
	#endif
	...
	if(globalparamsK.ENABLE_PARTITIONCOMMAND == ON && en_partition == ON){
		...
		#endif
		TOPP2_dispatch(OFF, globalposition.EN_PARTITION, OFF, kvdram, vbuffer, edgestats, stats, globalcapsule, NAp, NAp, _globalparamsE, globalparamsK, globalposition, PARTITIONP2_CHKPT, globalposition.v_chunkid, NAp, collections);
	}
	#endif
	...
	if(globalparamsK.ENABLE_APPLYUPDATESCOMMAND == ON && en_reduce == ON){ 
		...
		#endif
		TOPP2_dispatch_reduce(kvdram, vbuffer, edgestats, stats, globalcapsule, _globalparamsE, globalparamsK, globalparamsV, globalposition, PARTITIONP2_CHKPT, globalposition.v_chunkid, NAp, collections);
	}
	#endif
	...
	if(__reduceit__i_am_last__ == true){ UTILP2_increment_graphiteration(kvdram, globalparamsK); } // NB: this should come last.
	#ifdef _DEBUGMODE_KERNELPRINTS
...
#pragma HLS DATA_PACK variable = vdram
				...
				TOPKERNEL1_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL1_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						if(stage==0 && source_partition % BLOCKRAM_CURRPMASK_SIZE == 0){
							...
							#endif
							MEMACCESSP2_readhelperstats(vdram, pmask_curr, globalparamsV.BASEOFFSETKVS_VERTICESPARTITIONMASK + source_partition, BLOCKRAM_CURRPMASK_SIZE, globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsV);
						}
						...
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							...
							unsigned int num_its = MEMACCESSP2_get_upropblock_workload(true, globalposition.source_partition, vdram, vbuffer0, stats0, globalposition.num_active_edgeblocks, globalparamsV, xload_kvs, buffer_offsets, globalparamsV.ALGORITHMINFO_GRAPHITERATIONID);
							...
							if(sparse_readu == true){
								for(unsigned int n=0; n<num_its; n++){
									...
									#endif			
									MERGEP2_broadcastVs1(vdram, kvdram0, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, NUM_VERTICESKVS_PER_UPROPBLOCK);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
								...
								for(unsigned int s=0; s<NUM_PEs; s++){
									...
									MERGEP2_broadcastVs1(vdram, kvdram0, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, vsz_kvs);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP2__broadcastACTVVs(vdram, kvdram0, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
						...
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 0, 0, enableprocess, enablepartition, enablereduce, kvdram0, vbuffer0, edgestats0, stats0, globalcapsule0, globalposition, globalparamsV, collections0);		
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeACTVVs(kvdram0, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	...
	UTILP2_WriteDatas(kvdram0, BASEOFFSET_MESSAGESDATA_KVS + MESSAGES_RETURNVALUES + MESSAGES_RETURNVALUES_STATSCOLLECTED + globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, host_stats);
	#endif 
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL2_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL2_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL2_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							...
							if(sparse_readu == true){
								for(unsigned int n=0; n<num_its; n++){
									...
									actsutilityobj->checkoutofbounds("topkernelP:: ERROR 30", xload_kvs[n].size, globalparamsK.SIZEKVS2_PROCESSEDGESPARTITION, source_partition, xload_kvs[n].offset_begin, s);
									#endif			
									MERGEP2_broadcastVs2(vdram, kvdram0,kvdram1, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, NUM_VERTICESKVS_PER_UPROPBLOCK);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[n].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
								...
								for(unsigned int s=0; s<NUM_PEs; s++){
									unsigned int offsetSRC_kvs = (s * globalparamsV.NUM_REDUCEPARTITIONS * globalparamsV.SIZEKVS2_REDUCEPARTITION) + voffset_kvs;
									unsigned int offsetDST_kvs = s * vsz_kvs; 
									MERGEP2_broadcastVs2(vdram, kvdram0,kvdram1, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, vsz_kvs);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[0].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP2__broadcastACTVVs(vdram, kvdram0,kvdram1, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
						...
						#ifdef CONFIG_RELEASE_FULLKERNEL
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 0, 0, enableprocess, enablepartition, enablereduce, kvdram0, vbuffer0, edgestats0, stats0, globalcapsule0, globalposition, globalparamsV, collections0);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 1, 1, enableprocess, enablepartition, enablereduce, kvdram1, vbuffer1, edgestats1, stats1, globalcapsule1, globalposition, globalparamsV, collections1);		
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeACTVVs(kvdram1, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL3_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL3_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL3_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							...
							if(sparse_readu == true){
								for(unsigned int n=0; n<num_its; n++){
									...
									actsutilityobj->checkoutofbounds("topkernelP:: ERROR 30", xload_kvs[n].size, globalparamsK.SIZEKVS2_PROCESSEDGESPARTITION, source_partition, xload_kvs[n].offset_begin, s);
									#endif			
									MERGEP2_broadcastVs3(vdram, kvdram0,kvdram1,kvdram2, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, NUM_VERTICESKVS_PER_UPROPBLOCK);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[n].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
								...
								for(unsigned int s=0; s<NUM_PEs; s++){
									unsigned int offsetSRC_kvs = (s * globalparamsV.NUM_REDUCEPARTITIONS * globalparamsV.SIZEKVS2_REDUCEPARTITION) + voffset_kvs;
									unsigned int offsetDST_kvs = s * vsz_kvs; 
									MERGEP2_broadcastVs3(vdram, kvdram0,kvdram1,kvdram2, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, vsz_kvs);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[0].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP2__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
						...
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 0, 0, enableprocess, enablepartition, enablereduce, kvdram0, vbuffer0, edgestats0, stats0, globalcapsule0, globalposition, globalparamsV, collections0);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 1, 1, enableprocess, enablepartition, enablereduce, kvdram1, vbuffer1, edgestats1, stats1, globalcapsule1, globalposition, globalparamsV, collections1);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 2, 2, enableprocess, enablepartition, enablereduce, kvdram2, vbuffer2, edgestats2, stats2, globalcapsule2, globalposition, globalparamsV, collections2);		
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeACTVVs(kvdram2, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL4_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL4_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL4_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							...
							if(sparse_readu == true){
								for(unsigned int n=0; n<num_its; n++){
									...
									actsutilityobj->checkoutofbounds("topkernelP:: ERROR 30", xload_kvs[n].size, globalparamsK.SIZEKVS2_PROCESSEDGESPARTITION, source_partition, xload_kvs[n].offset_begin, s);
									#endif			
									MERGEP2_broadcastVs4(vdram, kvdram0,kvdram1,kvdram2,kvdram3, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, NUM_VERTICESKVS_PER_UPROPBLOCK);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[n].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
								...
								for(unsigned int s=0; s<NUM_PEs; s++){
									unsigned int offsetSRC_kvs = (s * globalparamsV.NUM_REDUCEPARTITIONS * globalparamsV.SIZEKVS2_REDUCEPARTITION) + voffset_kvs;
									unsigned int offsetDST_kvs = s * vsz_kvs; 
									MERGEP2_broadcastVs4(vdram, kvdram0,kvdram1,kvdram2,kvdram3, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, vsz_kvs);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[0].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP2__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
						...
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 1, 1, enableprocess, enablepartition, enablereduce, kvdram1, vbuffer1, edgestats1, stats1, globalcapsule1, globalposition, globalparamsV, collections1);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 2, 2, enableprocess, enablepartition, enablereduce, kvdram2, vbuffer2, edgestats2, stats2, globalcapsule2, globalposition, globalparamsV, collections2);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 3, 3, enableprocess, enablepartition, enablereduce, kvdram3, vbuffer3, edgestats3, stats3, globalcapsule3, globalposition, globalparamsV, collections3);		
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeACTVVs(kvdram3, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL5_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL5_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL5_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							...
							if(sparse_readu == true){
								for(unsigned int n=0; n<num_its; n++){
									...
									actsutilityobj->checkoutofbounds("topkernelP:: ERROR 30", xload_kvs[n].size, globalparamsK.SIZEKVS2_PROCESSEDGESPARTITION, source_partition, xload_kvs[n].offset_begin, s);
									#endif			
									MERGEP2_broadcastVs5(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, NUM_VERTICESKVS_PER_UPROPBLOCK);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[n].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
								...
								for(unsigned int s=0; s<NUM_PEs; s++){
									unsigned int offsetSRC_kvs = (s * globalparamsV.NUM_REDUCEPARTITIONS * globalparamsV.SIZEKVS2_REDUCEPARTITION) + voffset_kvs;
									unsigned int offsetDST_kvs = s * vsz_kvs; 
									MERGEP2_broadcastVs5(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, vsz_kvs);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[0].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP2__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
						...
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 2, 2, enableprocess, enablepartition, enablereduce, kvdram2, vbuffer2, edgestats2, stats2, globalcapsule2, globalposition, globalparamsV, collections2);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 3, 3, enableprocess, enablepartition, enablereduce, kvdram3, vbuffer3, edgestats3, stats3, globalcapsule3, globalposition, globalparamsV, collections3);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 4, 4, enableprocess, enablepartition, enablereduce, kvdram4, vbuffer4, edgestats4, stats4, globalcapsule4, globalposition, globalparamsV, collections4);		
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeACTVVs(kvdram4, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL6_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL6_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL6_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							...
							if(sparse_readu == true){
								for(unsigned int n=0; n<num_its; n++){
									...
									actsutilityobj->checkoutofbounds("topkernelP:: ERROR 30", xload_kvs[n].size, globalparamsK.SIZEKVS2_PROCESSEDGESPARTITION, source_partition, xload_kvs[n].offset_begin, s);
									#endif			
									MERGEP2_broadcastVs6(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, NUM_VERTICESKVS_PER_UPROPBLOCK);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[n].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
								...
								for(unsigned int s=0; s<NUM_PEs; s++){
									unsigned int offsetSRC_kvs = (s * globalparamsV.NUM_REDUCEPARTITIONS * globalparamsV.SIZEKVS2_REDUCEPARTITION) + voffset_kvs;
									unsigned int offsetDST_kvs = s * vsz_kvs; 
									MERGEP2_broadcastVs6(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, vsz_kvs);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[0].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP2__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
						...
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 3, 3, enableprocess, enablepartition, enablereduce, kvdram3, vbuffer3, edgestats3, stats3, globalcapsule3, globalposition, globalparamsV, collections3);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 4, 4, enableprocess, enablepartition, enablereduce, kvdram4, vbuffer4, edgestats4, stats4, globalcapsule4, globalposition, globalparamsV, collections4);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 5, 5, enableprocess, enablepartition, enablereduce, kvdram5, vbuffer5, edgestats5, stats5, globalcapsule5, globalposition, globalparamsV, collections5);		
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeACTVVs(kvdram5, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL7_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL7_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL7_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							...
							if(sparse_readu == true){
								for(unsigned int n=0; n<num_its; n++){
									...
									actsutilityobj->checkoutofbounds("topkernelP:: ERROR 30", xload_kvs[n].size, globalparamsK.SIZEKVS2_PROCESSEDGESPARTITION, source_partition, xload_kvs[n].offset_begin, s);
									#endif			
									MERGEP2_broadcastVs7(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, NUM_VERTICESKVS_PER_UPROPBLOCK);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[n].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
								...
								for(unsigned int s=0; s<NUM_PEs; s++){
									unsigned int offsetSRC_kvs = (s * globalparamsV.NUM_REDUCEPARTITIONS * globalparamsV.SIZEKVS2_REDUCEPARTITION) + voffset_kvs;
									unsigned int offsetDST_kvs = s * vsz_kvs; 
									MERGEP2_broadcastVs7(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, vsz_kvs);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[0].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP2__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
						...
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 4, 4, enableprocess, enablepartition, enablereduce, kvdram4, vbuffer4, edgestats4, stats4, globalcapsule4, globalposition, globalparamsV, collections4);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 5, 5, enableprocess, enablepartition, enablereduce, kvdram5, vbuffer5, edgestats5, stats5, globalcapsule5, globalposition, globalparamsV, collections5);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 6, 6, enableprocess, enablepartition, enablereduce, kvdram6, vbuffer6, edgestats6, stats6, globalcapsule6, globalposition, globalparamsV, collections6);		
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeACTVVs(kvdram6, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL8_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL8_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL8_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							...
							if(sparse_readu == true){
								for(unsigned int n=0; n<num_its; n++){
									...
									actsutilityobj->checkoutofbounds("topkernelP:: ERROR 30", xload_kvs[n].size, globalparamsK.SIZEKVS2_PROCESSEDGESPARTITION, source_partition, xload_kvs[n].offset_begin, s);
									#endif			
									MERGEP2_broadcastVs8(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, NUM_VERTICESKVS_PER_UPROPBLOCK);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[n].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
								...
								for(unsigned int s=0; s<NUM_PEs; s++){
									unsigned int offsetSRC_kvs = (s * globalparamsV.NUM_REDUCEPARTITIONS * globalparamsV.SIZEKVS2_REDUCEPARTITION) + voffset_kvs;
									unsigned int offsetDST_kvs = s * vsz_kvs; 
									MERGEP2_broadcastVs8(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, vsz_kvs);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[0].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP2__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
						...
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 5, 5, enableprocess, enablepartition, enablereduce, kvdram5, vbuffer5, edgestats5, stats5, globalcapsule5, globalposition, globalparamsV, collections5);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 6, 6, enableprocess, enablepartition, enablereduce, kvdram6, vbuffer6, edgestats6, stats6, globalcapsule6, globalposition, globalparamsV, collections6);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 7, 7, enableprocess, enablepartition, enablereduce, kvdram7, vbuffer7, edgestats7, stats7, globalcapsule7, globalposition, globalparamsV, collections7);		
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeACTVVs(kvdram7, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL9_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL9_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL9_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							...
							if(sparse_readu == true){
								for(unsigned int n=0; n<num_its; n++){
									...
									actsutilityobj->checkoutofbounds("topkernelP:: ERROR 30", xload_kvs[n].size, globalparamsK.SIZEKVS2_PROCESSEDGESPARTITION, source_partition, xload_kvs[n].offset_begin, s);
									#endif			
									MERGEP2_broadcastVs9(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7,kvdram8, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, NUM_VERTICESKVS_PER_UPROPBLOCK);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[n].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
								...
								for(unsigned int s=0; s<NUM_PEs; s++){
									unsigned int offsetSRC_kvs = (s * globalparamsV.NUM_REDUCEPARTITIONS * globalparamsV.SIZEKVS2_REDUCEPARTITION) + voffset_kvs;
									unsigned int offsetDST_kvs = s * vsz_kvs; 
									MERGEP2_broadcastVs9(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7,kvdram8, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, vsz_kvs);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[0].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP2__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7,kvdram8, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
						...
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 6, 6, enableprocess, enablepartition, enablereduce, kvdram6, vbuffer6, edgestats6, stats6, globalcapsule6, globalposition, globalparamsV, collections6);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 7, 7, enableprocess, enablepartition, enablereduce, kvdram7, vbuffer7, edgestats7, stats7, globalcapsule7, globalposition, globalparamsV, collections7);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 8, 8, enableprocess, enablepartition, enablereduce, kvdram8, vbuffer8, edgestats8, stats8, globalcapsule8, globalposition, globalparamsV, collections8);		
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeACTVVs(kvdram8, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL10_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL10_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL10_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							...
							if(sparse_readu == true){
								for(unsigned int n=0; n<num_its; n++){
									...
									actsutilityobj->checkoutofbounds("topkernelP:: ERROR 30", xload_kvs[n].size, globalparamsK.SIZEKVS2_PROCESSEDGESPARTITION, source_partition, xload_kvs[n].offset_begin, s);
									#endif			
									MERGEP2_broadcastVs10(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7,kvdram8,kvdram9, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, NUM_VERTICESKVS_PER_UPROPBLOCK);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[n].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
								...
								for(unsigned int s=0; s<NUM_PEs; s++){
									unsigned int offsetSRC_kvs = (s * globalparamsV.NUM_REDUCEPARTITIONS * globalparamsV.SIZEKVS2_REDUCEPARTITION) + voffset_kvs;
									unsigned int offsetDST_kvs = s * vsz_kvs; 
									MERGEP2_broadcastVs10(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7,kvdram8,kvdram9, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, vsz_kvs);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[0].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP2__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7,kvdram8,kvdram9, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
						...
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 7, 7, enableprocess, enablepartition, enablereduce, kvdram7, vbuffer7, edgestats7, stats7, globalcapsule7, globalposition, globalparamsV, collections7);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 8, 8, enableprocess, enablepartition, enablereduce, kvdram8, vbuffer8, edgestats8, stats8, globalcapsule8, globalposition, globalparamsV, collections8);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 9, 9, enableprocess, enablepartition, enablereduce, kvdram9, vbuffer9, edgestats9, stats9, globalcapsule9, globalposition, globalparamsV, collections9);		
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeACTVVs(kvdram9, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL11_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL11_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL11_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							...
							if(sparse_readu == true){
								for(unsigned int n=0; n<num_its; n++){
									...
									actsutilityobj->checkoutofbounds("topkernelP:: ERROR 30", xload_kvs[n].size, globalparamsK.SIZEKVS2_PROCESSEDGESPARTITION, source_partition, xload_kvs[n].offset_begin, s);
									#endif			
									MERGEP2_broadcastVs11(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7,kvdram8,kvdram9,kvdram10, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, NUM_VERTICESKVS_PER_UPROPBLOCK);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[n].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
								...
								for(unsigned int s=0; s<NUM_PEs; s++){
									unsigned int offsetSRC_kvs = (s * globalparamsV.NUM_REDUCEPARTITIONS * globalparamsV.SIZEKVS2_REDUCEPARTITION) + voffset_kvs;
									unsigned int offsetDST_kvs = s * vsz_kvs; 
									MERGEP2_broadcastVs11(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7,kvdram8,kvdram9,kvdram10, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, vsz_kvs);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[0].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP2__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7,kvdram8,kvdram9,kvdram10, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
						...
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 8, 8, enableprocess, enablepartition, enablereduce, kvdram8, vbuffer8, edgestats8, stats8, globalcapsule8, globalposition, globalparamsV, collections8);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 9, 9, enableprocess, enablepartition, enablereduce, kvdram9, vbuffer9, edgestats9, stats9, globalcapsule9, globalposition, globalparamsV, collections9);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 10, 10, enableprocess, enablepartition, enablereduce, kvdram10, vbuffer10, edgestats10, stats10, globalcapsule10, globalposition, globalparamsV, collections10);		
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeACTVVs(kvdram10, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL12_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL12_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL12_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							...
							if(sparse_readu == true){
								for(unsigned int n=0; n<num_its; n++){
									...
									actsutilityobj->checkoutofbounds("topkernelP:: ERROR 30", xload_kvs[n].size, globalparamsK.SIZEKVS2_PROCESSEDGESPARTITION, source_partition, xload_kvs[n].offset_begin, s);
									#endif			
									MERGEP2_broadcastVs12(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7,kvdram8,kvdram9,kvdram10,kvdram11, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, NUM_VERTICESKVS_PER_UPROPBLOCK);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[n].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
								...
								for(unsigned int s=0; s<NUM_PEs; s++){
									unsigned int offsetSRC_kvs = (s * globalparamsV.NUM_REDUCEPARTITIONS * globalparamsV.SIZEKVS2_REDUCEPARTITION) + voffset_kvs;
									unsigned int offsetDST_kvs = s * vsz_kvs; 
									MERGEP2_broadcastVs12(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7,kvdram8,kvdram9,kvdram10,kvdram11, globalparamsV.BASEOFFSETKVS_SRCVERTICESDATA + offsetSRC_kvs, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offsetDST_kvs, vsz_kvs);
									#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
									transfsz_kvs += xload_kvs[0].size + REPORT__DRAM_ACCESS_LATENCY;
									#endif 
								}
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP2__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7,kvdram8,kvdram9,kvdram10,kvdram11, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
						...
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 9, 9, enableprocess, enablepartition, enablereduce, kvdram9, vbuffer9, edgestats9, stats9, globalcapsule9, globalposition, globalparamsV, collections9);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 10, 10, enableprocess, enablepartition, enablereduce, kvdram10, vbuffer10, edgestats10, stats10, globalcapsule10, globalposition, globalparamsV, collections10);		
						TOPP2_topkernelproc_embedded(globalparamsK.ALGORITHMINFO_GRAPHITERATIONID, globalparamsK.ACTSPARAMS_INSTID + 11, 11, enableprocess, enablepartition, enablereduce, kvdram11, vbuffer11, edgestats11, stats11, globalcapsule11, globalposition, globalparamsV, collections11);		
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeACTVVs(kvdram11, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
