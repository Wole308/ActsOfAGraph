...
void acts_all::reduce_update(unsigned int mode, bool enx, unsigned int v, unsigned int loc, keyvalue_t update_data, keyvalue_vbuffer_t vbuffer[MAX_BLOCKRAM_VDESTDATA_SIZE], unsigned int stats[BLOCKRAM_SIZE], unsigned int memory[1], globalparams_t globalparams){
	...
	if(en == true){ // REMOVEME.
		...
		if(mode == ACTSREDUCEMODE){ cout<<">>> REDUCE VECTOR:: REDUCE UPDATE SEEN @: v: "<<v<<", loc: "<<loc<<", vdata_tmp: "<<vdata_tmp<<", mask: "<<mask<<", update_data.key: "<<update_data.key<<", update_data.value: "<<update_data.value<<", new_vprop: "<<new_vprop<<", new combo: "<<((new_vprop << 1) | 0x1)<<", dstvid: "<<UTILP2_GETREALVID(update_data.key, globalparams.ACTSPARAMS_INSTID)<<endl; }
		#endif
...
unsigned int acts_all::MEMACCESSP2_get_upropblock_workload(bool en, unsigned int process_partition, uint512_dt * dram, keyvalue_vbuffer_t buffer[VDATA_PACKINGSIZE][MAX_BLOCKRAM_VDESTDATA_SIZE], stats_t stats[STATS_PACKINGSIZE][BLOCKRAM_SIZE], unsigned int num_active_upropblocks, globalparams_t globalparams, workload_t xload_kvs[BLOCKRAM_SIZE], unsigned int buffer_offsets[BLOCKRAM_SIZE], unsigned int graphiterationid){
	...
	if(sparse_readu == true){
		...
		#else 
		MEMACCESSP2_read__reduce(READDATAMODE, dram, buffer, stats, wkl_kvs, collections_tmp, globalparams);
...
void acts_all::MEMACCESSP2_get_vertexblock_ids(bool en, unsigned int process_partition, uint512_dt * dram, keyvalue_vbuffer_t buffer[VDATA_PACKINGSIZE][MAX_BLOCKRAM_VDESTDATA_SIZE], unsigned int vertexblock_ids[VDATA_PACKINGSIZE][BLOCKRAM_SIZE], stats_t stats[STATS_PACKINGSIZE][BLOCKRAM_SIZE], collection_t collections_tmp[COLLECTIONS_BUFFERSZ], unsigned int num_active_edgeblocks, globalparams_t globalparamsK, globalparams_t globalparamsE){			
	...
	#else 
	MEMACCESSP2_read__reduce(READDATAMODE, dram, buffer, stats, wkl_kvs, collections_tmp, globalparamsE);
		for(unsigned int t=0; t<wkl_kvs.size; t++){
...
unsigned int acts_all::MERGEP2_mergeVs(uint512_dt * kvdram, uint512_dt * vdram){
	...
	// #ifdef TESTKERNEL 
	// if(UTILP2_isbufferused(globalparams.ACTSPARAMS_INSTID) == false){ return 0; }
	// #endif 	
...
void acts_all::MERGEP2_exchange(uint512_dt * vdramA, uint512_dt * vdramB, uint512_dt * vdramC){
	...
	EXCHANGEP2_LOOP1: for(unsigned int reduce_partition=0; reduce_partition<globalparamsvA.NUM_REDUCEPARTITIONS; reduce_partition++){
		...
		#ifdef _COLLECT_METADATA_FROM_KERNEL_
		EXCHANGEP2_LOOP1B: for(unsigned int v_p=0; v_p<globalparamsvA.NUM_PROCESSEDGESPARTITIONS; v_p++){
			if(vpartition_stats[v_p] < globalparamsvA.THRESHOLD_HYBRIDGPMODE_MAXLIMIT_ACTVEDGEBLOCKS_PER_VPARTITION){
				...
				for(unsigned int t=begin; t<begin + sz; t++){
					...
					if(upropblock_stats[t].data[0] > 0 || upropblock_stats[t].data[1] > 0 || upropblock_stats[t].data[2] > 0){
						UTILP2_WriteData(vdramA, globalparamsvA.BASEOFFSETKVS_ACTIVEEDGEBLOCKS + offset_vs, index, t-begin);
						UTILP2_WriteData(vdramB, globalparamsvB.BASEOFFSETKVS_ACTIVEEDGEBLOCKS + offset_vs, index, t-begin);
						UTILP2_WriteData(vdramC, globalparamsvC.BASEOFFSETKVS_ACTIVEEDGEBLOCKS + offset_vs, index, t-begin);
				...
				UTILP2_WriteData(vdramA, globalparamsvA.BASEOFFSETKVS_ACTIVEEDGEBLOCKS + offset_vs, 0, index-1);
				UTILP2_WriteData(vdramB, globalparamsvB.BASEOFFSETKVS_ACTIVEEDGEBLOCKS + offset_vs, 0, index-1);
				UTILP2_WriteData(vdramC, globalparamsvC.BASEOFFSETKVS_ACTIVEEDGEBLOCKS + offset_vs, 0, index-1);
			}
		}
		#endif 
	}
	...
	#ifdef _COLLECT_METADATA_FROM_KERNEL_
	MEMACCESSP2_savehelperstats(vdramA, vpartition_stats, globalparamsvA.BASEOFFSETKVS_VERTICESPARTITIONMASK, BLOCKRAM_CURRPMASK_SIZE, globalparamsvA.ALGORITHMINFO_GRAPHITERATIONID, globalparamsvA);
	MEMACCESSP2_savehelperstats(vdramB, vpartition_stats, globalparamsvA.BASEOFFSETKVS_VERTICESPARTITIONMASK, BLOCKRAM_CURRPMASK_SIZE, globalparamsvA.ALGORITHMINFO_GRAPHITERATIONID, globalparamsvA);
	MEMACCESSP2_savehelperstats(vdramC, vpartition_stats, globalparamsvA.BASEOFFSETKVS_VERTICESPARTITIONMASK, BLOCKRAM_CURRPMASK_SIZE, globalparamsvA.ALGORITHMINFO_GRAPHITERATIONID, globalparamsvA);
	#endif 
	...
	if(globalparamsK.ENABLE_RUNKERNELCOMMAND == ON && en == true){
		...
		if(mode == ACTSREDUCEMODE){		
			...
			for(unsigned int n=0; n<num_its; n++){
				...
				#else 
				MEMACCESSP2_read__reduce(READVPROPERTYMODE, kvdram, vbuffer, stats, xload_kvs[n], collections_tmp, globalparamsK);
				#endif 
				#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
			...
			for(unsigned int n=0; n<num_its; n++){
				...
				#else 
				unsigned int transfsz_kvs = MEMACCESSP2_write__process_base(SAVEVPROPERTYMODE, NAp, kvdram, vbuffer, stats, xload_kvs[n], collections_tmp, globalparamsK);
				#endif 
				#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
				collections_tmp[REDUCEPHASE_TRANSFSZ_COLLECTIONID].data1 += (xload_kvs[n].size + REPORT__DRAM_ACCESS_LATENCY) * VECTOR2_SIZE; 
				collections_tmp[NUMVERTICESPROCESSED_COLLECTIONID].data1 += (xload_kvs[n].size + REPORT__DRAM_ACCESS_LATENCY) * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeVs(kvdram0, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
...
#pragma HLS DATA_PACK variable = vdram
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeVs(kvdram1, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeVs(kvdram2, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeVs(kvdram3, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeVs(kvdram4, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeVs(kvdram5, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeVs(kvdram6, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeVs(kvdram7, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeVs(kvdram8, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeVs(kvdram9, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeVs(kvdram10, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP2_mergeVs(kvdram11, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
