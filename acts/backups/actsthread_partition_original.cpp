...
void 
	...
	actsthread_partition::
	#endif 
readKVS(uint512_dt * kvdram, uint512_dt * buffer, batch_type offset_kvs, atp_travstate_t travstate){
...
void 
	...
	actsthread_partition::
	#endif 
partition(uint512_dt * sourcebuffer, uint512_dt * destbuffer, keyvalue_t * localcapsule, sweepparams_t sweepparams, atp_travstate_t travstate){
	unsigned int analysis_srcbuffersz = ATP_SRCBUFFER_SIZE;
	buffer_type chunk_size = getchunksize(ATP_SRCBUFFER_SIZE, travstate, 0);
...
void 
	...
	actsthread_partition::
	#endif 
saveKVS(uint512_dt * kvdram, uint512_dt * buffer, keyvalue_t * globalcapsule, keyvalue_t * localcapsule, batch_type globalbaseaddress_kvs){				
...
#pragma HLS DATA_PACK variable = statsC
	...
	for(unsigned int currentreadindex=1; currentreadindex<2; currentreadindex+=1){
		...
		#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_mainloop avg=analysis_mainloop	
			...
			readKVS(sourceAvolume, sourceAbuffer, (sourcebaseaddress_kvs + offset_kvs), travstate);
			readKVS(sourceBvolume, sourceBbuffer, (sourcebaseaddress_kvs + offset_kvs), travstate);
			readKVS(sourceCvolume, sourceCbuffer, (sourcebaseaddress_kvs + offset_kvs), travstate);
			
			partition(sourceAbuffer, destAbuffer, localcapsuleA, sweepparams, travstate);
			partition(sourceBbuffer, destBbuffer, localcapsuleB, sweepparams, travstate);
			partition(sourceCbuffer, destCbuffer, localcapsuleC, sweepparams, travstate);
			
			saveKVS(sourceAvolume, destAbuffer, globalcapsuleA, localcapsuleA, destbaseaddress_kvs);
			saveKVS(sourceBvolume, destBbuffer, globalcapsuleB, localcapsuleB, destbaseaddress_kvs);
			saveKVS(sourceCvolume, destCbuffer, globalcapsuleC, localcapsuleC, destbaseaddress_kvs);
		}
