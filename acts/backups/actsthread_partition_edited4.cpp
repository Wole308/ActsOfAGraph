...
getpartition(keyvalue_t keyvalue, unsigned int currentLOP, vertex_t upperlimit){
	...
	#endif
	// cout<<"getpartition:: partition: "<<partition<<endl;
...
void
	...
	actsthread_partition::
	#endif 
partitionmany(uint512_dt * source1buffer, uint512_dt * source2buffer, uint512_dt * source3buffer, uint512_dt * dest1buffer, uint512_dt * dest2buffer, uint512_dt * dest3buffer, keyvalue_t * localcapsule1, keyvalue_t * localcapsule2, keyvalue_t * localcapsule3, sweepparams_t sweepparams, atp_travstate_t travstate){
	unsigned int analysis_srcbuffersz = ATP_SRCBUFFER_SIZE;
	buffer_type chunk_size = getchunksize(ATP_SRCBUFFER_SIZE, travstate, 0);
	...
	PARTITION_LOOP1: for(unsigned int si=0; si<2; si++){
		...
		for(partition_type p=0; p<NUM_PARTITIONS; p++){ localcapsule1[p].key = 0; localcapsule1[p].value = 0; localcapsule2[p].key = 0; localcapsule2[p].value = 0; localcapsule3[p].key = 0; localcapsule3[p].value = 0; } // REMOVEME.
		...
		#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_srcbuffersz avg=analysis_srcbuffersz	
			...
			#pragma HLS PIPELINE II=1
				...
				buffer_type index1 = localcapsule1[p1].key + localcapsule1[p1].value;
				...
				buffer_type idy1 = index1 / VECTOR_SIZE; vector_type idx1 = index1 % VECTOR_SIZE; 
	...
	APPENDINVALIDS_LOOP1: for(partition_type p=0; p<NUM_PARTITIONS; p++){
		buffer_type ovsize1 = allignhigher_KV(localcapsule1[p].value) - localcapsule1[p].value;
		...
		buffer_type index1 = localcapsule1[p].key + localcapsule1[p].value;
		buffer_type index2 = localcapsule2[p].key + localcapsule2[p].value;
		buffer_type index3 = localcapsule3[p].key + localcapsule3[p].value;
		batch_type idy1 = index1 / VECTOR_SIZE; vector_type idx1 = index1 % VECTOR_SIZE;
		batch_type idy2 = index2 / VECTOR_SIZE; vector_type idx2 = index2 % VECTOR_SIZE;
		batch_type idy3 = index3 / VECTOR_SIZE; vector_type idx3 = index3 % VECTOR_SIZE;
...
partitionmany(uint512_dt * source1buffer, uint512_dt * source2buffer, uint512_dt * source3buffer, uint512_dt * dest1buffer, uint512_dt * dest2buffer, uint512_dt * dest3buffer, keyvalue_t * localcapsule1, keyvalue_t * localcapsule2, keyvalue_t * localcapsule3, sweepparams_t sweepparams, atp_travstate_t travstate){
	unsigned int analysis_srcbuffersz = ATP_SRCBUFFER_SIZE;
	buffer_type chunk_size = getchunksize(ATP_SRCBUFFER_SIZE, travstate, 0);
	// buffer_type chunk_size = ATP_SRCBUFFER_SIZE;
	COLLECTSTATS_LOOP1: for(partition_type p=0; p<NUM_PARTITIONS; p++){ localcapsule1[p].key = 0; localcapsule1[p].value = 0; localcapsule2[p].key = 0; localcapsule2[p].value = 0; localcapsule3[p].key = 0; localcapsule3[p].value = 0; }
	...
	PARTITION_LOOP3: for(partition_type p=1; p<NUM_PARTITIONS; p++){ localcapsule1[p].key = allignhigher_KV(localcapsule1[p-1].key + localcapsule1[p-1].value); localcapsule2[p].key = allignhigher_KV(localcapsule2[p-1].key + localcapsule2[p-1].value); localcapsule3[p].key = allignhigher_KV(localcapsule3[p-1].key + localcapsule3[p-1].value); }
	PARTITION_LOOP4: for(partition_type p=0; p<NUM_PARTITIONS; p++){ localcapsule1[p].key = 0; localcapsule1[p].value = 0; localcapsule2[p].key = 0; localcapsule2[p].value = 0; localcapsule3[p].key = 0; localcapsule3[p].value = 0; }
...
void 
	...
	actsthread_partition:: 
	#endif
topkernel(uint512_dt * sourceAvolume, uint512_dt * sourceBvolume, uint512_dt * sourceCvolume){
...
#pragma HLS DATA_PACK variable = sourceCvolume
	...
	keyvalue_t statsA[NUM_PARTITIONS];
	...
	keyvalue_t statsB[NUM_PARTITIONS];
	...
	keyvalue_t statsC[NUM_PARTITIONS];
