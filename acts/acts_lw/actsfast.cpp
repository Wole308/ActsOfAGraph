...
void 
	...
	actslw::
	#endif
collectglobalstats(bool_type enable, keyvalue_t globalstatsbuffer[GLOBALSTATSBUFFERSZ], step_type currentLOP, batch_type source_partition){					
	if(enable == OFF){ return; }
	
	if(currentLOP == 1){ // FIXME. make simpler
		...
		for(partition_type p=0; p<NUM_PARTITIONS; p++){
			#ifdef _DEBUGMODE_CHECKS2
			actsutilityobj->checkoutofbounds("collectglobalstats 2", NUM_PARTITIONS + (source_partition*NUM_PARTITIONS) + (p*per) + (per-1), NUM_PARTITIONS + (1 << (NUM_PARTITIONS_POW * TREE_DEPTH)), p, per, NUM_PARTITIONS);
			#endif
	...
	#ifdef _DEBUGMODE_KERNELPRINTS2
	actsutilityobj->printkeyvalues(")))))))))))))))))))))))))))))))) collectglobalstats", globalstatsbuffer, NUM_PARTITIONS); 
	#endif
	return;
}
...
readkeyvalues(bool_type enable, uint512_dt * kvdram, keyvalue_t buffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], batch_type offset_kvs, travstate_t travstate){
	...
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE;
	buffer_type chunk_size = getchunksize_kvs(SRCBUFFER_SIZE, travstate, 0);
	// if(chunk_size < SRCBUFFER_SIZE){ chunk_size += 1; } // for optimal process edges
...
readkeyvalues(bool_type enable, uint512_dt * kvdram, keyvalue_t buffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], batch_type offset_kvs, batch_type size_kvs, travstate_t travstate){
	...
	cout<<"readkeyvalues:: keyvalues read: offset_kvs: "<<offset_kvs<<", size_kvs: "<<chunk_size<<endl;
	// cout<<"readkeyvalues:: keyvalues read: offset_kvs: "<<offset_kvs-BASEOFFSET_KVDRAM_KVS<<", size_kvs: "<<chunk_size<<endl; // REMOVEME.
	#endif
	return;
}
...
partitionkeyvalues2(bool_type enable, keyvalue_t sourcebuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], keyvalue_t destbuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], skeyvalue_t localcapsule[VECTOR_SIZE][NUM_PARTITIONS], step_type currentLOP, vertex_t upperlimit, travstate_t travstate, globalparams_t globalparams){
	...
	PARTITIONKEYVALUES_LOOP1: for(step_type c=0; c<2; c++){
		...
		#pragma HLS PIPELINE II=1
			...
			#ifdef _DEBUGMODE_CHECKS2
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", localcapsule[0][p0].key + localcapsule[0][p0].value, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", localcapsule[1][p1].key + localcapsule[1][p1].value, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", localcapsule[2][p2].key + localcapsule[2][p2].value, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", localcapsule[3][p3].key + localcapsule[3][p3].value, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", localcapsule[4][p4].key + localcapsule[4][p4].value, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", localcapsule[5][p5].key + localcapsule[5][p5].value, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", localcapsule[6][p6].key + localcapsule[6][p6].value, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", localcapsule[7][p7].key + localcapsule[7][p7].value, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			#endif
			...
			#ifdef _DEBUGMODE_CHECKS2
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", p0, NUM_PARTITIONS, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", p1, NUM_PARTITIONS, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", p2, NUM_PARTITIONS, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", p3, NUM_PARTITIONS, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", p4, NUM_PARTITIONS, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", p5, NUM_PARTITIONS, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", p6, NUM_PARTITIONS, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", p7, NUM_PARTITIONS, NAp, NAp, NAp);
			#endif
		}
	...
	#ifdef XXXX // DELETEME.
	for(partition_type i=0; i<SRCBUFFER_SIZE - cutoff; i++){
	#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_edgecond1 avg=analysis_edgecond1
		destbuffer[0][cutoff + i] = dummykv;
		localcapsule[0][NUM_PARTITIONS-1].value += 1;
		destbuffer[1][cutoff + i] = dummykv;
		localcapsule[1][NUM_PARTITIONS-1].value += 1;
		destbuffer[2][cutoff + i] = dummykv;
		localcapsule[2][NUM_PARTITIONS-1].value += 1;
		destbuffer[3][cutoff + i] = dummykv;
		localcapsule[3][NUM_PARTITIONS-1].value += 1;
		destbuffer[4][cutoff + i] = dummykv;
		localcapsule[4][NUM_PARTITIONS-1].value += 1;
		destbuffer[5][cutoff + i] = dummykv;
		localcapsule[5][NUM_PARTITIONS-1].value += 1;
		destbuffer[6][cutoff + i] = dummykv;
		localcapsule[6][NUM_PARTITIONS-1].value += 1;
		destbuffer[7][cutoff + i] = dummykv;
	...
	#endif 
	#ifdef _DEBUGMODE_KERNELPRINTS
	cout<<"--------------- cutoff: "<<cutoff<<", ovsz[0]: "<<ovsz[0]<<endl;
	#endif 
	#ifdef _DEBUGMODE_KERNELPRINTS
	if(false){ actsutilityobj->printprofile("partitionkeyvalues2.destbuffer", (keyvalue_t *)&destbuffer[0], PADDEDDESTBUFFER_SIZE * VECTOR_SIZE, currentLOP, upperlimit, globalparams.batch_range_pow); }
	if(false){ actsutilityobj->printprofileso1("partitionkeyvalues2.destbuffer", destbuffer, localcapsule, currentLOP, upperlimit, globalparams.batch_range_pow); }
	for(unsigned int v=0; v<8; v++){ actsutilityobj->printkeyvalues("partitionkeyvalues2.localcapsule[v]", (keyvalue_t *)localcapsule[v], NUM_PARTITIONS); }
	for(unsigned int v=0; v<8; v++){ actsutilityobj->printvalues("partitionkeyvalues2.limitsz[v]", (value_t *)limitsz[v], NUM_PARTITIONS); }
	#endif
	// exit(EXIT_SUCCESS);
	return cutoff;
}
...
partitionkeyvalues3(bool_type enable, keyvalue_t sourcebuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], keyvalue_t destbuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], skeyvalue_t localcapsule[VECTOR_SIZE][NUM_PARTITIONS], step_type currentLOP, vertex_t upperlimit, travstate_t travstate, globalparams_t globalparams){
	...
	PARTITIONKEYVALUES_LOOP1: for(step_type c=0; c<2; c++){
		...
		#pragma HLS PIPELINE II=1
			...
			#ifdef _DEBUGMODE_CHECKS2
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", localcapsule[0][p0].key + templocalcapsule0[p0], PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", localcapsule[1][p1].key + templocalcapsule1[p1], PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", localcapsule[2][p2].key + templocalcapsule2[p2], PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", localcapsule[3][p3].key + templocalcapsule3[p3], PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", localcapsule[4][p4].key + templocalcapsule4[p4], PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", localcapsule[5][p5].key + templocalcapsule5[p5], PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", localcapsule[6][p6].key + templocalcapsule6[p6], PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("partitionkeyvalues2", localcapsule[7][p7].key + templocalcapsule7[p7], PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			#endif
	...
	#ifdef _DEBUGMODE_CHECKS2
	actsutilityobj->checkoutofbounds("cutoff", cutoff, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
	for(unsigned int v=0; v<VECTOR_SIZE; v++){ actsutilityobj->checkforequal("partitionkeyvalues3.cutoff", ovsz[v], ovsz[0], v, ovsz[0], NAp); }
	actsutilityobj->checkforequal("partitionkeyvalues3.cutoff+ovsz[0],chunk_size", cutoff+ovsz[0], chunk_size, cutoff, ovsz[0], NAp); // REMOVEME.
	#endif
	#ifdef _DEBUGMODE_KERNELPRINTS
	if(false){ actsutilityobj->printprofile("partitionkeyvalues2.destbuffer", (keyvalue_t *)&destbuffer[0], PADDEDDESTBUFFER_SIZE * VECTOR_SIZE, currentLOP, upperlimit, globalparams.batch_range_pow); }
	if(false){ actsutilityobj->printprofileso1("partitionkeyvalues2.destbuffer", destbuffer, localcapsule, currentLOP, upperlimit, globalparams.batch_range_pow); }
...
savekeyvalues(bool_type enable, uint512_dt * kvdram, keyvalue_t buffer[8][PADDEDDESTBUFFER_SIZE], keyvalue_t * globalcapsule, skeyvalue_t localcapsule[NUM_PARTITIONS], batch_type globalbaseaddress_kvs, globalparams_t globalparams){				
	...
	#ifdef _DEBUGMODE_KERNELPRINTS3
	cout<<"&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&& savekeyvalues:: savekeyvalues started. "<<endl;
	#endif
...
process_edges(bool_type enable, keyvalue_t sourcebuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], keyvalue_t destbuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], keyvalue_t statsbuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], vertex_t upperlimit, unsigned int GraphIter, unsigned int GraphAlgo, travstate_t travstate, globalparams_t globalparams){
	...
	#pragma HLS PIPELINE II=2
		...
		partition_type p0 = 0;
		if(localsourceid0 < PADDEDDESTBUFFER_SIZE){ p0 = getglobalpartition(vertexupdate0, 0, 0, globalparams.batch_range_pow); }
		partition_type p1 = 0;
		if(localsourceid1 < PADDEDDESTBUFFER_SIZE){ p1 = getglobalpartition(vertexupdate1, 0, 0, globalparams.batch_range_pow); }
		partition_type p2 = 0;
		if(localsourceid2 < PADDEDDESTBUFFER_SIZE){ p2 = getglobalpartition(vertexupdate2, 0, 0, globalparams.batch_range_pow); }
		partition_type p3 = 0;
		if(localsourceid3 < PADDEDDESTBUFFER_SIZE){ p3 = getglobalpartition(vertexupdate3, 0, 0, globalparams.batch_range_pow); }
		partition_type p4 = 0;
		if(localsourceid4 < PADDEDDESTBUFFER_SIZE){ p4 = getglobalpartition(vertexupdate4, 0, 0, globalparams.batch_range_pow); }
		partition_type p5 = 0;
		if(localsourceid5 < PADDEDDESTBUFFER_SIZE){ p5 = getglobalpartition(vertexupdate5, 0, 0, globalparams.batch_range_pow); }
		partition_type p6 = 0;
		if(localsourceid6 < PADDEDDESTBUFFER_SIZE){ p6 = getglobalpartition(vertexupdate6, 0, 0, globalparams.batch_range_pow); }
		partition_type p7 = 0;
		if(localsourceid7 < PADDEDDESTBUFFER_SIZE){ p7 = getglobalpartition(vertexupdate7, 0, 0, globalparams.batch_range_pow); }
...
runpipeline(bool_type enable, keyvalue_t bufferA[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], skeyvalue_t buffer1capsule[VECTOR_SIZE][NUM_PARTITIONS], 
	...
	for(unsigned int v=0; v<0; v++){ actsutilityobj->printkeyvalues("--- runpipeline.bufferA[0]", bufferA[v], 16); }
	for(unsigned int v=0; v<0; v++){ actsutilityobj->printkeyvalues("--- runpipeline.buffer1capsule[0]", (keyvalue_t *)buffer1capsule[v], NUM_PARTITIONS); }
	#endif
	// exit(EXIT_SUCCESS);
	...
	for(partition_type p=0; p<NUM_PARTITIONS; p++){
		// bufferDcapsule[p].key = ((bufferCcapsule[0][p].key + bufferCcapsule[1][p].key) / 2) * 8; // ping-pong takes place at D
		// bufferDcapsule[p].key = (((bufferCcapsule[0][p].key + bufferCcapsule[1][p].key) + (2-1)) / 2) * 8; // ping-pong takes place at D // NEWCHANGE. FIXME. OPTIMIZEME.
		
		if(p<NUM_PARTITIONS-1){ bufferDcapsule[p+1].key = bufferDcapsule[p].key + allignhigher_KV(((bufferCcapsule[0][p].value + bufferCcapsule[1][p].value) * 4)); }
	...
	#ifdef _DEBUGMODE_CHECKS2
	for(partition_type p=0; p<NUM_PARTITIONS; p++){
		actsutilityobj->checkfordivisibleby("bufferDcapsule[p].key", bufferDcapsule[p].key, 1);
		
		actsutilityobj->checkfordivisibleby("bufferCcapsule[0][p].key", bufferCcapsule[0][p].key, 1);
		actsutilityobj->checkfordivisibleby("bufferCcapsule[1][p].key", bufferCcapsule[1][p].key, 1);
		
		actsutilityobj->checkfordivisibleby("bufferBcapsule[0][p].key", bufferBcapsule[0][p].key, 2);
		actsutilityobj->checkfordivisibleby("bufferBcapsule[1][p].key", bufferBcapsule[1][p].key, 2);
		actsutilityobj->checkfordivisibleby("bufferBcapsule[2][p].key", bufferBcapsule[2][p].key, 2);
		actsutilityobj->checkfordivisibleby("bufferBcapsule[3][p].key", bufferBcapsule[3][p].key, 2);
		
		actsutilityobj->checkfordivisibleby("buffer1capsule[0][p].key", buffer1capsule[0][p].key, 4);
		actsutilityobj->checkfordivisibleby("buffer1capsule[1][p].key", buffer1capsule[1][p].key, 4);
		actsutilityobj->checkfordivisibleby("buffer1capsule[2][p].key", buffer1capsule[2][p].key, 4);
		actsutilityobj->checkfordivisibleby("buffer1capsule[3][p].key", buffer1capsule[3][p].key, 4);
		actsutilityobj->checkfordivisibleby("buffer1capsule[4][p].key", buffer1capsule[4][p].key, 4);
		actsutilityobj->checkfordivisibleby("buffer1capsule[5][p].key", buffer1capsule[5][p].key, 4);
		actsutilityobj->checkfordivisibleby("buffer1capsule[6][p].key", buffer1capsule[6][p].key, 4);
		actsutilityobj->checkfordivisibleby("buffer1capsule[7][p].key", buffer1capsule[7][p].key, 4);
	}
	...
	RUNPIPELINE_LOOP1: for(n=0; n<2; n++){
		...
		#pragma HLS PIPELINE II=4
			...
			#ifdef _DEBUGMODE_CHECKS2
			actsutilityobj->checkn("kvA0", kvA0, currentLOP, upperlimit, globalparams.batch_range_pow, 4);
			actsutilityobj->checkn("kvA2", kvA2, currentLOP, upperlimit, globalparams.batch_range_pow, 4);
			actsutilityobj->checkn("kvA4", kvA4, currentLOP, upperlimit, globalparams.batch_range_pow, 4);
			actsutilityobj->checkn("kvA6", kvA6, currentLOP, upperlimit, globalparams.batch_range_pow, 4);
			#endif 
			// continue;
			// exit(EXIT_SUCCESS);
			
			// cout<<"&&&&&****(((((----- runpipeline:: before 1st partition stage."<<endl;
			partition_type pA0 = getpartition(kvA0[0], currentLOP, upperlimit, globalparams.batch_range_pow);
			partition_type pA2 = getpartition(kvA2[0], currentLOP, upperlimit, globalparams.batch_range_pow);
			partition_type pA4 = getpartition(kvA4[0], currentLOP, upperlimit, globalparams.batch_range_pow);
			partition_type pA6 = getpartition(kvA6[0], currentLOP, upperlimit, globalparams.batch_range_pow);
			// cout<<"&&&&&****(((((----- runpipeline:: 1st partition stage passed."<<endl;
			// continue;
			// exit(EXIT_SUCCESS);
			...
			#ifdef _DEBUGMODE_CHECKS2
			// actsutilityobj->checkfordivisibleby("posB0", posB0, 2);
			actsutilityobj->checkoutofbounds("posB0", posB0, SRCBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("posB0", posB0, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			#endif 
			...
			#ifdef _DEBUGMODE_CHECKS2
			// actsutilityobj->checkfordivisibleby("posB2", posB2, 2);
			actsutilityobj->checkoutofbounds("posB2", posB2, SRCBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("posB2", posB2, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			#endif 
			...
			#ifdef _DEBUGMODE_CHECKS2
			// actsutilityobj->checkfordivisibleby("posB4", posB4, 2);
			actsutilityobj->checkoutofbounds("posB4", posB4, SRCBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("posB4", posB4, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			#endif 
			...
			#ifdef _DEBUGMODE_CHECKS2
			// actsutilityobj->checkfordivisibleby("posB6", posB6, 2);
			actsutilityobj->checkoutofbounds("posB6", posB6, SRCBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("posB6", posB6, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			#endif 
			...
			partition_type pB0 = getpartition(kvB0[0], currentLOP, upperlimit, globalparams.batch_range_pow);
			partition_type pB2 = getpartition(kvB2[0], currentLOP, upperlimit, globalparams.batch_range_pow);
			partition_type pB4 = getpartition(kvB4[0], currentLOP, upperlimit, globalparams.batch_range_pow);
			partition_type pB6 = getpartition(kvB6[0], currentLOP, upperlimit, globalparams.batch_range_pow);
			// cout<<"&&&&&****(((((----- runpipeline:: 2nd partition stage passed."<<endl;
			// exit(EXIT_SUCCESS);
			
			#ifdef _DEBUGMODE_CHECKS2
			CHK[0] = kvB0[0]; CHK[1] = kvB1[0]; CHK[2] = kvB0[1]; CHK[3] = kvB1[1];
			actsutilityobj->checkn("kvB0,kvB1", CHK, currentLOP, upperlimit, globalparams.batch_range_pow, 4);
			CHK[0] = kvB2[0]; CHK[1] = kvB3[0]; CHK[2] = kvB2[1]; CHK[3] = kvB3[1];
			actsutilityobj->checkn("kvB2,kvB3", CHK, currentLOP, upperlimit, globalparams.batch_range_pow, 4);
			CHK[0] = kvB4[0]; CHK[1] = kvB5[0]; CHK[2] = kvB4[1]; CHK[3] = kvB5[1];
			actsutilityobj->checkn("kvB4,kvB5", CHK, currentLOP, upperlimit, globalparams.batch_range_pow, 4);
			CHK[0] = kvB6[0]; CHK[1] = kvB7[0]; CHK[2] = kvB6[1]; CHK[3] = kvB7[1];
			actsutilityobj->checkn("kvB6,kvB7", CHK, currentLOP, upperlimit, globalparams.batch_range_pow, 4);
			#endif 
			// continue;
			// exit(EXIT_SUCCESS);
			...
			#ifdef _DEBUGMODE_CHECKS2
			actsutilityobj->checkoutofbounds("posC0", posC0, SRCBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("posC0", posC0, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			#endif 
			...
			#ifdef _DEBUGMODE_CHECKS2
			actsutilityobj->checkoutofbounds("_posC0", _posC0, SRCBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("_posC0", _posC0, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			#endif 
			...
			#ifdef _DEBUGMODE_CHECKS2
			actsutilityobj->checkoutofbounds("posC4", posC4, SRCBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("posC4", posC4, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			#endif 
			...
			#ifdef _DEBUGMODE_CHECKS2
			actsutilityobj->checkoutofbounds("_posC4", _posC4, SRCBUFFER_SIZE, NAp, NAp, NAp);
			actsutilityobj->checkoutofbounds("_posC4", _posC4, PADDEDDESTBUFFER_SIZE, NAp, NAp, NAp);
			#endif 
			...
			partition_type pC0 = getpartition(kvC0[0], currentLOP, upperlimit, globalparams.batch_range_pow);
			partition_type _pC0 = getpartition(_kvC0[0], currentLOP, upperlimit, globalparams.batch_range_pow);
			partition_type pC4 = getpartition(kvC4[0], currentLOP, upperlimit, globalparams.batch_range_pow);
			partition_type _pC4 = getpartition(_kvC4[0], currentLOP, upperlimit, globalparams.batch_range_pow);
			// cout<<"&&&&&****(((((----- runpipeline:: 3rd partition stage passed."<<endl;
			
			#ifdef _DEBUGMODE_CHECKS2
			CHK[0] = kvC0[0]; CHK[1] = kvC1[0]; CHK[2] = kvC2[0]; CHK[3] = kvC3[0];
			actsutilityobj->checkn("kvC0[0],kvC1[0],kvC2[0],kvC3[0]", CHK, currentLOP, upperlimit, globalparams.batch_range_pow, 4);
			
			CHK[0] = _kvC0[0]; CHK[1] = _kvC1[0]; CHK[2] = _kvC2[0]; CHK[3] = _kvC3[0];
			actsutilityobj->checkn("_kvC0[0],_kvC1[0],_kvC2[0],_kvC3[0]", CHK, currentLOP, upperlimit, globalparams.batch_range_pow, 4);
			
			CHK[0] = kvC4[0]; CHK[1] = kvC5[0]; CHK[2] = kvC6[0]; CHK[3] = kvC7[0];
			actsutilityobj->checkn("kvC4[0],kvC5[0],kvC6[0],kvC7[0]", CHK, currentLOP, upperlimit, globalparams.batch_range_pow, 4);
			
			CHK[0] = _kvC4[0]; CHK[1] = _kvC5[0]; CHK[2] = _kvC6[0]; CHK[3] = _kvC7[0];
			actsutilityobj->checkn("_kvC4[0],_kvC5[0],_kvC6[0],_kvC7[0]", CHK, currentLOP, upperlimit, globalparams.batch_range_pow, 4);
			#endif 
			// continue;
			// exit(EXIT_SUCCESS);
			...
			#ifdef _DEBUGMODE_CHECKS2
			actsutilityobj->checkfordivisibleby("posD0", posD0, 4);
			actsutilityobj->checkoutofbounds("posD0", posD0, PADDEDDESTBUFFER_SIZE * VECTOR_SIZE, NAp, NAp, NAp);
			...
			#ifdef _DEBUGMODE_CHECKS2
			actsutilityobj->checkfordivisibleby("_posD0", _posD0, 4);
			actsutilityobj->checkoutofbounds("_posD0", _posD0, PADDEDDESTBUFFER_SIZE * VECTOR_SIZE, NAp, NAp, NAp);
			...
			#ifdef _DEBUGMODE_CHECKS2
			actsutilityobj->checkfordivisibleby("__posD0", __posD0, 4);
			actsutilityobj->checkoutofbounds("__posD0", __posD0, PADDEDDESTBUFFER_SIZE * VECTOR_SIZE, NAp, NAp, NAp);
			...
			#ifdef _DEBUGMODE_CHECKS2
			actsutilityobj->checkfordivisibleby("___posD0", ___posD0, 4);
			actsutilityobj->checkoutofbounds("___posD0", ___posD0, PADDEDDESTBUFFER_SIZE * VECTOR_SIZE, NAp, NAp, NAp);
	...
	for(partition_type p=0; p<NUM_PARTITIONS; p++){
		...
		#ifdef _DEBUGMODE_CHECKS2
		actsutilityobj->checkfordivisibleby("bufferDcapsule[p].value", bufferDcapsule[p].value, 8);
		#endif
	}
	
	#ifdef _DEBUGMODE_CHECKS2
	actsutilityobj->checkprofile("bufferB", bufferB, SRCBUFFER_SIZE, currentLOP, upperlimit, globalparams.batch_range_pow, 4, SRCBUFFER_SIZE*VECTOR_SIZE); 
	actsutilityobj->checkprofile("bufferC", bufferC, SRCBUFFER_SIZE, currentLOP, upperlimit, globalparams.batch_range_pow, 4, SRCBUFFER_SIZE*VECTOR_SIZE); 
	actsutilityobj->checkprofile("bufferD", bufferD, SRCBUFFER_SIZE, currentLOP, upperlimit, globalparams.batch_range_pow, 4, SRCBUFFER_SIZE*VECTOR_SIZE); 
	actsutilityobj->checkbufferprofile("runpipeline::bufferD", bufferD, (keyvalue_t *)bufferDcapsule, currentLOP, upperlimit, globalparams.batch_range_pow); 
	#endif
	#ifdef _DEBUGMODE_KERNELPRINTS2
	...
	actsutilityobj->printprofile("bufferD", (keyvalue_t *)&bufferD[0], PADDEDDESTBUFFER_SIZE * VECTOR_SIZE, currentLOP, upperlimit, globalparams.batch_range_pow);
	
	actsutilityobj->printprofileso1("bufferA", bufferA, buffer1capsule, currentLOP, upperlimit, globalparams.batch_range_pow);
	actsutilityobj->printprofileso2("bufferB", bufferB, bufferBcapsule, currentLOP, upperlimit, globalparams.batch_range_pow);
	actsutilityobj->printprofileso4("bufferC", bufferC, bufferCcapsule, currentLOP, upperlimit, globalparams.batch_range_pow);
	actsutilityobj->printkeyvalues("+++[after] runpipeline.bufferDcapsule", (keyvalue_t *)bufferDcapsule, NUM_PARTITIONS);
	#endif 
	#ifdef _DEBUGMODE_KERNELPRINTS
	for(buffer_type i=0; i<PADDEDDESTBUFFER_SIZE; i++){
		for(vector_type v=0; v<VECTOR_SIZE; v++){ CHKK[v] = bufferD[v][i]; }
		actsutilityobj->checkn("bufferD[v][i]", CHKK, currentLOP, upperlimit, globalparams.batch_range_pow, 8);
	}
...
partitionupdates(
	...
	#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_partitionloop avg=analysis_partitionloop
		...
		partitionkeyvalues(ON, sourcebuffer, buffer_setof1, templocalcapsule_so1, sweepparams.currentLOP, sweepparams.upperlimit, ptravstatepp0, globalparams);
		#ifdef PP1
		...
		#ifdef PP1
		partitionkeyvalues(ON, sourcebuffer, buffer_setof1, templocalcapsule_so1, sweepparams.currentLOP, sweepparams.upperlimit, ptravstatepp1, globalparams);
		#endif
		#ifdef PP2
		...
		#endif
		#ifdef PP2
		partitionkeyvalues(ON, sourcebuffer, buffer_setof1, templocalcapsule_so1, sweepparams.currentLOP, sweepparams.upperlimit, ptravstatepp2, globalparams);
		#endif
...
fastpartitionupdates(
		...
		travstate_t ptravstate){
	#pragma HLS INLINE
	analysis_type analysis_partitionloop = KVDATA_BATCHSIZE_KVS / (2 * SRCBUFFER_SIZE); // 2
	...
	////////////////////////////////////////////////////////////////////////
	// keyvalue_t dummykv;
	// dummykv.key = ((NUM_PARTITIONS-1) << (globalparams.batch_range_pow - (NUM_PARTITIONS_POW * sweepparams.currentLOP))) + sweepparams.upperlimit;
	// dummykv.value = INVALIDDATA;
	...
	// buffer_type pp0readoffset_kvs = 0;
	buffer_type pp0readoffset_kvs = ptravstate.begin_kvs; // NEWCHANGE.
	buffer_type pp1readoffset_kvs = SRCBUFFER_SIZE;
	...
	MAINLOOP_PARTITION: for(batch_type offset_kvs=ptravstate.begin_kvs; offset_kvs<ptravstate.end_kvs + (2*ptravstate.skip_kvs); offset_kvs+=ptravstate.skip_kvs * 1){ // NEWCHANGE.
	// MAINLOOP_PARTITION: for(batch_type offset_kvs=ptravstate.begin_kvs; offset_kvs<3*ptravstate.skip_kvs; offset_kvs+=ptravstate.skip_kvs){
	// MAINLOOP_PARTITION: for(batch_type offset_kvs=ptravstate.begin_kvs; offset_kvs<30*ptravstate.skip_kvs; offset_kvs+=ptravstate.skip_kvs){
	#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_partitionloop avg=analysis_partitionloop
		#ifdef _DEBUGMODE_KERNELPRINTS3
		actsutilityobj->print5("############################################### dispatch::partition:: itercount", "offset1_kvs", "begin_kvs", "end_kvs", "skip", itercount, pp0readoffset_kvs, ptravstate.begin_kvs, ptravstate.end_kvs, SRCBUFFER_SIZE);
		#endif
		ptravstate.i_kvs = NAp;
		ptravstatepp0.i_kvs = pp0readoffset_kvs;
		if(itercount >= 2){ pp0writeen = ON; } else { pp0writeen = OFF; } // NEWCHANGE.
		// if(offset_kvs >= ptravstate.end_kvs){ pp0readen = OFF; } else { pp0readen = ON; } // NEWCHANGE.
	
		// readkeyvalues(pp0readen, kvdram, sourcebuffer, (sweepparams.worksourcebaseaddress_kvs + ptravstate.begin_kvs + pp0readoffset_kvs), pp0readsize_kvs, ptravstatepp0); // NEWCHANGE.
		readkeyvalues(pp0readen, kvdram, sourcebuffer, (sweepparams.worksourcebaseaddress_kvs + pp0readoffset_kvs), pp0readsize_kvs, ptravstatepp0); // NEWCHANGE.
		/* if(sweepparams.currentLOP == 2 && sweepparams.source_partition==1){ 
			for(unsigned int v=0; v<VECTOR_SIZE; v++){
				actsutilityobj->printkeyvalues("-------+++++++ sourcebuffer[v]", sourcebuffer[v], SRCBUFFER_SIZE);
			}
			// actsutilityobj->printkeyvalues("-------+++++++ sourcebuffer", (keyvalue_t *)&sourcebuffer[0], SRCBUFFER_SIZE*VECTOR_SIZE); 
		} */
		
		cout<<"&&&&&****(((((----- dispatch:: before partitionkeyvalues3."<<endl;
		pp0cutoff = partitionkeyvalues3(ON, sourcebuffer, buffer_setof1, templocalcapsule_so1, sweepparams.currentLOP, sweepparams.upperlimit, ptravstatepp0, globalparams);
		if(sweepparams.currentLOP == 2 && sweepparams.source_partition==1){ cout<<"&&&&&****(((((----- partitionkeyvalues3 passed"<<endl; }
		...
		if(sweepparams.currentLOP == 2 && sweepparams.source_partition==1){ cout<<"&&&&&****(((((----- runpipeline passed"<<endl; }
		
		savekeyvalues(pp0writeen, kvdram, buffer_setof8, globalstatsbuffer, templocalcapsule_so8, sweepparams.workdestbaseaddress_kvs, globalparams);
		
		pp0readoffset_kvs = pp0readoffset_kvs + pp0readsize_kvs;
		pp0readsize_kvs = pp0cutoff;
		
		itercount += 1;
		if(pp0readoffset_kvs >= ptravstate.end_kvs + (2*ptravstate.skip_kvs)){ // NEWCHANGE.
			#ifdef _DEBUGMODE_KERNELPRINTS3
	...
	/** MAINLOOP_PARTITION: for(batch_type offset_kvs=ptravstate.begin_kvs; offset_kvs<ptravstate.end_kvs; offset_kvs+=ptravstate.skip_kvs * 2){ // 2
	#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_partitionloop avg=analysis_partitionloop
		#ifdef _DEBUGMODE_KERNELPRINTS2
		...
		bool_type en; if(offset_kvs >= 2 * (ptravstate.skip_kvs * 2)){ pp1en = ON; }
	
		readkeyvalues(ON, kvdram, sourcebuffer, (sweepparams.worksourcebaseaddress_kvs + pp0readoffset_kvs), pp0readsize_kvs, ptravstatepp0);
		#ifdef PP1
		runpipeline(pp1en, buffer_setof1, templocalcapsule_so1, buffer_setof2, templocalcapsule_so2, buffer_setof4, templocalcapsule_so4, buffer_setof8, templocalcapsule_so8, sweepparams.currentLOP, sweepparams.upperlimit, pp1cutoff, globalparams);
		#endif 
		
		pp0cutoff = partitionkeyvalues3(ON, sourcebuffer, buffer_setof1, templocalcapsule_so1, sweepparams.currentLOP, sweepparams.upperlimit, ptravstatepp0, globalparams);
		#ifdef PP1
		savekeyvalues(pp1en, kvdram, buffer_setof8, globalstatsbuffer, templocalcapsule_so8, sweepparams.workdestbaseaddress_kvs, globalparams);
		#endif 
		
		runpipeline(ON, buffer_setof1, templocalcapsule_so1, buffer_setof2, templocalcapsule_so2, buffer_setof4, templocalcapsule_so4, buffer_setof8, templocalcapsule_so8, sweepparams.currentLOP, sweepparams.upperlimit, pp0cutoff, globalparams);
		#ifdef PP1
		readkeyvalues(ON, kvdram, sourcebuffer, (sweepparams.worksourcebaseaddress_kvs + pp1readoffset_kvs), pp1readsize_kvs, ptravstatepp1);
		#endif 
		
		savekeyvalues(ON, kvdram, buffer_setof8, globalstatsbuffer, templocalcapsule_so8, sweepparams.workdestbaseaddress_kvs, globalparams);
		#ifdef PP1
		pp1cutoff = partitionkeyvalues3(ON, sourcebuffer, buffer_setof1, templocalcapsule_so1, sweepparams.currentLOP, sweepparams.upperlimit, ptravstatepp1, globalparams);
		#endif 
		
		pp0readoffset_kvs = pp1readoffset_kvs + pp1readsize_kvs;
		pp0readsize_kvs = pp0cutoff;
		pp1readoffset_kvs = pp0readoffset_kvs + pp0cutoff; // pp0readsize_kvs; // pp0cutoff;
...
dispatch(uint512_dt * kvdram){
	...
	#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_numllops avg=analysis_numllops	
		...
		#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_numsourcepartitions avg=analysis_numsourcepartitions	
			...
			// cout<<"------------- dispatch:: exiting.... "<<endl;
			// if(currentLOP==2 && source_partition==1){ break; } // 4 exit(EXIT_SUCCESS); }
			// if(currentLOP==2 && source_partition==0){ break; } // 4 exit(EXIT_SUCCESS); }
		}
		#ifdef _DEBUGMODE_KERNELPRINTS3
		actsutilityobj->printglobalvars();
		#endif 
