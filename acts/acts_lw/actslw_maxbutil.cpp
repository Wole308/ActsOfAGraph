...
void 
	...
	actslw_maxbutil::
	#endif 
readglobalstats04(bool_type enable, uint512_dt * kvdram, keyvalue_t globalstatsbuffer[NUM_PARTITIONS], batch_type offset_kvs){
	if(enable == OFF){ return; }
	#ifdef _DEBUGMODE_CHECKS2
	actsutilityobj->checkoutofbounds("readglobalstats0", offset_kvs + NUM_PARTITIONS, BASEOFFSET_STATSDRAM_KVS + KVSTATSDRAMSZ_KVS + 1, NAp, NAp, NAp);
	#endif
...
void 
	...
	actslw_maxbutil::
	#endif
collectglobalstats04(bool_type enable, keyvalue_t sourcebuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], keyvalue_t destbuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], step_type currentLOP, vertex_t upperlimit, travstate_t travstate, globalparams_t globalparams){					
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE;
	buffer_type chunk_size = getchunksize_kvs(SRCBUFFER_SIZE, travstate, 0);
...
void 
	...
	actslw_maxbutil::
	#endif
prepareglobalstats04(bool_type enable, keyvalue_t buffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], keyvalue_t globalstatsbuffer[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
...
void 
	...
	actslw_maxbutil::
	#endif 
saveglobalstats04(bool_type enable, uint512_dt * kvdram, keyvalue_t globalstatsbuffer[NUM_PARTITIONS], batch_type offset_kvs){
	if(enable == OFF){ return; }
	#ifdef _DEBUGMODE_CHECKS2
	actsutilityobj->checkoutofbounds("saveglobalstats0", offset_kvs + NUM_PARTITIONS, BASEOFFSET_STATSDRAM_KVS + KVSTATSDRAMSZ_KVS + 1, offset_kvs, NUM_PARTITIONS, KVSTATSDRAMSZ_KVS);
	#endif
...
void 
	...
	actslw_maxbutil::
	#endif 
readkeyvalues04(bool_type enable, uint512_dt * kvdram, keyvalue_t buffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], batch_type offset_kvs, travstate_t travstate){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE;
	buffer_type chunk_size = getchunksize_kvs(SRCBUFFER_SIZE, travstate, 0);
...
void 
	...
	actslw_maxbutil::
	#endif
partitionkeyvalues04(bool_type enable, keyvalue_t sourcebuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], keyvalue_t destbuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], skeyvalue_t localcapsule[VECTOR_SIZE][NUM_PARTITIONS], step_type currentLOP, vertex_t upperlimit, travstate_t travstate, globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE;
	buffer_type chunk_size = getchunksize_kvs(SRCBUFFER_SIZE, travstate, 0);
...
void 
	...
	actslw_maxbutil::
	#endif 
savekeyvalues04(bool_type enable, uint512_dt * kvdram, keyvalue_t buffer[8][PADDEDDESTBUFFER_SIZE], keyvalue_t * globalcapsule, skeyvalue_t localcapsule[NUM_PARTITIONS], batch_type globalbaseaddress_kvs, globalparams_t globalparams){				
	if(enable == OFF){ return; }
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof204_I0(bool_type enable, keyvalue_t buffer_setof1M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof1M1[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof2R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2R1[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof204_I1(bool_type enable, keyvalue_t buffer_setof1M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof1M1[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof2R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2R1[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof204_I2(bool_type enable, keyvalue_t buffer_setof1M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof1M1[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof2R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2R1[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof204_I3(bool_type enable, keyvalue_t buffer_setof1M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof1M1[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof2R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2R1[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof2stoSetof404_I0(bool_type enable, keyvalue_t buffer_setof2M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M3[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof4R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R3[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){			
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof2stoSetof404_I1(bool_type enable, keyvalue_t buffer_setof2M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M3[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof4R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R3[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){			
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof4stoSetof804_I0(bool_type enable, keyvalue_t buffer_setof4M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M3[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M4[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M5[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M6[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M7[PADDEDDESTBUFFER_SIZE],
															keyvalue_t buffer_setof8R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R3[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R4[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R5[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R6[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R7[PADDEDDESTBUFFER_SIZE],
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){				
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof2s04(bool_type enable, keyvalue_t buffer_setof1[8][PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2[8][PADDEDDESTBUFFER_SIZE], skeyvalue_t templocalcapsule_so1[8][NUM_PARTITIONS], skeyvalue_t templocalcapsule_so2[4][NUM_PARTITIONS], globalparams_t globalparams){
	// #pragma HLS INLINE
	// 1s->2s
	combineSetof1stoSetof204_I0(enable, buffer_setof1[0], buffer_setof1[1], buffer_setof2[0], buffer_setof2[1], templocalcapsule_so1[0], templocalcapsule_so1[1], templocalcapsule_so2[0], globalparams);
	combineSetof1stoSetof204_I1(enable, buffer_setof1[2], buffer_setof1[3], buffer_setof2[2], buffer_setof2[3], templocalcapsule_so1[2], templocalcapsule_so1[3], templocalcapsule_so2[1], globalparams);
	combineSetof1stoSetof204_I2(enable, buffer_setof1[4], buffer_setof1[5], buffer_setof2[4], buffer_setof2[5], templocalcapsule_so1[4], templocalcapsule_so1[5], templocalcapsule_so2[2], globalparams);
	combineSetof1stoSetof204_I3(enable, buffer_setof1[6], buffer_setof1[7], buffer_setof2[6], buffer_setof2[7], templocalcapsule_so1[6], templocalcapsule_so1[7], templocalcapsule_so2[3], globalparams);
	return;
}
void 
	#ifdef SW 
	actslw_maxbutil::
	#endif
combineSetof2stoSetof4s04(bool_type enable, keyvalue_t buffer_setof2[8][PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4[8][PADDEDDESTBUFFER_SIZE], skeyvalue_t templocalcapsule_so2[4][NUM_PARTITIONS], skeyvalue_t templocalcapsule_so4[2][NUM_PARTITIONS], globalparams_t globalparams){
	// #pragma HLS INLINE
	// 2s->4s
	combineSetof2stoSetof404_I0(enable, buffer_setof2[0], buffer_setof2[1], buffer_setof2[2], buffer_setof2[3], buffer_setof4[0], buffer_setof4[1], buffer_setof4[2], buffer_setof4[3], templocalcapsule_so2[0], templocalcapsule_so2[1], templocalcapsule_so4[0], globalparams);
	combineSetof2stoSetof404_I1(enable, buffer_setof2[4], buffer_setof2[5], buffer_setof2[6], buffer_setof2[7], buffer_setof4[4], buffer_setof4[5], buffer_setof4[6], buffer_setof4[7], templocalcapsule_so2[2], templocalcapsule_so2[3], templocalcapsule_so4[1], globalparams);
	return;
}
void 
	#ifdef SW 
	actslw_maxbutil::
	#endif
combineSetof4stoSetof8s04(bool_type enable, keyvalue_t buffer_setof4[8][PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8[8][PADDEDDESTBUFFER_SIZE], skeyvalue_t templocalcapsule_so4[2][NUM_PARTITIONS], skeyvalue_t templocalcapsule_so8[NUM_PARTITIONS], globalparams_t globalparams){
	// #pragma HLS INLINE
	// 4s->8s
	combineSetof4stoSetof804_I0(enable, buffer_setof4[0], buffer_setof4[1], buffer_setof4[2], buffer_setof4[3], buffer_setof4[4], buffer_setof4[5], buffer_setof4[6], buffer_setof4[7], 
												buffer_setof8[0], buffer_setof8[1], buffer_setof8[2], buffer_setof8[3], buffer_setof8[4], buffer_setof8[5], buffer_setof8[6], buffer_setof8[7], 
													templocalcapsule_so4[0], templocalcapsule_so4[1], templocalcapsule_so8, globalparams);
	return;
}
...
void 
	...
	actslw_maxbutil::
	#endif 
readglobalstats05(bool_type enable, uint512_dt * kvdram, keyvalue_t globalstatsbuffer[NUM_PARTITIONS], batch_type offset_kvs){
	if(enable == OFF){ return; }
	#ifdef _DEBUGMODE_CHECKS2
	actsutilityobj->checkoutofbounds("readglobalstats0", offset_kvs + NUM_PARTITIONS, BASEOFFSET_STATSDRAM_KVS + KVSTATSDRAMSZ_KVS + 1, NAp, NAp, NAp);
	#endif
...
void 
	...
	actslw_maxbutil::
	#endif
collectglobalstats05(bool_type enable, keyvalue_t sourcebuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], keyvalue_t destbuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], step_type currentLOP, vertex_t upperlimit, travstate_t travstate, globalparams_t globalparams){					
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE;
	buffer_type chunk_size = getchunksize_kvs(SRCBUFFER_SIZE, travstate, 0);
...
void 
	...
	actslw_maxbutil::
	#endif
prepareglobalstats05(bool_type enable, keyvalue_t buffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], keyvalue_t globalstatsbuffer[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
...
void 
	...
	actslw_maxbutil::
	#endif 
saveglobalstats05(bool_type enable, uint512_dt * kvdram, keyvalue_t globalstatsbuffer[NUM_PARTITIONS], batch_type offset_kvs){
	if(enable == OFF){ return; }
	#ifdef _DEBUGMODE_CHECKS2
	actsutilityobj->checkoutofbounds("saveglobalstats0", offset_kvs + NUM_PARTITIONS, BASEOFFSET_STATSDRAM_KVS + KVSTATSDRAMSZ_KVS + 1, offset_kvs, NUM_PARTITIONS, KVSTATSDRAMSZ_KVS);
	#endif
...
void 
	...
	actslw_maxbutil::
	#endif 
readkeyvalues05(bool_type enable, uint512_dt * kvdram, keyvalue_t buffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], batch_type offset_kvs, travstate_t travstate){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE;
	buffer_type chunk_size = getchunksize_kvs(SRCBUFFER_SIZE, travstate, 0);
...
void 
	...
	actslw_maxbutil::
	#endif
partitionkeyvalues05(bool_type enable, keyvalue_t sourcebuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], keyvalue_t destbuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], skeyvalue_t localcapsule[VECTOR_SIZE][NUM_PARTITIONS], step_type currentLOP, vertex_t upperlimit, travstate_t travstate, globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE;
	buffer_type chunk_size = getchunksize_kvs(SRCBUFFER_SIZE, travstate, 0);
...
void 
	...
	actslw_maxbutil::
	#endif 
savekeyvalues05(bool_type enable, uint512_dt * kvdram, keyvalue_t buffer[8][PADDEDDESTBUFFER_SIZE], keyvalue_t * globalcapsule, skeyvalue_t localcapsule[NUM_PARTITIONS], batch_type globalbaseaddress_kvs, globalparams_t globalparams){				
	if(enable == OFF){ return; }
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof205_I0(bool_type enable, keyvalue_t buffer_setof1M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof1M1[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof2R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2R1[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof205_I1(bool_type enable, keyvalue_t buffer_setof1M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof1M1[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof2R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2R1[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof205_I2(bool_type enable, keyvalue_t buffer_setof1M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof1M1[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof2R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2R1[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof205_I3(bool_type enable, keyvalue_t buffer_setof1M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof1M1[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof2R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2R1[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof2stoSetof405_I0(bool_type enable, keyvalue_t buffer_setof2M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M3[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof4R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R3[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){			
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof2stoSetof405_I1(bool_type enable, keyvalue_t buffer_setof2M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M3[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof4R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R3[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){			
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof4stoSetof805_I0(bool_type enable, keyvalue_t buffer_setof4M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M3[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M4[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M5[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M6[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M7[PADDEDDESTBUFFER_SIZE],
															keyvalue_t buffer_setof8R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R3[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R4[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R5[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R6[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R7[PADDEDDESTBUFFER_SIZE],
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){				
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof2s05(bool_type enable, keyvalue_t buffer_setof1[8][PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2[8][PADDEDDESTBUFFER_SIZE], skeyvalue_t templocalcapsule_so1[8][NUM_PARTITIONS], skeyvalue_t templocalcapsule_so2[4][NUM_PARTITIONS], globalparams_t globalparams){
	// #pragma HLS INLINE
	// 1s->2s
	combineSetof1stoSetof205_I0(enable, buffer_setof1[0], buffer_setof1[1], buffer_setof2[0], buffer_setof2[1], templocalcapsule_so1[0], templocalcapsule_so1[1], templocalcapsule_so2[0], globalparams);
	combineSetof1stoSetof205_I1(enable, buffer_setof1[2], buffer_setof1[3], buffer_setof2[2], buffer_setof2[3], templocalcapsule_so1[2], templocalcapsule_so1[3], templocalcapsule_so2[1], globalparams);
	combineSetof1stoSetof205_I2(enable, buffer_setof1[4], buffer_setof1[5], buffer_setof2[4], buffer_setof2[5], templocalcapsule_so1[4], templocalcapsule_so1[5], templocalcapsule_so2[2], globalparams);
	combineSetof1stoSetof205_I3(enable, buffer_setof1[6], buffer_setof1[7], buffer_setof2[6], buffer_setof2[7], templocalcapsule_so1[6], templocalcapsule_so1[7], templocalcapsule_so2[3], globalparams);
	return;
}
void 
	#ifdef SW 
	actslw_maxbutil::
	#endif
combineSetof2stoSetof4s05(bool_type enable, keyvalue_t buffer_setof2[8][PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4[8][PADDEDDESTBUFFER_SIZE], skeyvalue_t templocalcapsule_so2[4][NUM_PARTITIONS], skeyvalue_t templocalcapsule_so4[2][NUM_PARTITIONS], globalparams_t globalparams){
	// #pragma HLS INLINE
	// 2s->4s
	combineSetof2stoSetof405_I0(enable, buffer_setof2[0], buffer_setof2[1], buffer_setof2[2], buffer_setof2[3], buffer_setof4[0], buffer_setof4[1], buffer_setof4[2], buffer_setof4[3], templocalcapsule_so2[0], templocalcapsule_so2[1], templocalcapsule_so4[0], globalparams);
	combineSetof2stoSetof405_I1(enable, buffer_setof2[4], buffer_setof2[5], buffer_setof2[6], buffer_setof2[7], buffer_setof4[4], buffer_setof4[5], buffer_setof4[6], buffer_setof4[7], templocalcapsule_so2[2], templocalcapsule_so2[3], templocalcapsule_so4[1], globalparams);
	return;
}
void 
	#ifdef SW 
	actslw_maxbutil::
	#endif
combineSetof4stoSetof8s05(bool_type enable, keyvalue_t buffer_setof4[8][PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8[8][PADDEDDESTBUFFER_SIZE], skeyvalue_t templocalcapsule_so4[2][NUM_PARTITIONS], skeyvalue_t templocalcapsule_so8[NUM_PARTITIONS], globalparams_t globalparams){
	// #pragma HLS INLINE
	// 4s->8s
	combineSetof4stoSetof805_I0(enable, buffer_setof4[0], buffer_setof4[1], buffer_setof4[2], buffer_setof4[3], buffer_setof4[4], buffer_setof4[5], buffer_setof4[6], buffer_setof4[7], 
												buffer_setof8[0], buffer_setof8[1], buffer_setof8[2], buffer_setof8[3], buffer_setof8[4], buffer_setof8[5], buffer_setof8[6], buffer_setof8[7], 
													templocalcapsule_so4[0], templocalcapsule_so4[1], templocalcapsule_so8, globalparams);
	return;
}
...
void 
	...
	actslw_maxbutil::
	#endif 
readglobalstats06(bool_type enable, uint512_dt * kvdram, keyvalue_t globalstatsbuffer[NUM_PARTITIONS], batch_type offset_kvs){
	if(enable == OFF){ return; }
	#ifdef _DEBUGMODE_CHECKS2
	actsutilityobj->checkoutofbounds("readglobalstats0", offset_kvs + NUM_PARTITIONS, BASEOFFSET_STATSDRAM_KVS + KVSTATSDRAMSZ_KVS + 1, NAp, NAp, NAp);
	#endif
...
void 
	...
	actslw_maxbutil::
	#endif
collectglobalstats06(bool_type enable, keyvalue_t sourcebuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], keyvalue_t destbuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], step_type currentLOP, vertex_t upperlimit, travstate_t travstate, globalparams_t globalparams){					
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE;
	buffer_type chunk_size = getchunksize_kvs(SRCBUFFER_SIZE, travstate, 0);
...
void 
	...
	actslw_maxbutil::
	#endif
prepareglobalstats06(bool_type enable, keyvalue_t buffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], keyvalue_t globalstatsbuffer[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
...
void 
	...
	actslw_maxbutil::
	#endif 
saveglobalstats06(bool_type enable, uint512_dt * kvdram, keyvalue_t globalstatsbuffer[NUM_PARTITIONS], batch_type offset_kvs){
	if(enable == OFF){ return; }
	#ifdef _DEBUGMODE_CHECKS2
	actsutilityobj->checkoutofbounds("saveglobalstats0", offset_kvs + NUM_PARTITIONS, BASEOFFSET_STATSDRAM_KVS + KVSTATSDRAMSZ_KVS + 1, offset_kvs, NUM_PARTITIONS, KVSTATSDRAMSZ_KVS);
	#endif
...
void 
	...
	actslw_maxbutil::
	#endif 
readkeyvalues06(bool_type enable, uint512_dt * kvdram, keyvalue_t buffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], batch_type offset_kvs, travstate_t travstate){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE;
	buffer_type chunk_size = getchunksize_kvs(SRCBUFFER_SIZE, travstate, 0);
...
void 
	...
	actslw_maxbutil::
	#endif
partitionkeyvalues06(bool_type enable, keyvalue_t sourcebuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], keyvalue_t destbuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], skeyvalue_t localcapsule[VECTOR_SIZE][NUM_PARTITIONS], step_type currentLOP, vertex_t upperlimit, travstate_t travstate, globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE;
	buffer_type chunk_size = getchunksize_kvs(SRCBUFFER_SIZE, travstate, 0);
...
void 
	...
	actslw_maxbutil::
	#endif 
savekeyvalues06(bool_type enable, uint512_dt * kvdram, keyvalue_t buffer[8][PADDEDDESTBUFFER_SIZE], keyvalue_t * globalcapsule, skeyvalue_t localcapsule[NUM_PARTITIONS], batch_type globalbaseaddress_kvs, globalparams_t globalparams){				
	if(enable == OFF){ return; }
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof206_I0(bool_type enable, keyvalue_t buffer_setof1M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof1M1[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof2R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2R1[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof206_I1(bool_type enable, keyvalue_t buffer_setof1M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof1M1[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof2R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2R1[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof206_I2(bool_type enable, keyvalue_t buffer_setof1M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof1M1[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof2R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2R1[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof206_I3(bool_type enable, keyvalue_t buffer_setof1M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof1M1[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof2R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2R1[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof2stoSetof406_I0(bool_type enable, keyvalue_t buffer_setof2M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M3[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof4R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R3[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){			
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof2stoSetof406_I1(bool_type enable, keyvalue_t buffer_setof2M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M3[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof4R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R3[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){			
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof4stoSetof806_I0(bool_type enable, keyvalue_t buffer_setof4M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M3[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M4[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M5[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M6[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M7[PADDEDDESTBUFFER_SIZE],
															keyvalue_t buffer_setof8R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R3[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R4[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R5[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R6[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R7[PADDEDDESTBUFFER_SIZE],
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){				
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof2s06(bool_type enable, keyvalue_t buffer_setof1[8][PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2[8][PADDEDDESTBUFFER_SIZE], skeyvalue_t templocalcapsule_so1[8][NUM_PARTITIONS], skeyvalue_t templocalcapsule_so2[4][NUM_PARTITIONS], globalparams_t globalparams){
	// #pragma HLS INLINE
	// 1s->2s
	combineSetof1stoSetof206_I0(enable, buffer_setof1[0], buffer_setof1[1], buffer_setof2[0], buffer_setof2[1], templocalcapsule_so1[0], templocalcapsule_so1[1], templocalcapsule_so2[0], globalparams);
	combineSetof1stoSetof206_I1(enable, buffer_setof1[2], buffer_setof1[3], buffer_setof2[2], buffer_setof2[3], templocalcapsule_so1[2], templocalcapsule_so1[3], templocalcapsule_so2[1], globalparams);
	combineSetof1stoSetof206_I2(enable, buffer_setof1[4], buffer_setof1[5], buffer_setof2[4], buffer_setof2[5], templocalcapsule_so1[4], templocalcapsule_so1[5], templocalcapsule_so2[2], globalparams);
	combineSetof1stoSetof206_I3(enable, buffer_setof1[6], buffer_setof1[7], buffer_setof2[6], buffer_setof2[7], templocalcapsule_so1[6], templocalcapsule_so1[7], templocalcapsule_so2[3], globalparams);
	return;
}
void 
	#ifdef SW 
	actslw_maxbutil::
	#endif
combineSetof2stoSetof4s06(bool_type enable, keyvalue_t buffer_setof2[8][PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4[8][PADDEDDESTBUFFER_SIZE], skeyvalue_t templocalcapsule_so2[4][NUM_PARTITIONS], skeyvalue_t templocalcapsule_so4[2][NUM_PARTITIONS], globalparams_t globalparams){
	// #pragma HLS INLINE
	// 2s->4s
	combineSetof2stoSetof406_I0(enable, buffer_setof2[0], buffer_setof2[1], buffer_setof2[2], buffer_setof2[3], buffer_setof4[0], buffer_setof4[1], buffer_setof4[2], buffer_setof4[3], templocalcapsule_so2[0], templocalcapsule_so2[1], templocalcapsule_so4[0], globalparams);
	combineSetof2stoSetof406_I1(enable, buffer_setof2[4], buffer_setof2[5], buffer_setof2[6], buffer_setof2[7], buffer_setof4[4], buffer_setof4[5], buffer_setof4[6], buffer_setof4[7], templocalcapsule_so2[2], templocalcapsule_so2[3], templocalcapsule_so4[1], globalparams);
	return;
}
void 
	#ifdef SW 
	actslw_maxbutil::
	#endif
combineSetof4stoSetof8s06(bool_type enable, keyvalue_t buffer_setof4[8][PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8[8][PADDEDDESTBUFFER_SIZE], skeyvalue_t templocalcapsule_so4[2][NUM_PARTITIONS], skeyvalue_t templocalcapsule_so8[NUM_PARTITIONS], globalparams_t globalparams){
	// #pragma HLS INLINE
	// 4s->8s
	combineSetof4stoSetof806_I0(enable, buffer_setof4[0], buffer_setof4[1], buffer_setof4[2], buffer_setof4[3], buffer_setof4[4], buffer_setof4[5], buffer_setof4[6], buffer_setof4[7], 
												buffer_setof8[0], buffer_setof8[1], buffer_setof8[2], buffer_setof8[3], buffer_setof8[4], buffer_setof8[5], buffer_setof8[6], buffer_setof8[7], 
													templocalcapsule_so4[0], templocalcapsule_so4[1], templocalcapsule_so8, globalparams);
	return;
}
...
void 
	...
	actslw_maxbutil::
	#endif 
readglobalstats07(bool_type enable, uint512_dt * kvdram, keyvalue_t globalstatsbuffer[NUM_PARTITIONS], batch_type offset_kvs){
	if(enable == OFF){ return; }
	#ifdef _DEBUGMODE_CHECKS2
	actsutilityobj->checkoutofbounds("readglobalstats0", offset_kvs + NUM_PARTITIONS, BASEOFFSET_STATSDRAM_KVS + KVSTATSDRAMSZ_KVS + 1, NAp, NAp, NAp);
	#endif
...
void 
	...
	actslw_maxbutil::
	#endif
collectglobalstats07(bool_type enable, keyvalue_t sourcebuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], keyvalue_t destbuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], step_type currentLOP, vertex_t upperlimit, travstate_t travstate, globalparams_t globalparams){					
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE;
	buffer_type chunk_size = getchunksize_kvs(SRCBUFFER_SIZE, travstate, 0);
...
void 
	...
	actslw_maxbutil::
	#endif
prepareglobalstats07(bool_type enable, keyvalue_t buffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], keyvalue_t globalstatsbuffer[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
...
void 
	...
	actslw_maxbutil::
	#endif 
saveglobalstats07(bool_type enable, uint512_dt * kvdram, keyvalue_t globalstatsbuffer[NUM_PARTITIONS], batch_type offset_kvs){
	if(enable == OFF){ return; }
	#ifdef _DEBUGMODE_CHECKS2
	actsutilityobj->checkoutofbounds("saveglobalstats0", offset_kvs + NUM_PARTITIONS, BASEOFFSET_STATSDRAM_KVS + KVSTATSDRAMSZ_KVS + 1, offset_kvs, NUM_PARTITIONS, KVSTATSDRAMSZ_KVS);
	#endif
...
void 
	...
	actslw_maxbutil::
	#endif 
readkeyvalues07(bool_type enable, uint512_dt * kvdram, keyvalue_t buffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], batch_type offset_kvs, travstate_t travstate){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE;
	buffer_type chunk_size = getchunksize_kvs(SRCBUFFER_SIZE, travstate, 0);
...
void 
	...
	actslw_maxbutil::
	#endif
partitionkeyvalues07(bool_type enable, keyvalue_t sourcebuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], keyvalue_t destbuffer[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE], skeyvalue_t localcapsule[VECTOR_SIZE][NUM_PARTITIONS], step_type currentLOP, vertex_t upperlimit, travstate_t travstate, globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE;
	buffer_type chunk_size = getchunksize_kvs(SRCBUFFER_SIZE, travstate, 0);
...
void 
	...
	actslw_maxbutil::
	#endif 
savekeyvalues07(bool_type enable, uint512_dt * kvdram, keyvalue_t buffer[8][PADDEDDESTBUFFER_SIZE], keyvalue_t * globalcapsule, skeyvalue_t localcapsule[NUM_PARTITIONS], batch_type globalbaseaddress_kvs, globalparams_t globalparams){				
	if(enable == OFF){ return; }
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof207_I0(bool_type enable, keyvalue_t buffer_setof1M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof1M1[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof2R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2R1[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof207_I1(bool_type enable, keyvalue_t buffer_setof1M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof1M1[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof2R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2R1[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof207_I2(bool_type enable, keyvalue_t buffer_setof1M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof1M1[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof2R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2R1[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof207_I3(bool_type enable, keyvalue_t buffer_setof1M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof1M1[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof2R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2R1[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof2stoSetof407_I0(bool_type enable, keyvalue_t buffer_setof2M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M3[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof4R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R3[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){			
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof2stoSetof407_I1(bool_type enable, keyvalue_t buffer_setof2M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2M3[PADDEDDESTBUFFER_SIZE], 
															keyvalue_t buffer_setof4R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4R3[PADDEDDESTBUFFER_SIZE], 
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){			
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof4stoSetof807_I0(bool_type enable, keyvalue_t buffer_setof4M0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M3[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M4[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M5[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M6[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4M7[PADDEDDESTBUFFER_SIZE],
															keyvalue_t buffer_setof8R0[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R1[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R2[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R3[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R4[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R5[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R6[PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8R7[PADDEDDESTBUFFER_SIZE],
																skeyvalue_t localcapsuleM[NUM_PARTITIONS], skeyvalue_t localcapsuleN[NUM_PARTITIONS], skeyvalue_t localcapsuleR[NUM_PARTITIONS], globalparams_t globalparams){				
	if(enable == OFF){ return; }
	analysis_type analysis_srcbuffersz = SRCBUFFER_SIZE / NUM_PARTITIONS;
...
void 
	...
	actslw_maxbutil::
	#endif
combineSetof1stoSetof2s07(bool_type enable, keyvalue_t buffer_setof1[8][PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof2[8][PADDEDDESTBUFFER_SIZE], skeyvalue_t templocalcapsule_so1[8][NUM_PARTITIONS], skeyvalue_t templocalcapsule_so2[4][NUM_PARTITIONS], globalparams_t globalparams){
	// #pragma HLS INLINE
	// 1s->2s
	combineSetof1stoSetof207_I0(enable, buffer_setof1[0], buffer_setof1[1], buffer_setof2[0], buffer_setof2[1], templocalcapsule_so1[0], templocalcapsule_so1[1], templocalcapsule_so2[0], globalparams);
	combineSetof1stoSetof207_I1(enable, buffer_setof1[2], buffer_setof1[3], buffer_setof2[2], buffer_setof2[3], templocalcapsule_so1[2], templocalcapsule_so1[3], templocalcapsule_so2[1], globalparams);
	combineSetof1stoSetof207_I2(enable, buffer_setof1[4], buffer_setof1[5], buffer_setof2[4], buffer_setof2[5], templocalcapsule_so1[4], templocalcapsule_so1[5], templocalcapsule_so2[2], globalparams);
	combineSetof1stoSetof207_I3(enable, buffer_setof1[6], buffer_setof1[7], buffer_setof2[6], buffer_setof2[7], templocalcapsule_so1[6], templocalcapsule_so1[7], templocalcapsule_so2[3], globalparams);
	return;
}
void 
	#ifdef SW 
	actslw_maxbutil::
	#endif
combineSetof2stoSetof4s07(bool_type enable, keyvalue_t buffer_setof2[8][PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof4[8][PADDEDDESTBUFFER_SIZE], skeyvalue_t templocalcapsule_so2[4][NUM_PARTITIONS], skeyvalue_t templocalcapsule_so4[2][NUM_PARTITIONS], globalparams_t globalparams){
	// #pragma HLS INLINE
	// 2s->4s
	combineSetof2stoSetof407_I0(enable, buffer_setof2[0], buffer_setof2[1], buffer_setof2[2], buffer_setof2[3], buffer_setof4[0], buffer_setof4[1], buffer_setof4[2], buffer_setof4[3], templocalcapsule_so2[0], templocalcapsule_so2[1], templocalcapsule_so4[0], globalparams);
	combineSetof2stoSetof407_I1(enable, buffer_setof2[4], buffer_setof2[5], buffer_setof2[6], buffer_setof2[7], buffer_setof4[4], buffer_setof4[5], buffer_setof4[6], buffer_setof4[7], templocalcapsule_so2[2], templocalcapsule_so2[3], templocalcapsule_so4[1], globalparams);
	return;
}
void 
	#ifdef SW 
	actslw_maxbutil::
	#endif
combineSetof4stoSetof8s07(bool_type enable, keyvalue_t buffer_setof4[8][PADDEDDESTBUFFER_SIZE], keyvalue_t buffer_setof8[8][PADDEDDESTBUFFER_SIZE], skeyvalue_t templocalcapsule_so4[2][NUM_PARTITIONS], skeyvalue_t templocalcapsule_so8[NUM_PARTITIONS], globalparams_t globalparams){
	// #pragma HLS INLINE
	// 4s->8s
	combineSetof4stoSetof807_I0(enable, buffer_setof4[0], buffer_setof4[1], buffer_setof4[2], buffer_setof4[3], buffer_setof4[4], buffer_setof4[5], buffer_setof4[6], buffer_setof4[7], 
												buffer_setof8[0], buffer_setof8[1], buffer_setof8[2], buffer_setof8[3], buffer_setof8[4], buffer_setof8[5], buffer_setof8[6], buffer_setof8[7], 
													templocalcapsule_so4[0], templocalcapsule_so4[1], templocalcapsule_so8, globalparams);
	return;
}
...
void 
	...
	actslw_maxbutil::
	#endif 
maxdispatch0( uint512_dt * kvdram0  ,uint512_dt * kvdram1  ,uint512_dt * kvdram2  ,uint512_dt * kvdram3  ,uint512_dt * kvdram4  ,uint512_dt * kvdram5  ,uint512_dt * kvdram6  ,uint512_dt * kvdram7 ){
	...
	#endif
	keyvalue_t sourcebuffer4[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = sourcebuffer4
	
	keyvalue_t buffer4_setof1[8][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = buffer4_setof1
	skeyvalue_t templocalcapsule4_so1[8][NUM_PARTITIONS];
	#pragma HLS array_partition variable = templocalcapsule4_so1
	
	keyvalue_t buffer4_setof2[8][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = buffer4_setof2
	skeyvalue_t templocalcapsule4_so2[4][NUM_PARTITIONS];
	#pragma HLS array_partition variable = templocalcapsule4_so2
	
	keyvalue_t buffer4_setof4[8][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = buffer4_setof4
	skeyvalue_t templocalcapsule4_so4[2][NUM_PARTITIONS];
	#pragma HLS array_partition variable = templocalcapsule4_so4
	
	keyvalue_t buffer4_setof8[8][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = buffer4_setof8
	skeyvalue_t templocalcapsule4_so8[NUM_PARTITIONS];

	keyvalue_t globalstatsbuffer4[NUM_PARTITIONS];
	batch_type skipsizes4[NUM_PARTITIONS];
	#ifdef _DEBUGMODE_CHECKS2
	keyvalue_t BIGKV4[NUM_PARTITIONS];
	#endif
	keyvalue_t sourcebuffer5[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = sourcebuffer5
	
	keyvalue_t buffer5_setof1[8][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = buffer5_setof1
	skeyvalue_t templocalcapsule5_so1[8][NUM_PARTITIONS];
	#pragma HLS array_partition variable = templocalcapsule5_so1
	
	keyvalue_t buffer5_setof2[8][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = buffer5_setof2
	skeyvalue_t templocalcapsule5_so2[4][NUM_PARTITIONS];
	#pragma HLS array_partition variable = templocalcapsule5_so2
	
	keyvalue_t buffer5_setof4[8][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = buffer5_setof4
	skeyvalue_t templocalcapsule5_so4[2][NUM_PARTITIONS];
	#pragma HLS array_partition variable = templocalcapsule5_so4
	
	keyvalue_t buffer5_setof8[8][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = buffer5_setof8
	skeyvalue_t templocalcapsule5_so8[NUM_PARTITIONS];

	keyvalue_t globalstatsbuffer5[NUM_PARTITIONS];
	batch_type skipsizes5[NUM_PARTITIONS];
	#ifdef _DEBUGMODE_CHECKS2
	keyvalue_t BIGKV5[NUM_PARTITIONS];
	#endif
	keyvalue_t sourcebuffer6[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = sourcebuffer6
	
	keyvalue_t buffer6_setof1[8][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = buffer6_setof1
	skeyvalue_t templocalcapsule6_so1[8][NUM_PARTITIONS];
	#pragma HLS array_partition variable = templocalcapsule6_so1
	
	keyvalue_t buffer6_setof2[8][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = buffer6_setof2
	skeyvalue_t templocalcapsule6_so2[4][NUM_PARTITIONS];
	#pragma HLS array_partition variable = templocalcapsule6_so2
	
	keyvalue_t buffer6_setof4[8][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = buffer6_setof4
	skeyvalue_t templocalcapsule6_so4[2][NUM_PARTITIONS];
	#pragma HLS array_partition variable = templocalcapsule6_so4
	
	keyvalue_t buffer6_setof8[8][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = buffer6_setof8
	skeyvalue_t templocalcapsule6_so8[NUM_PARTITIONS];

	keyvalue_t globalstatsbuffer6[NUM_PARTITIONS];
	batch_type skipsizes6[NUM_PARTITIONS];
	#ifdef _DEBUGMODE_CHECKS2
	keyvalue_t BIGKV6[NUM_PARTITIONS];
	#endif
	keyvalue_t sourcebuffer7[VECTOR_SIZE][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = sourcebuffer7
	
	keyvalue_t buffer7_setof1[8][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = buffer7_setof1
	skeyvalue_t templocalcapsule7_so1[8][NUM_PARTITIONS];
	#pragma HLS array_partition variable = templocalcapsule7_so1
	
	keyvalue_t buffer7_setof2[8][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = buffer7_setof2
	skeyvalue_t templocalcapsule7_so2[4][NUM_PARTITIONS];
	#pragma HLS array_partition variable = templocalcapsule7_so2
	
	keyvalue_t buffer7_setof4[8][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = buffer7_setof4
	skeyvalue_t templocalcapsule7_so4[2][NUM_PARTITIONS];
	#pragma HLS array_partition variable = templocalcapsule7_so4
	
	keyvalue_t buffer7_setof8[8][PADDEDDESTBUFFER_SIZE];
	#pragma HLS array_partition variable = buffer7_setof8
	skeyvalue_t templocalcapsule7_so8[NUM_PARTITIONS];

	keyvalue_t globalstatsbuffer7[NUM_PARTITIONS];
	batch_type skipsizes7[NUM_PARTITIONS];
	#ifdef _DEBUGMODE_CHECKS2
	keyvalue_t BIGKV7[NUM_PARTITIONS];
	#endif
	...
	config_t config4;
	globalparams_t globalparams4 = getglobalparams(kvdram4);
	sweepparams_t sweepparams4;
	#ifdef _DEBUGMODE_CHECKS2
	if(globalparams4.runsize >= MAXKVDATA_BATCHSIZE){ cout<<"maxdispatch:ERROR. runsize too large!. globalparams4.runsize: "<<globalparams4.runsize<<", MAXKVDATA_BATCHSIZE: "<<MAXKVDATA_BATCHSIZE<<". EXITING"<<endl; exit(EXIT_FAILURE); }
	#endif
	batch_type sourcestatsmarker5 = 0;
	batch_type deststatsmarker5 = 1;
	batch_type destoffset5 = 0;
	config_t config5;
	globalparams_t globalparams5 = getglobalparams(kvdram5);
	sweepparams_t sweepparams5;
	#ifdef _DEBUGMODE_CHECKS2
	if(globalparams5.runsize >= MAXKVDATA_BATCHSIZE){ cout<<"maxdispatch:ERROR. runsize too large!. globalparams5.runsize: "<<globalparams5.runsize<<", MAXKVDATA_BATCHSIZE: "<<MAXKVDATA_BATCHSIZE<<". EXITING"<<endl; exit(EXIT_FAILURE); }
	#endif
	batch_type sourcestatsmarker6 = 0;
	batch_type deststatsmarker6 = 1;
	batch_type destoffset6 = 0;
	config_t config6;
	globalparams_t globalparams6 = getglobalparams(kvdram6);
	sweepparams_t sweepparams6;
	#ifdef _DEBUGMODE_CHECKS2
	if(globalparams6.runsize >= MAXKVDATA_BATCHSIZE){ cout<<"maxdispatch:ERROR. runsize too large!. globalparams6.runsize: "<<globalparams6.runsize<<", MAXKVDATA_BATCHSIZE: "<<MAXKVDATA_BATCHSIZE<<". EXITING"<<endl; exit(EXIT_FAILURE); }
	#endif
	batch_type sourcestatsmarker7 = 0;
	batch_type deststatsmarker7 = 1;
	batch_type destoffset7 = 0;
	config_t config7;
	globalparams_t globalparams7 = getglobalparams(kvdram7);
	sweepparams_t sweepparams7;
	#ifdef _DEBUGMODE_CHECKS2
	if(globalparams7.runsize >= MAXKVDATA_BATCHSIZE){ cout<<"maxdispatch:ERROR. runsize too large!. globalparams7.runsize: "<<globalparams7.runsize<<", MAXKVDATA_BATCHSIZE: "<<MAXKVDATA_BATCHSIZE<<". EXITING"<<endl; exit(EXIT_FAILURE); }
	#endif
	...
	#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_numllops avg=analysis_numllops	
		...
		#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_numsourcepartitions avg=analysis_numsourcepartitions	
			...
			resetmanykeyandvalues(buffer4_setof1, NUM_PARTITIONS);
			resetmanykeyandvalues(globalstatsbuffer4, NUM_PARTITIONS);
			resetmanykeyandvalues(buffer5_setof1, NUM_PARTITIONS);
			resetmanykeyandvalues(globalstatsbuffer5, NUM_PARTITIONS);
			resetmanykeyandvalues(buffer6_setof1, NUM_PARTITIONS);
			resetmanykeyandvalues(globalstatsbuffer6, NUM_PARTITIONS);
			resetmanykeyandvalues(buffer7_setof1, NUM_PARTITIONS);
			resetmanykeyandvalues(globalstatsbuffer7, NUM_PARTITIONS);
			...
			#endif
			sweepparams4 = getsweepparams(globalparams4, currentLOP, source_partition);
			travstate_t travstate4 = gettravstate(kvdram4, globalparams4, currentLOP, sourcestatsmarker4);
			travstate_t CStravstate4 = travstate4;
			travstate_t Ptravstate4 = travstate4;
			travstate_t Rtravstate4 = travstate4;
			travstate_t AVtravstate4;
			#ifdef _DEBUGMODE_KERNELPRINTS2
			actsutilityobj->setstructs(config4, sweepparams4, travstate4);
			#endif
			sweepparams5 = getsweepparams(globalparams5, currentLOP, source_partition);
			travstate_t travstate5 = gettravstate(kvdram5, globalparams5, currentLOP, sourcestatsmarker5);
			travstate_t CStravstate5 = travstate5;
			travstate_t Ptravstate5 = travstate5;
			travstate_t Rtravstate5 = travstate5;
			travstate_t AVtravstate5;
			#ifdef _DEBUGMODE_KERNELPRINTS2
			actsutilityobj->setstructs(config5, sweepparams5, travstate5);
			#endif
			sweepparams6 = getsweepparams(globalparams6, currentLOP, source_partition);
			travstate_t travstate6 = gettravstate(kvdram6, globalparams6, currentLOP, sourcestatsmarker6);
			travstate_t CStravstate6 = travstate6;
			travstate_t Ptravstate6 = travstate6;
			travstate_t Rtravstate6 = travstate6;
			travstate_t AVtravstate6;
			#ifdef _DEBUGMODE_KERNELPRINTS2
			actsutilityobj->setstructs(config6, sweepparams6, travstate6);
			#endif
			sweepparams7 = getsweepparams(globalparams7, currentLOP, source_partition);
			travstate_t travstate7 = gettravstate(kvdram7, globalparams7, currentLOP, sourcestatsmarker7);
			travstate_t CStravstate7 = travstate7;
			travstate_t Ptravstate7 = travstate7;
			travstate_t Rtravstate7 = travstate7;
			travstate_t AVtravstate7;
			#ifdef _DEBUGMODE_KERNELPRINTS2
			actsutilityobj->setstructs(config7, sweepparams7, travstate7);
			#endif
			...
			#endif
			if(currentLOP >= 1 && currentLOP <= globalparams4.treedepth){ config4.enableprocessedges = OFF; config4.enablecollectglobalstats = ON; config4.enablepartition = OFF; config4.enablereduce = OFF; } 
			else { CStravstate4.begin_kvs = 0; CStravstate4.end_kvs = 0; config4.enablecollectglobalstats = OFF; }
			#ifdef _DEBUGMODE_KERNELPRINTS2
			if(config4.enablecollectglobalstats == ON){ actsutilityobj->print7("### maxdispatch0::collectgstats:: source_p", "upperlimit", "begin", "end", "size", "dest range", "currentLOP", sweepparams4.source_partition, sweepparams4.upperlimit, CStravstate4.begin_kvs * VECTOR_SIZE, CStravstate4.end_kvs * VECTOR_SIZE, (CStravstate4.end_kvs - CStravstate4.begin_kvs) * VECTOR_SIZE, BATCH_RANGE / (1 << (NUM_PARTITIONS_POW * sweepparams4.currentLOP)), sweepparams4.currentLOP); }					
			#endif
			if(currentLOP >= 1 && currentLOP <= globalparams5.treedepth){ config5.enableprocessedges = OFF; config5.enablecollectglobalstats = ON; config5.enablepartition = OFF; config5.enablereduce = OFF; } 
			else { CStravstate5.begin_kvs = 0; CStravstate5.end_kvs = 0; config5.enablecollectglobalstats = OFF; }
			#ifdef _DEBUGMODE_KERNELPRINTS2
			if(config5.enablecollectglobalstats == ON){ actsutilityobj->print7("### maxdispatch0::collectgstats:: source_p", "upperlimit", "begin", "end", "size", "dest range", "currentLOP", sweepparams5.source_partition, sweepparams5.upperlimit, CStravstate5.begin_kvs * VECTOR_SIZE, CStravstate5.end_kvs * VECTOR_SIZE, (CStravstate5.end_kvs - CStravstate5.begin_kvs) * VECTOR_SIZE, BATCH_RANGE / (1 << (NUM_PARTITIONS_POW * sweepparams5.currentLOP)), sweepparams5.currentLOP); }					
			#endif
			if(currentLOP >= 1 && currentLOP <= globalparams6.treedepth){ config6.enableprocessedges = OFF; config6.enablecollectglobalstats = ON; config6.enablepartition = OFF; config6.enablereduce = OFF; } 
			else { CStravstate6.begin_kvs = 0; CStravstate6.end_kvs = 0; config6.enablecollectglobalstats = OFF; }
			#ifdef _DEBUGMODE_KERNELPRINTS2
			if(config6.enablecollectglobalstats == ON){ actsutilityobj->print7("### maxdispatch0::collectgstats:: source_p", "upperlimit", "begin", "end", "size", "dest range", "currentLOP", sweepparams6.source_partition, sweepparams6.upperlimit, CStravstate6.begin_kvs * VECTOR_SIZE, CStravstate6.end_kvs * VECTOR_SIZE, (CStravstate6.end_kvs - CStravstate6.begin_kvs) * VECTOR_SIZE, BATCH_RANGE / (1 << (NUM_PARTITIONS_POW * sweepparams6.currentLOP)), sweepparams6.currentLOP); }					
			#endif
			if(currentLOP >= 1 && currentLOP <= globalparams7.treedepth){ config7.enableprocessedges = OFF; config7.enablecollectglobalstats = ON; config7.enablepartition = OFF; config7.enablereduce = OFF; } 
			else { CStravstate7.begin_kvs = 0; CStravstate7.end_kvs = 0; config7.enablecollectglobalstats = OFF; }
			#ifdef _DEBUGMODE_KERNELPRINTS2
			if(config7.enablecollectglobalstats == ON){ actsutilityobj->print7("### maxdispatch0::collectgstats:: source_p", "upperlimit", "begin", "end", "size", "dest range", "currentLOP", sweepparams7.source_partition, sweepparams7.upperlimit, CStravstate7.begin_kvs * VECTOR_SIZE, CStravstate7.end_kvs * VECTOR_SIZE, (CStravstate7.end_kvs - CStravstate7.begin_kvs) * VECTOR_SIZE, BATCH_RANGE / (1 << (NUM_PARTITIONS_POW * sweepparams7.currentLOP)), sweepparams7.currentLOP); }					
			#endif
			MAIN_LOOP1C_COLLECTGLOBALSTATS: for(batch_type offset_kvs=CStravstate0.begin_kvs; offset_kvs<CStravstate0.end_kvs; offset_kvs+=CStravstate0.skip_kvs * NUMSUBWORKERS){
			#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_collectstatsloop avg=analysis_collectstatsloop
				...
				CStravstate4.i_kvs = offset_kvs;
				CStravstate5.i_kvs = offset_kvs;
				CStravstate6.i_kvs = offset_kvs;
				CStravstate7.i_kvs = offset_kvs;
				...
				readkeyvalues04(ON, kvdram4, sourcebuffer4, (sweepparams4.worksourcebaseaddress_kvs + offset_kvs), CStravstate4);
				readkeyvalues05(ON, kvdram5, sourcebuffer5, (sweepparams5.worksourcebaseaddress_kvs + offset_kvs), CStravstate5);
				readkeyvalues06(ON, kvdram6, sourcebuffer6, (sweepparams6.worksourcebaseaddress_kvs + offset_kvs), CStravstate6);
				readkeyvalues07(ON, kvdram7, sourcebuffer7, (sweepparams7.worksourcebaseaddress_kvs + offset_kvs), CStravstate7);
				...
				collectglobalstats04(ON, sourcebuffer4, buffer4_setof1, sweepparams4.currentLOP, sweepparams4.upperlimit, CStravstate4, globalparams4);
				collectglobalstats05(ON, sourcebuffer5, buffer5_setof1, sweepparams5.currentLOP, sweepparams5.upperlimit, CStravstate5, globalparams5);
				collectglobalstats06(ON, sourcebuffer6, buffer6_setof1, sweepparams6.currentLOP, sweepparams6.upperlimit, CStravstate6, globalparams6);
				collectglobalstats07(ON, sourcebuffer7, buffer7_setof1, sweepparams7.currentLOP, sweepparams7.upperlimit, CStravstate7, globalparams7);
			}
			...
			prepareglobalstats04(config4.enablecollectglobalstats, buffer4_setof1, globalstatsbuffer4, globalparams4);
			for(partition_type p=0; p<NUM_PARTITIONS; p++){ batch_type A = (globalstatsbuffer4[p].value + (VECTOR_SIZE-1)) / VECTOR_SIZE; batch_type B = (A + (SRCBUFFER_SIZE-1)) / SRCBUFFER_SIZE; if(B < 80){ B = B * 2; } batch_type C = ((4 * 4 * 2) * NUM_PARTITIONS) + VECTOR_SIZE; skipsizes4[p] = (B * C) + 128; } //'128' is safety padd // FIXME. REMOVEME.
			prepareglobalstats05(config5.enablecollectglobalstats, buffer5_setof1, globalstatsbuffer5, globalparams5);
			for(partition_type p=0; p<NUM_PARTITIONS; p++){ batch_type A = (globalstatsbuffer5[p].value + (VECTOR_SIZE-1)) / VECTOR_SIZE; batch_type B = (A + (SRCBUFFER_SIZE-1)) / SRCBUFFER_SIZE; if(B < 80){ B = B * 2; } batch_type C = ((4 * 4 * 2) * NUM_PARTITIONS) + VECTOR_SIZE; skipsizes5[p] = (B * C) + 128; } //'128' is safety padd // FIXME. REMOVEME.
			prepareglobalstats06(config6.enablecollectglobalstats, buffer6_setof1, globalstatsbuffer6, globalparams6);
			for(partition_type p=0; p<NUM_PARTITIONS; p++){ batch_type A = (globalstatsbuffer6[p].value + (VECTOR_SIZE-1)) / VECTOR_SIZE; batch_type B = (A + (SRCBUFFER_SIZE-1)) / SRCBUFFER_SIZE; if(B < 80){ B = B * 2; } batch_type C = ((4 * 4 * 2) * NUM_PARTITIONS) + VECTOR_SIZE; skipsizes6[p] = (B * C) + 128; } //'128' is safety padd // FIXME. REMOVEME.
			prepareglobalstats07(config7.enablecollectglobalstats, buffer7_setof1, globalstatsbuffer7, globalparams7);
			for(partition_type p=0; p<NUM_PARTITIONS; p++){ batch_type A = (globalstatsbuffer7[p].value + (VECTOR_SIZE-1)) / VECTOR_SIZE; batch_type B = (A + (SRCBUFFER_SIZE-1)) / SRCBUFFER_SIZE; if(B < 80){ B = B * 2; } batch_type C = ((4 * 4 * 2) * NUM_PARTITIONS) + VECTOR_SIZE; skipsizes7[p] = (B * C) + 128; } //'128' is safety padd // FIXME. REMOVEME.
			...
			#ifdef _DEBUGMODE_CHECKS2
			resetvalues(BIGKV4, NUM_PARTITIONS);
			for(partition_type p=0; p<NUM_PARTITIONS; p++){ BIGKV4[p].value = globalstatsbuffer4[p].value + skipsizes4[p]; }
			#endif
			calculateoffsets(globalstatsbuffer4, NUM_PARTITIONS, destoffset4, skipsizes4);
			resetvalues(globalstatsbuffer4, NUM_PARTITIONS);
			saveglobalstats04(config4.enablecollectglobalstats, kvdram4, globalstatsbuffer4, BASEOFFSET_STATSDRAM_KVS + deststatsmarker4);
			#ifdef _DEBUGMODE_CHECKS2
			resetvalues(BIGKV5, NUM_PARTITIONS);
			for(partition_type p=0; p<NUM_PARTITIONS; p++){ BIGKV5[p].value = globalstatsbuffer5[p].value + skipsizes5[p]; }
			#endif
			calculateoffsets(globalstatsbuffer5, NUM_PARTITIONS, destoffset5, skipsizes5);
			resetvalues(globalstatsbuffer5, NUM_PARTITIONS);
			saveglobalstats05(config5.enablecollectglobalstats, kvdram5, globalstatsbuffer5, BASEOFFSET_STATSDRAM_KVS + deststatsmarker5);
			#ifdef _DEBUGMODE_CHECKS2
			resetvalues(BIGKV6, NUM_PARTITIONS);
			for(partition_type p=0; p<NUM_PARTITIONS; p++){ BIGKV6[p].value = globalstatsbuffer6[p].value + skipsizes6[p]; }
			#endif
			calculateoffsets(globalstatsbuffer6, NUM_PARTITIONS, destoffset6, skipsizes6);
			resetvalues(globalstatsbuffer6, NUM_PARTITIONS);
			saveglobalstats06(config6.enablecollectglobalstats, kvdram6, globalstatsbuffer6, BASEOFFSET_STATSDRAM_KVS + deststatsmarker6);
			#ifdef _DEBUGMODE_CHECKS2
			resetvalues(BIGKV7, NUM_PARTITIONS);
			for(partition_type p=0; p<NUM_PARTITIONS; p++){ BIGKV7[p].value = globalstatsbuffer7[p].value + skipsizes7[p]; }
			#endif
			calculateoffsets(globalstatsbuffer7, NUM_PARTITIONS, destoffset7, skipsizes7);
			resetvalues(globalstatsbuffer7, NUM_PARTITIONS);
			saveglobalstats07(config7.enablecollectglobalstats, kvdram7, globalstatsbuffer7, BASEOFFSET_STATSDRAM_KVS + deststatsmarker7);
			#endif 
			...
			resetvalues(globalstatsbuffer3, NUM_PARTITIONS);
			if((currentLOP >= 1) && (currentLOP <= globalparams4.treedepth) && (Ptravstate4.size_kvs > 0)){ config4.enableprocessedges = OFF; config4.enablecollectglobalstats = OFF; config4.enablepartition = ON; config4.enablereduce = OFF; } 
			else { Ptravstate4.begin_kvs = 0; Ptravstate4.end_kvs = 0; config4.enablepartition = OFF; }
			travstate_t Ptravstate4pp0 = Ptravstate4;
			travstate_t Ptravstate4pp1 = Ptravstate4;
			travstate_t Ptravstate4pp2 = Ptravstate4;
			#ifdef _DEBUGMODE_KERNELPRINTS2
			if((currentLOP >= 1) && (currentLOP <= globalparams4.treedepth)){ actsutilityobj->print7("### maxdispatch0::partition:: source_p", "upperlimit", "begin", "end", "size", "dest range", "currentLOP", sweepparams4.source_partition, sweepparams4.upperlimit, Ptravstate4.begin_kvs * VECTOR_SIZE, Ptravstate4.end_kvs * VECTOR_SIZE, Ptravstate4.size_kvs * VECTOR_SIZE, BATCH_RANGE / (1 << (NUM_PARTITIONS_POW * sweepparams4.currentLOP)), sweepparams4.currentLOP); }	
			#endif
			readglobalstats04(config4.enablepartition, kvdram4, globalstatsbuffer4, BASEOFFSET_STATSDRAM_KVS + deststatsmarker4);
			resetvalues(globalstatsbuffer4, NUM_PARTITIONS);
			if((currentLOP >= 1) && (currentLOP <= globalparams5.treedepth) && (Ptravstate5.size_kvs > 0)){ config5.enableprocessedges = OFF; config5.enablecollectglobalstats = OFF; config5.enablepartition = ON; config5.enablereduce = OFF; } 
			else { Ptravstate5.begin_kvs = 0; Ptravstate5.end_kvs = 0; config5.enablepartition = OFF; }
			travstate_t Ptravstate5pp0 = Ptravstate5;
			travstate_t Ptravstate5pp1 = Ptravstate5;
			travstate_t Ptravstate5pp2 = Ptravstate5;
			#ifdef _DEBUGMODE_KERNELPRINTS2
			if((currentLOP >= 1) && (currentLOP <= globalparams5.treedepth)){ actsutilityobj->print7("### maxdispatch0::partition:: source_p", "upperlimit", "begin", "end", "size", "dest range", "currentLOP", sweepparams5.source_partition, sweepparams5.upperlimit, Ptravstate5.begin_kvs * VECTOR_SIZE, Ptravstate5.end_kvs * VECTOR_SIZE, Ptravstate5.size_kvs * VECTOR_SIZE, BATCH_RANGE / (1 << (NUM_PARTITIONS_POW * sweepparams5.currentLOP)), sweepparams5.currentLOP); }	
			#endif
			readglobalstats05(config5.enablepartition, kvdram5, globalstatsbuffer5, BASEOFFSET_STATSDRAM_KVS + deststatsmarker5);
			resetvalues(globalstatsbuffer5, NUM_PARTITIONS);
			if((currentLOP >= 1) && (currentLOP <= globalparams6.treedepth) && (Ptravstate6.size_kvs > 0)){ config6.enableprocessedges = OFF; config6.enablecollectglobalstats = OFF; config6.enablepartition = ON; config6.enablereduce = OFF; } 
			else { Ptravstate6.begin_kvs = 0; Ptravstate6.end_kvs = 0; config6.enablepartition = OFF; }
			travstate_t Ptravstate6pp0 = Ptravstate6;
			travstate_t Ptravstate6pp1 = Ptravstate6;
			travstate_t Ptravstate6pp2 = Ptravstate6;
			#ifdef _DEBUGMODE_KERNELPRINTS2
			if((currentLOP >= 1) && (currentLOP <= globalparams6.treedepth)){ actsutilityobj->print7("### maxdispatch0::partition:: source_p", "upperlimit", "begin", "end", "size", "dest range", "currentLOP", sweepparams6.source_partition, sweepparams6.upperlimit, Ptravstate6.begin_kvs * VECTOR_SIZE, Ptravstate6.end_kvs * VECTOR_SIZE, Ptravstate6.size_kvs * VECTOR_SIZE, BATCH_RANGE / (1 << (NUM_PARTITIONS_POW * sweepparams6.currentLOP)), sweepparams6.currentLOP); }	
			#endif
			readglobalstats06(config6.enablepartition, kvdram6, globalstatsbuffer6, BASEOFFSET_STATSDRAM_KVS + deststatsmarker6);
			resetvalues(globalstatsbuffer6, NUM_PARTITIONS);
			if((currentLOP >= 1) && (currentLOP <= globalparams7.treedepth) && (Ptravstate7.size_kvs > 0)){ config7.enableprocessedges = OFF; config7.enablecollectglobalstats = OFF; config7.enablepartition = ON; config7.enablereduce = OFF; } 
			else { Ptravstate7.begin_kvs = 0; Ptravstate7.end_kvs = 0; config7.enablepartition = OFF; }
			travstate_t Ptravstate7pp0 = Ptravstate7;
			travstate_t Ptravstate7pp1 = Ptravstate7;
			travstate_t Ptravstate7pp2 = Ptravstate7;
			#ifdef _DEBUGMODE_KERNELPRINTS2
			if((currentLOP >= 1) && (currentLOP <= globalparams7.treedepth)){ actsutilityobj->print7("### maxdispatch0::partition:: source_p", "upperlimit", "begin", "end", "size", "dest range", "currentLOP", sweepparams7.source_partition, sweepparams7.upperlimit, Ptravstate7.begin_kvs * VECTOR_SIZE, Ptravstate7.end_kvs * VECTOR_SIZE, Ptravstate7.size_kvs * VECTOR_SIZE, BATCH_RANGE / (1 << (NUM_PARTITIONS_POW * sweepparams7.currentLOP)), sweepparams7.currentLOP); }	
			#endif
			readglobalstats07(config7.enablepartition, kvdram7, globalstatsbuffer7, BASEOFFSET_STATSDRAM_KVS + deststatsmarker7);
			resetvalues(globalstatsbuffer7, NUM_PARTITIONS);
			...
			#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_partitionloop avg=analysis_partitionloop
				#ifdef _DEBUGMODE_KERNELPRINTS
				actsutilityobj->print4("### maxdispatch0::partition:: offset_kvs", "begin_kvs", "end_kvs", "skip", offset_kvs, Ptravstate.begin_kvs, Ptravstate.end_kvs, SRCBUFFER_SIZE);
				#endif
				...
				Ptravstate4.i_kvs = offset_kvs;
				Ptravstate4pp0.i_kvs = offset_kvs;
				Ptravstate4pp1.i_kvs = offset_kvs + SRCBUFFER_SIZE;
				Ptravstate4pp2.i_kvs = offset_kvs + (2 * SRCBUFFER_SIZE);
				unsigned int en4; if(offset_kvs == Ptravstate4.begin_kvs){ en4 = OFF; } else { en4 = ON; }
				Ptravstate5.i_kvs = offset_kvs;
				Ptravstate5pp0.i_kvs = offset_kvs;
				Ptravstate5pp1.i_kvs = offset_kvs + SRCBUFFER_SIZE;
				Ptravstate5pp2.i_kvs = offset_kvs + (2 * SRCBUFFER_SIZE);
				unsigned int en5; if(offset_kvs == Ptravstate5.begin_kvs){ en5 = OFF; } else { en5 = ON; }
				Ptravstate6.i_kvs = offset_kvs;
				Ptravstate6pp0.i_kvs = offset_kvs;
				Ptravstate6pp1.i_kvs = offset_kvs + SRCBUFFER_SIZE;
				Ptravstate6pp2.i_kvs = offset_kvs + (2 * SRCBUFFER_SIZE);
				unsigned int en6; if(offset_kvs == Ptravstate6.begin_kvs){ en6 = OFF; } else { en6 = ON; }
				Ptravstate7.i_kvs = offset_kvs;
				Ptravstate7pp0.i_kvs = offset_kvs;
				Ptravstate7pp1.i_kvs = offset_kvs + SRCBUFFER_SIZE;
				Ptravstate7pp2.i_kvs = offset_kvs + (2 * SRCBUFFER_SIZE);
				unsigned int en7; if(offset_kvs == Ptravstate7.begin_kvs){ en7 = OFF; } else { en7 = ON; }
				...
				#endif
				readkeyvalues04(ON, kvdram4, sourcebuffer4, (sweepparams4.worksourcebaseaddress_kvs + offset_kvs), Ptravstate4pp0);
				#ifdef PP1 // pipeline overflow from bottom
				savekeyvalues04(en4, kvdram4, buffer4_setof8, globalstatsbuffer4, templocalcapsule4_so8, sweepparams4.workdestbaseaddress_kvs, globalparams4);
				#endif
				#ifdef PP2 // pipeline overflow from bottom
				combineSetof4stoSetof8s04(en4, buffer4_setof4, buffer4_setof8, templocalcapsule4_so4, templocalcapsule4_so8, globalparams4);
				#endif
				readkeyvalues05(ON, kvdram5, sourcebuffer5, (sweepparams5.worksourcebaseaddress_kvs + offset_kvs), Ptravstate5pp0);
				#ifdef PP1 // pipeline overflow from bottom
				savekeyvalues05(en5, kvdram5, buffer5_setof8, globalstatsbuffer5, templocalcapsule5_so8, sweepparams5.workdestbaseaddress_kvs, globalparams5);
				#endif
				#ifdef PP2 // pipeline overflow from bottom
				combineSetof4stoSetof8s05(en5, buffer5_setof4, buffer5_setof8, templocalcapsule5_so4, templocalcapsule5_so8, globalparams5);
				#endif
				readkeyvalues06(ON, kvdram6, sourcebuffer6, (sweepparams6.worksourcebaseaddress_kvs + offset_kvs), Ptravstate6pp0);
				#ifdef PP1 // pipeline overflow from bottom
				savekeyvalues06(en6, kvdram6, buffer6_setof8, globalstatsbuffer6, templocalcapsule6_so8, sweepparams6.workdestbaseaddress_kvs, globalparams6);
				#endif
				#ifdef PP2 // pipeline overflow from bottom
				combineSetof4stoSetof8s06(en6, buffer6_setof4, buffer6_setof8, templocalcapsule6_so4, templocalcapsule6_so8, globalparams6);
				#endif
				readkeyvalues07(ON, kvdram7, sourcebuffer7, (sweepparams7.worksourcebaseaddress_kvs + offset_kvs), Ptravstate7pp0);
				#ifdef PP1 // pipeline overflow from bottom
				savekeyvalues07(en7, kvdram7, buffer7_setof8, globalstatsbuffer7, templocalcapsule7_so8, sweepparams7.workdestbaseaddress_kvs, globalparams7);
				#endif
				#ifdef PP2 // pipeline overflow from bottom
				combineSetof4stoSetof8s07(en7, buffer7_setof4, buffer7_setof8, templocalcapsule7_so4, templocalcapsule7_so8, globalparams7);
				#endif
				...
				savekeyvalues03(en3, kvdram3, buffer3_setof8, globalstatsbuffer3, templocalcapsule3_so8, sweepparams3.workdestbaseaddress_kvs, globalparams3);
				#endif
				partitionkeyvalues04(ON, sourcebuffer4, buffer4_setof1, templocalcapsule4_so1, sweepparams4.currentLOP, sweepparams4.upperlimit, Ptravstate4pp0, globalparams4);
				#ifdef PP1
				readkeyvalues04(ON, kvdram4, sourcebuffer4, (sweepparams4.worksourcebaseaddress_kvs + offset_kvs + SRCBUFFER_SIZE), Ptravstate4pp1);
				#endif
				#ifdef PP2 // pipeline overflow from bottom
				savekeyvalues04(en4, kvdram4, buffer4_setof8, globalstatsbuffer4, templocalcapsule4_so8, sweepparams4.workdestbaseaddress_kvs, globalparams4);
				#endif
				partitionkeyvalues05(ON, sourcebuffer5, buffer5_setof1, templocalcapsule5_so1, sweepparams5.currentLOP, sweepparams5.upperlimit, Ptravstate5pp0, globalparams5);
				#ifdef PP1
				readkeyvalues05(ON, kvdram5, sourcebuffer5, (sweepparams5.worksourcebaseaddress_kvs + offset_kvs + SRCBUFFER_SIZE), Ptravstate5pp1);
				#endif
				#ifdef PP2 // pipeline overflow from bottom
				savekeyvalues05(en5, kvdram5, buffer5_setof8, globalstatsbuffer5, templocalcapsule5_so8, sweepparams5.workdestbaseaddress_kvs, globalparams5);
				#endif
				partitionkeyvalues06(ON, sourcebuffer6, buffer6_setof1, templocalcapsule6_so1, sweepparams6.currentLOP, sweepparams6.upperlimit, Ptravstate6pp0, globalparams6);
				#ifdef PP1
				readkeyvalues06(ON, kvdram6, sourcebuffer6, (sweepparams6.worksourcebaseaddress_kvs + offset_kvs + SRCBUFFER_SIZE), Ptravstate6pp1);
				#endif
				#ifdef PP2 // pipeline overflow from bottom
				savekeyvalues06(en6, kvdram6, buffer6_setof8, globalstatsbuffer6, templocalcapsule6_so8, sweepparams6.workdestbaseaddress_kvs, globalparams6);
				#endif
				partitionkeyvalues07(ON, sourcebuffer7, buffer7_setof1, templocalcapsule7_so1, sweepparams7.currentLOP, sweepparams7.upperlimit, Ptravstate7pp0, globalparams7);
				#ifdef PP1
				readkeyvalues07(ON, kvdram7, sourcebuffer7, (sweepparams7.worksourcebaseaddress_kvs + offset_kvs + SRCBUFFER_SIZE), Ptravstate7pp1);
				#endif
				#ifdef PP2 // pipeline overflow from bottom
				savekeyvalues07(en7, kvdram7, buffer7_setof8, globalstatsbuffer7, templocalcapsule7_so8, sweepparams7.workdestbaseaddress_kvs, globalparams7);
				#endif
				...
				#endif
				combineSetof1stoSetof2s04(ON, buffer4_setof1, buffer4_setof2, templocalcapsule4_so1, templocalcapsule4_so2, globalparams4);
				#ifdef PP1
				partitionkeyvalues04(ON, sourcebuffer4, buffer4_setof1, templocalcapsule4_so1, sweepparams4.currentLOP, sweepparams4.upperlimit, Ptravstate4pp1, globalparams4);
				#endif
				#ifdef PP2
				readkeyvalues04(ON, kvdram4, sourcebuffer4, (sweepparams4.worksourcebaseaddress_kvs + offset_kvs + SRCBUFFER_SIZE + SRCBUFFER_SIZE), Ptravstate4pp2);
				#endif
				combineSetof1stoSetof2s05(ON, buffer5_setof1, buffer5_setof2, templocalcapsule5_so1, templocalcapsule5_so2, globalparams5);
				#ifdef PP1
				partitionkeyvalues05(ON, sourcebuffer5, buffer5_setof1, templocalcapsule5_so1, sweepparams5.currentLOP, sweepparams5.upperlimit, Ptravstate5pp1, globalparams5);
				#endif
				#ifdef PP2
				readkeyvalues05(ON, kvdram5, sourcebuffer5, (sweepparams5.worksourcebaseaddress_kvs + offset_kvs + SRCBUFFER_SIZE + SRCBUFFER_SIZE), Ptravstate5pp2);
				#endif
				combineSetof1stoSetof2s06(ON, buffer6_setof1, buffer6_setof2, templocalcapsule6_so1, templocalcapsule6_so2, globalparams6);
				#ifdef PP1
				partitionkeyvalues06(ON, sourcebuffer6, buffer6_setof1, templocalcapsule6_so1, sweepparams6.currentLOP, sweepparams6.upperlimit, Ptravstate6pp1, globalparams6);
				#endif
				#ifdef PP2
				readkeyvalues06(ON, kvdram6, sourcebuffer6, (sweepparams6.worksourcebaseaddress_kvs + offset_kvs + SRCBUFFER_SIZE + SRCBUFFER_SIZE), Ptravstate6pp2);
				#endif
				combineSetof1stoSetof2s07(ON, buffer7_setof1, buffer7_setof2, templocalcapsule7_so1, templocalcapsule7_so2, globalparams7);
				#ifdef PP1
				partitionkeyvalues07(ON, sourcebuffer7, buffer7_setof1, templocalcapsule7_so1, sweepparams7.currentLOP, sweepparams7.upperlimit, Ptravstate7pp1, globalparams7);
				#endif
				#ifdef PP2
				readkeyvalues07(ON, kvdram7, sourcebuffer7, (sweepparams7.worksourcebaseaddress_kvs + offset_kvs + SRCBUFFER_SIZE + SRCBUFFER_SIZE), Ptravstate7pp2);
				#endif
				...
				#endif
				combineSetof2stoSetof4s04(ON, buffer4_setof2, buffer4_setof4, templocalcapsule4_so2, templocalcapsule4_so4, globalparams4);
				#ifdef PP1
				combineSetof1stoSetof2s04(ON, buffer4_setof1, buffer4_setof2, templocalcapsule4_so1, templocalcapsule4_so2, globalparams4);
				#endif
				#ifdef PP2
				partitionkeyvalues04(ON, sourcebuffer4, buffer4_setof1, templocalcapsule4_so1, sweepparams4.currentLOP, sweepparams4.upperlimit, Ptravstate4pp2, globalparams4);
				#endif
				combineSetof2stoSetof4s05(ON, buffer5_setof2, buffer5_setof4, templocalcapsule5_so2, templocalcapsule5_so4, globalparams5);
				#ifdef PP1
				combineSetof1stoSetof2s05(ON, buffer5_setof1, buffer5_setof2, templocalcapsule5_so1, templocalcapsule5_so2, globalparams5);
				#endif
				#ifdef PP2
				partitionkeyvalues05(ON, sourcebuffer5, buffer5_setof1, templocalcapsule5_so1, sweepparams5.currentLOP, sweepparams5.upperlimit, Ptravstate5pp2, globalparams5);
				#endif
				combineSetof2stoSetof4s06(ON, buffer6_setof2, buffer6_setof4, templocalcapsule6_so2, templocalcapsule6_so4, globalparams6);
				#ifdef PP1
				combineSetof1stoSetof2s06(ON, buffer6_setof1, buffer6_setof2, templocalcapsule6_so1, templocalcapsule6_so2, globalparams6);
				#endif
				#ifdef PP2
				partitionkeyvalues06(ON, sourcebuffer6, buffer6_setof1, templocalcapsule6_so1, sweepparams6.currentLOP, sweepparams6.upperlimit, Ptravstate6pp2, globalparams6);
				#endif
				combineSetof2stoSetof4s07(ON, buffer7_setof2, buffer7_setof4, templocalcapsule7_so2, templocalcapsule7_so4, globalparams7);
				#ifdef PP1
				combineSetof1stoSetof2s07(ON, buffer7_setof1, buffer7_setof2, templocalcapsule7_so1, templocalcapsule7_so2, globalparams7);
				#endif
				#ifdef PP2
				partitionkeyvalues07(ON, sourcebuffer7, buffer7_setof1, templocalcapsule7_so1, sweepparams7.currentLOP, sweepparams7.upperlimit, Ptravstate7pp2, globalparams7);
				#endif
				...
				combineSetof1stoSetof2s03(ON, buffer3_setof1, buffer3_setof2, templocalcapsule3_so1, templocalcapsule3_so2, globalparams3);
				#endif
				combineSetof4stoSetof8s04(ON, buffer4_setof4, buffer4_setof8, templocalcapsule4_so4, templocalcapsule4_so8, globalparams4);
				#ifdef PP1
				combineSetof2stoSetof4s04(ON, buffer4_setof2, buffer4_setof4, templocalcapsule4_so2, templocalcapsule4_so4, globalparams4);
				#endif
				#ifdef PP2
				combineSetof1stoSetof2s04(ON, buffer4_setof1, buffer4_setof2, templocalcapsule4_so1, templocalcapsule4_so2, globalparams4);
				#endif
				combineSetof4stoSetof8s05(ON, buffer5_setof4, buffer5_setof8, templocalcapsule5_so4, templocalcapsule5_so8, globalparams5);
				#ifdef PP1
				combineSetof2stoSetof4s05(ON, buffer5_setof2, buffer5_setof4, templocalcapsule5_so2, templocalcapsule5_so4, globalparams5);
				#endif
				#ifdef PP2
				combineSetof1stoSetof2s05(ON, buffer5_setof1, buffer5_setof2, templocalcapsule5_so1, templocalcapsule5_so2, globalparams5);
				#endif
				combineSetof4stoSetof8s06(ON, buffer6_setof4, buffer6_setof8, templocalcapsule6_so4, templocalcapsule6_so8, globalparams6);
				#ifdef PP1
				combineSetof2stoSetof4s06(ON, buffer6_setof2, buffer6_setof4, templocalcapsule6_so2, templocalcapsule6_so4, globalparams6);
				#endif
				#ifdef PP2
				combineSetof1stoSetof2s06(ON, buffer6_setof1, buffer6_setof2, templocalcapsule6_so1, templocalcapsule6_so2, globalparams6);
				#endif
				combineSetof4stoSetof8s07(ON, buffer7_setof4, buffer7_setof8, templocalcapsule7_so4, templocalcapsule7_so8, globalparams7);
				#ifdef PP1
				combineSetof2stoSetof4s07(ON, buffer7_setof2, buffer7_setof4, templocalcapsule7_so2, templocalcapsule7_so4, globalparams7);
				#endif
				#ifdef PP2
				...
				combineSetof2stoSetof4s03(ON, buffer3_setof2, buffer3_setof4, templocalcapsule3_so2, templocalcapsule3_so4, globalparams3);
				#endif
				savekeyvalues04(ON, kvdram4, buffer4_setof8, globalstatsbuffer4, templocalcapsule4_so8, sweepparams4.workdestbaseaddress_kvs, globalparams4);
				#ifdef PP1
				combineSetof4stoSetof8s04(ON, buffer4_setof4, buffer4_setof8, templocalcapsule4_so4, templocalcapsule4_so8, globalparams4);
				#endif
				#ifdef PP2
				combineSetof2stoSetof4s04(ON, buffer4_setof2, buffer4_setof4, templocalcapsule4_so2, templocalcapsule4_so4, globalparams4);
				#endif
				savekeyvalues05(ON, kvdram5, buffer5_setof8, globalstatsbuffer5, templocalcapsule5_so8, sweepparams5.workdestbaseaddress_kvs, globalparams5);
				#ifdef PP1
				combineSetof4stoSetof8s05(ON, buffer5_setof4, buffer5_setof8, templocalcapsule5_so4, templocalcapsule5_so8, globalparams5);
				#endif
				#ifdef PP2
				combineSetof2stoSetof4s05(ON, buffer5_setof2, buffer5_setof4, templocalcapsule5_so2, templocalcapsule5_so4, globalparams5);
				#endif
				savekeyvalues06(ON, kvdram6, buffer6_setof8, globalstatsbuffer6, templocalcapsule6_so8, sweepparams6.workdestbaseaddress_kvs, globalparams6);
				#ifdef PP1
				combineSetof4stoSetof8s06(ON, buffer6_setof4, buffer6_setof8, templocalcapsule6_so4, templocalcapsule6_so8, globalparams6);
				#endif
				#ifdef PP2
				combineSetof2stoSetof4s06(ON, buffer6_setof2, buffer6_setof4, templocalcapsule6_so2, templocalcapsule6_so4, globalparams6);
				#endif
				savekeyvalues07(ON, kvdram7, buffer7_setof8, globalstatsbuffer7, templocalcapsule7_so8, sweepparams7.workdestbaseaddress_kvs, globalparams7);
				#ifdef PP1
				combineSetof4stoSetof8s07(ON, buffer7_setof4, buffer7_setof8, templocalcapsule7_so4, templocalcapsule7_so8, globalparams7);
				#endif
				#ifdef PP2
			...
			#endif
			#ifdef PP1 // pipeline overflow from bottom
			savekeyvalues04(config4.enablepartition, kvdram4, buffer4_setof8, globalstatsbuffer4, templocalcapsule4_so8, sweepparams4.workdestbaseaddress_kvs, globalparams4);
			#endif
			#ifdef PP2 // pipeline overflow from bottom
			combineSetof4stoSetof8s04(config4.enablepartition, buffer4_setof4, buffer4_setof8, templocalcapsule4_so4, templocalcapsule4_so8, globalparams4);
			#endif
			#ifdef PP1 // pipeline overflow from bottom
			savekeyvalues05(config5.enablepartition, kvdram5, buffer5_setof8, globalstatsbuffer5, templocalcapsule5_so8, sweepparams5.workdestbaseaddress_kvs, globalparams5);
			#endif
			#ifdef PP2 // pipeline overflow from bottom
			combineSetof4stoSetof8s05(config5.enablepartition, buffer5_setof4, buffer5_setof8, templocalcapsule5_so4, templocalcapsule5_so8, globalparams5);
			#endif
			#ifdef PP1 // pipeline overflow from bottom
			savekeyvalues06(config6.enablepartition, kvdram6, buffer6_setof8, globalstatsbuffer6, templocalcapsule6_so8, sweepparams6.workdestbaseaddress_kvs, globalparams6);
			#endif
			#ifdef PP2 // pipeline overflow from bottom
			combineSetof4stoSetof8s06(config6.enablepartition, buffer6_setof4, buffer6_setof8, templocalcapsule6_so4, templocalcapsule6_so8, globalparams6);
			#endif
			#ifdef PP1 // pipeline overflow from bottom
			savekeyvalues07(config7.enablepartition, kvdram7, buffer7_setof8, globalstatsbuffer7, templocalcapsule7_so8, sweepparams7.workdestbaseaddress_kvs, globalparams7);
			#endif
			#ifdef PP2 // pipeline overflow from bottom
			combineSetof4stoSetof8s07(config7.enablepartition, buffer7_setof4, buffer7_setof8, templocalcapsule7_so4, templocalcapsule7_so8, globalparams7);
			#endif
			...
			#endif
			saveglobalstats04(config4.enablepartition, kvdram4, globalstatsbuffer4, BASEOFFSET_STATSDRAM_KVS + deststatsmarker4);
			#if defined(_DEBUGMODE_CHECKS2) && defined(ENABLE_PERFECTACCURACY)
			if(config4.enablereduce == OFF){ actsutilityobj->checkforgreaterthan("maxdispatch. comparing BIGKV4 & globalstatsbuffer4", BIGKV, globalstatsbuffer4, NUM_PARTITIONS); }
			#endif
			saveglobalstats05(config5.enablepartition, kvdram5, globalstatsbuffer5, BASEOFFSET_STATSDRAM_KVS + deststatsmarker5);
			#if defined(_DEBUGMODE_CHECKS2) && defined(ENABLE_PERFECTACCURACY)
			if(config5.enablereduce == OFF){ actsutilityobj->checkforgreaterthan("maxdispatch. comparing BIGKV5 & globalstatsbuffer5", BIGKV, globalstatsbuffer5, NUM_PARTITIONS); }
			#endif
			saveglobalstats06(config6.enablepartition, kvdram6, globalstatsbuffer6, BASEOFFSET_STATSDRAM_KVS + deststatsmarker6);
			#if defined(_DEBUGMODE_CHECKS2) && defined(ENABLE_PERFECTACCURACY)
			if(config6.enablereduce == OFF){ actsutilityobj->checkforgreaterthan("maxdispatch. comparing BIGKV6 & globalstatsbuffer6", BIGKV, globalstatsbuffer6, NUM_PARTITIONS); }
			#endif
			saveglobalstats07(config7.enablepartition, kvdram7, globalstatsbuffer7, BASEOFFSET_STATSDRAM_KVS + deststatsmarker7);
			#if defined(_DEBUGMODE_CHECKS2) && defined(ENABLE_PERFECTACCURACY)
			if(config7.enablereduce == OFF){ actsutilityobj->checkforgreaterthan("maxdispatch. comparing BIGKV7 & globalstatsbuffer7", BIGKV, globalstatsbuffer7, NUM_PARTITIONS); }
			#endif
			#endif
			...
			if(currentLOP > 0){
				sourcestatsmarker4 += 1;
				deststatsmarker4 += NUM_PARTITIONS;
				destoffset4 = globalstatsbuffer4[NUM_PARTITIONS-1].key + globalstatsbuffer4[NUM_PARTITIONS-1].value + 64; }
			if(currentLOP > 0){
				sourcestatsmarker5 += 1;
				deststatsmarker5 += NUM_PARTITIONS;
				destoffset5 = globalstatsbuffer5[NUM_PARTITIONS-1].key + globalstatsbuffer5[NUM_PARTITIONS-1].value + 64; }
			if(currentLOP > 0){
				sourcestatsmarker6 += 1;
				deststatsmarker6 += NUM_PARTITIONS;
				destoffset6 = globalstatsbuffer6[NUM_PARTITIONS-1].key + globalstatsbuffer6[NUM_PARTITIONS-1].value + 64; }
			if(currentLOP > 0){
				sourcestatsmarker7 += 1;
				deststatsmarker7 += NUM_PARTITIONS;
				destoffset7 = globalstatsbuffer7[NUM_PARTITIONS-1].key + globalstatsbuffer7[NUM_PARTITIONS-1].value + 64; }
			...
			if(config4.enablereduce == OFF){ actsutilityobj->printpartitionresult2(OFF, kvdram4, globalstatsbuffer4, sweepparams4); }
			if(config5.enablereduce == OFF){ actsutilityobj->printpartitionresult2(OFF, kvdram5, globalstatsbuffer5, sweepparams5); }
			if(config6.enablereduce == OFF){ actsutilityobj->printpartitionresult2(OFF, kvdram6, globalstatsbuffer6, sweepparams6); }
			if(config7.enablereduce == OFF){ actsutilityobj->printpartitionresult2(OFF, kvdram7, globalstatsbuffer7, sweepparams7); }
			#endif
		}
	...
	actsutilityobj->countvalueslessthan("maxdispatch", (value_t *)&kvdram4[BASEOFFSET_VERTICESDATA_KVS], BATCH_RANGE, INFINITI);
	actsutilityobj->countvalueslessthan("maxdispatch", (value_t *)&kvdram5[BASEOFFSET_VERTICESDATA_KVS], BATCH_RANGE, INFINITI);
	actsutilityobj->countvalueslessthan("maxdispatch", (value_t *)&kvdram6[BASEOFFSET_VERTICESDATA_KVS], BATCH_RANGE, INFINITI);
	actsutilityobj->countvalueslessthan("maxdispatch", (value_t *)&kvdram7[BASEOFFSET_VERTICESDATA_KVS], BATCH_RANGE, INFINITI);
	#endif 
	return;
}
...
void 
	...
	#endif
topkernel(
  uint512_dt * sourceA0volume  ,uint512_dt * sourceA1volume  ,uint512_dt * sourceA2volume  ,uint512_dt * sourceA3volume  ,uint512_dt * sourceA4volume  ,uint512_dt * sourceA5volume  ,uint512_dt * sourceA6volume  ,uint512_dt * sourceA7volume 	){
...
#pragma HLS INTERFACE m_axi port = sourceA4volume offset = slave bundle = gmem4 // max_read_burst_length=64 max_write_burst_length=64
#pragma HLS INTERFACE m_axi port = sourceA5volume offset = slave bundle = gmem5 // max_read_burst_length=64 max_write_burst_length=64
#pragma HLS INTERFACE m_axi port = sourceA6volume offset = slave bundle = gmem6 // max_read_burst_length=64 max_write_burst_length=64
#pragma HLS INTERFACE m_axi port = sourceA7volume offset = slave bundle = gmem7 // max_read_burst_length=64 max_write_burst_length=64
...
#pragma HLS INTERFACE s_axilite port = sourceA4volume bundle = control
#pragma HLS INTERFACE s_axilite port = sourceA5volume bundle = control
#pragma HLS INTERFACE s_axilite port = sourceA6volume bundle = control
#pragma HLS INTERFACE s_axilite port = sourceA7volume bundle = control
...
#pragma HLS DATA_PACK variable = sourceA4volume
#pragma HLS DATA_PACK variable = sourceA5volume
#pragma HLS DATA_PACK variable = sourceA6volume
#pragma HLS DATA_PACK variable = sourceA7volume
	...
	cout<<">>> Light weight ACTS MAXBUTIL (L2) Launched... size: "<<(unsigned int)(sourceA4volume[BASEOFFSET_MESSAGESDRAM_KVS + MESSAGES_RUNSIZE].range(31, 0))<<endl; 
	cout<<">>> Light weight ACTS MAXBUTIL (L2) Launched... size: "<<(unsigned int)(sourceA5volume[BASEOFFSET_MESSAGESDRAM_KVS + MESSAGES_RUNSIZE].range(31, 0))<<endl; 
	cout<<">>> Light weight ACTS MAXBUTIL (L2) Launched... size: "<<(unsigned int)(sourceA6volume[BASEOFFSET_MESSAGESDRAM_KVS + MESSAGES_RUNSIZE].range(31, 0))<<endl; 
	cout<<">>> Light weight ACTS MAXBUTIL (L2) Launched... size: "<<(unsigned int)(sourceA7volume[BASEOFFSET_MESSAGESDRAM_KVS + MESSAGES_RUNSIZE].range(31, 0))<<endl; 
	#else
	...
	cout<<">>> Light weight ACTS MAXBUTIL (L2) Launched... size: "<<sourceA4volume[BASEOFFSET_MESSAGESDRAM_KVS + MESSAGES_RUNSIZE].data[0].key<<endl; 
	cout<<">>> Light weight ACTS MAXBUTIL (L2) Launched... size: "<<sourceA5volume[BASEOFFSET_MESSAGESDRAM_KVS + MESSAGES_RUNSIZE].data[0].key<<endl; 
	cout<<">>> Light weight ACTS MAXBUTIL (L2) Launched... size: "<<sourceA6volume[BASEOFFSET_MESSAGESDRAM_KVS + MESSAGES_RUNSIZE].data[0].key<<endl; 
	cout<<">>> Light weight ACTS MAXBUTIL (L2) Launched... size: "<<sourceA7volume[BASEOFFSET_MESSAGESDRAM_KVS + MESSAGES_RUNSIZE].data[0].key<<endl; 
	#endif
	#endif
	
	maxdispatch0( sourceA0volume  ,sourceA1volume  ,sourceA2volume  ,sourceA3volume  ,sourceA4volume  ,sourceA5volume  ,sourceA6volume  ,sourceA7volume );
	return;
}
}
