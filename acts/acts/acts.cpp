...
void
	...
	acts::
	#endif
synchronize(bool_type enable, keyvalue_vbuffer_t buffer0[VBUFFER_VECTOR_SIZE][BLOCKRAM_SIZE],keyvalue_vbuffer_t buffer1[VBUFFER_VECTOR_SIZE][BLOCKRAM_SIZE], keyvalue_vbuffer_t tail[VBUFFER_VECTOR_SIZE][BLOCKRAM_SIZE], globalparams_t globalparams){
	if(enable == OFF){ return; }
	#ifdef _DEBUGMODE_KERNELPRINTS_TRACE
	...
	#pragma HLS PIPELINE II=1
		...
		tail[0][i] = GETKV2(keyvalue0_vault2);
		tail[1][i] = GETKV2(keyvalue1_vault2);
		tail[2][i] = GETKV2(keyvalue2_vault2);
		tail[3][i] = GETKV2(keyvalue3_vault2);
		tail[4][i] = GETKV2(keyvalue4_vault2);
		tail[5][i] = GETKV2(keyvalue5_vault2);
		tail[6][i] = GETKV2(keyvalue6_vault2);
		tail[7][i] = GETKV2(keyvalue7_vault2);
		tail[8][i] = GETKV2(keyvalue8_vault2);
		tail[9][i] = GETKV2(keyvalue9_vault2);
		tail[10][i] = GETKV2(keyvalue10_vault2);
		tail[11][i] = GETKV2(keyvalue11_vault2);
		tail[12][i] = GETKV2(keyvalue12_vault2);
		tail[13][i] = GETKV2(keyvalue13_vault2);
		tail[14][i] = GETKV2(keyvalue14_vault2);
		tail[15][i] = GETKV2(keyvalue15_vault2);
		#ifdef _DEBUGMODE_KERNELPRINTS
...
void
	...
	acts::
	#endif
synchronize2(bool_type enable, keyvalue_vbuffer_t buffer0[VBUFFER_VECTOR_SIZE][BLOCKRAM_SIZE],keyvalue_vbuffer_t buffer1[VBUFFER_VECTOR_SIZE][BLOCKRAM_SIZE], keyvalue_vbuffer_t tail[VBUFFER_VECTOR_SIZE][BLOCKRAM_SIZE], globalparams_t globalparams){
	if(enable == OFF){ return; }
	#ifdef _DEBUGMODE_KERNELPRINTS_TRACE
	cout<<"synchronize: synchronize function called."<<endl;
	#endif 
...
void
	...
	acts::
	#endif
spreadvdata(bool_type enable, keyvalue_vbuffer_t buffer0[VBUFFER_VECTOR_SIZE][BLOCKRAM_SIZE],keyvalue_vbuffer_t buffer1[VBUFFER_VECTOR_SIZE][BLOCKRAM_SIZE], keyvalue_vbuffer_t tail[VBUFFER_VECTOR_SIZE][BLOCKRAM_SIZE], globalparams_t globalparams){
	if(enable == OFF){ return; }
	#ifdef _DEBUGMODE_KERNELPRINTS_TRACE
...
void
	...
	acts::
	#endif
spreadvdata2(bool_type enable, keyvalue_vbuffer_t buffer0[VBUFFER_VECTOR_SIZE][BLOCKRAM_SIZE],keyvalue_vbuffer_t buffer1[VBUFFER_VECTOR_SIZE][BLOCKRAM_SIZE], keyvalue_vbuffer_t tail[VBUFFER_VECTOR_SIZE][BLOCKRAM_SIZE], globalparams_t globalparams){
	if(enable == OFF){ return; }
	#ifdef _DEBUGMODE_KERNELPRINTS_TRACE
	cout<<"spreadvdata: spread (vdata) function called."<<endl;
	#endif 
...
void
	...
	acts::
	#endif
spreadvmask(bool_type enable, uintNUMPby2_type vmask[BLOCKRAM_SIZE], uintNUMPby2_type vmask0[BLOCKRAM_SIZE], globalparams_t globalparams){
	if(enable == OFF){ return; }
	#ifdef _DEBUGMODE_KERNELPRINTS_TRACE
...
keyvalue_buffer_t sourcebuffer0[VECTOR_SIZE][BLOCKRAM_SIZE], keyvalue_vbuffer_t vbuffer0[VBUFFER_VECTOR_SIZE][BLOCKRAM_SIZE], uintNUMPby2_type vmask0[BLOCKRAM_SIZE], uint32_type vmask_p0[BLOCKRAM_SIZE], 
	...
	#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_loop1 avg=analysis_loop1
		...
		synchronize(enablereduce, synvbuffer_head, vbuffer0, synvbuffer_tail, _globalparams);
		vmask_p[source_partition] = apply(enablereduce, synvbuffer_tail, synvbuffer_head, vmask0, voffset_kvs, _globalparams);
		spreadvdata(enablereduce, synvbuffer_tail, vbuffer0, synvbuffer_head, _globalparams);  // CRITICAL FIXME. vbuffers should be reversed in order to help sdaccel linker
		#ifndef ALLVERTEXISACTIVE_ALGORITHM
		spreadvmask(enablereduce, vmask0, vmask0, _globalparams);
		#endif 
		#else 
		vmask_p[source_partition] = synchronize(enablereduce, synvbuffer_head, vbuffer0, vmask0, _globalparams); 	
		#endif
		#endif
...
#pragma HLS DATA_PACK variable = kvdram0
	...
	RUNITERATIONS_LOOP: for(unsigned int GraphIter=0; GraphIter<numGraphIters; GraphIter++){
		...
		if(globalparams[0].ENABLE_APPLYUPDATESCOMMAND == ON){ 
			#ifdef _DEBUGMODE_KERNELPRINTS2
			cout<<"topkernel: reducing instances 0-1... "<<endl;
			#endif
			dispatch_reduce(kvdram0, 
sourcebuffer0, vbuffer0, vmask0, vmask_p0, 
...
#pragma HLS DATA_PACK variable = kvdram0
	...
	#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_loop1 avg=analysis_loop1
		...
		TOPKERNELSYNC_MAINLOOP1B: for(batch_type it=0; it<2; it+=1){
			...
			// synchronize
			synchronize2(enablereduce, synvbuffer_head, vbuffer0, synvbuffer_tail, _globalparams);
			vmask_p_temp[it] = apply2(enablereduce, synvbuffer_tail, synvbuffer_head, vmask0, it*VECTOR_SIZE, voffset_kvs, _globalparams);
			spreadvdata2(enablereduce, synvbuffer_tail, vbuffer0, synvbuffer_head, _globalparams);  // CRITICAL FIXME. vbuffers should be reversed in order to help sdaccel linker
...
void 
	...
	acts:: 
	#endif
mainkernel(uint512_dt * kvdram0){ // NB: for CPU test only
