...
void
	...
	actsmerge::
	#endif
topkernelmerge(uint512_dt * kvdram0, uint512_dt * kvdram1, uint512_dt * kvdram2, uint512_dt * kvdram3){
...
#pragma HLS DATA_PACK variable = kvdram3
	...
	READANDMERGE_LOOP1: for (buffer_type i=0; i<(globalparams.SIZE_DESTVERTICESDATA / 2); i+=BLOCKRAM_SIZE){
		...
		#pragma HLS PIPELINE II=1
			...
			buffer[dramoffset_kvs + j][0].key = keyC0;
			buffer[dramoffset_kvs + j][0].value = valC0;
			buffer[dramoffset_kvs + j][1].key = keyC1;
			buffer[dramoffset_kvs + j][1].value = valC1;
			buffer[dramoffset_kvs + j][2].key = keyC2;
			buffer[dramoffset_kvs + j][2].value = valC2;
			buffer[dramoffset_kvs + j][3].key = keyC3;
			buffer[dramoffset_kvs + j][3].value = valC3;
			buffer[dramoffset_kvs + j][4].key = keyC4;
			buffer[dramoffset_kvs + j][4].value = valC4;
			buffer[dramoffset_kvs + j][5].key = keyC5;
			buffer[dramoffset_kvs + j][5].value = valC5;
			buffer[dramoffset_kvs + j][6].key = keyC6;
			buffer[dramoffset_kvs + j][6].value = valC6;
			buffer[dramoffset_kvs + j][7].key = keyC7;
			buffer[dramoffset_kvs + j][7].value = valC7;
		}
