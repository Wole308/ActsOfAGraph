void REDUCEP0_reducevector(unsigned int col, keyvalue_buffer_t kvdata, keyvalue_vbuffer_t destbuffer[BLOCKRAM_VDATA_SIZE], buffer_type destoffset, unsigned int upperlimit, sweepparams_t sweepparams, globalposition_t globalposition, globalparams_t globalparams, collection_t collections[COLLECTIONS_BUFFERSZ]){
	...
	if(loc >= (globalparams.SIZEKVS2_REDUCEPARTITION * VDATA_SHRINK_RATIO) && en == true){ // SIZEKVS2_PROCESSEDGESPARTITION, SIZEKVS2_REDUCEPARTITION
	...
	#endif 
	#ifdef ALGORITHMTYPE_REPRESENTVDATASASBITS
		if(en == true){ vmdata = MEMCAP0_READFROMBUFFER_VDATAWITHVMASK2(loc, NAp, destbuffer, bufferoffset_kvs);
			// cout<<"++TTTTTT++ REDUCE VECTOR:: VERTEX-MASK READ: @ bufferoffset_kvs: "<<bufferoffset_kvs<<", loc: "<<loc<<", vmdata.vdata: "<<vmdata.vdata<<", vmdata.vmask: "<<vmdata.vmask<<", destbuffer["<<bufferoffset_kvs + (loc / VDATA_SHRINK_RATIO)<<"]: "<<destbuffer[bufferoffset_kvs + (loc / VDATA_SHRINK_RATIO)]<<endl;
			}
			#else 
	...
	#endif
	// if(en == true){ cout<<")))) REDUCE VECTOR:: ACTIVE MASK SEEN AT: "<<loc<<", vmdata.vdata: "<<vmdata.vdata<<", vmdata.vmask: "<<vmdata.vmask<<", new_vprop: "<<new_vprop<<", new_vprop: "<<new_vprop<<", en: "<<(unsigned int)en<<endl; }

	// write
	#ifdef ALGORITHMTYPE_REPRESENTVDATASASBITS // (globalposition.source_partition % VDATA_SHRINK_RATIO)
		if(en == true){ MEMCAP0_WRITETOBUFFER_VDATAWITHVMASK2(loc, destbuffer, new_vprop, vmdata.vmask, bufferoffset_kvs); }
			#else 
...
void REDUCEP0_priorreduceandbuffer(bool_type enable, keyvalue_buffer_t buffer[VECTOR_SIZE][SOURCEBLOCKRAM_SIZE], keyvalue_capsule_t localcapsule[MAX_NUM_PARTITIONS], keyvalue_vbuffer_t vbuffer[VDATA_PACKINGSIZE][BLOCKRAM_VDATA_SIZE], buffer_type chunk_size, sweepparams_t sweepparams, globalposition_t globalposition, globalparams_t globalparams, collection_t collections[NUM_COLLECTIONS][COLLECTIONS_BUFFERSZ]){				
	if(enable == OFF){ return; }
