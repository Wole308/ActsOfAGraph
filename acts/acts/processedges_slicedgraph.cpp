	...
	#pragma HLS PIPELINE II=1
		...
		// if(lvid_head >= validbound){ lvid_head = 0; } // CRITICAL REMOVEME.
			...
			if(srcvid_head < travstate.i2){ cout<<"readandprocess: INVALID srcvid_head. this is an error. i: "<<i<<"(of "<<chunk_size<<"), srcvid_head: "<<srcvid_head<<", travstate.i2: "<<travstate.i2<<" offset_kvs: "<<offset_kvs<<". exiting..."<<endl;					 
				for(unsigned int v=0; v<VECTOR_SIZE; v++){ cout<<"readandprocess: E["<<v<<"]: "<<E[v]<<endl; }
				exit(EXIT_FAILURE); }
 ...
 // AUTOMATEME. OPTIMIZEME. FIXME.BOTTLENECK
		...
		if(GraphAlgo == PAGERANK){  mask0 = 1;  mask1 = 1;  mask2 = 1;  mask3 = 1;  mask4 = 1;  mask5 = 1;  mask6 = 1;  mask7 = 1;  mask8 = 1;  mask9 = 1;  mask10 = 1;  mask11 = 1;  mask12 = 1;  mask13 = 1;  mask14 = 1;  mask15 = 1;  mask0 = 0; ens0 = OFF; } 
			...
			if(ens0 == ON && mask0 > 2){ 
				...
				for(unsigned int n=0; n<16; n++){ cout<<">>> readandprocess.mask: E["<<n<<"].dstvid: "<<acts_utilobj->UTIL_READFROM_UINT(E[n], 0, 28)<<", E["<<n<<"].incr: "<<acts_utilobj->UTIL_READFROM_UINT(E[0], 28, 4)<<endl; } 
				exit(EXIT_FAILURE); 
			}
			if(ens1 == ON && mask1 > 2){ 
				cout<<"ERROR @ readandprocess.mask1.2. mask1: "<<mask1<<", incr1: "<<incr1<<endl; 
				for(unsigned int n=0; n<MAX_NUM_UNIQ_EDGES_PER_VEC; n++){ cout<<">>> readandprocess.mask: masks["<<n<<"]: "<<masks[n]<<endl; } 
				for(unsigned int n=0; n<16; n++){ cout<<">>> readandprocess.mask: E["<<n<<"].dstvid: "<<acts_utilobj->UTIL_READFROM_UINT(E[n], 0, 28)<<", E["<<n<<"].incr: "<<acts_utilobj->UTIL_READFROM_UINT(E[1], 28, 4)<<endl; } 
				exit(EXIT_FAILURE); 
			}
			if(ens2 == ON && mask2 > 2){ 
				cout<<"ERROR @ readandprocess.mask2.2. mask2: "<<mask2<<", incr2: "<<incr2<<endl; 
				for(unsigned int n=0; n<MAX_NUM_UNIQ_EDGES_PER_VEC; n++){ cout<<">>> readandprocess.mask: masks["<<n<<"]: "<<masks[n]<<endl; } 
				for(unsigned int n=0; n<16; n++){ cout<<">>> readandprocess.mask: E["<<n<<"].dstvid: "<<acts_utilobj->UTIL_READFROM_UINT(E[n], 0, 28)<<", E["<<n<<"].incr: "<<acts_utilobj->UTIL_READFROM_UINT(E[2], 28, 4)<<endl; } 
				exit(EXIT_FAILURE); 
			}
			if(ens3 == ON && mask3 > 2){ 
				cout<<"ERROR @ readandprocess.mask3.2. mask3: "<<mask3<<", incr3: "<<incr3<<endl; 
				for(unsigned int n=0; n<MAX_NUM_UNIQ_EDGES_PER_VEC; n++){ cout<<">>> readandprocess.mask: masks["<<n<<"]: "<<masks[n]<<endl; } 
				for(unsigned int n=0; n<16; n++){ cout<<">>> readandprocess.mask: E["<<n<<"].dstvid: "<<acts_utilobj->UTIL_READFROM_UINT(E[n], 0, 28)<<", E["<<n<<"].incr: "<<acts_utilobj->UTIL_READFROM_UINT(E[3], 28, 4)<<endl; } 
				exit(EXIT_FAILURE); 
			}
			if(ens4 == ON && mask4 > 2){ 
				cout<<"ERROR @ readandprocess.mask4.2. mask4: "<<mask4<<", incr4: "<<incr4<<endl; 
				for(unsigned int n=0; n<MAX_NUM_UNIQ_EDGES_PER_VEC; n++){ cout<<">>> readandprocess.mask: masks["<<n<<"]: "<<masks[n]<<endl; } 
				for(unsigned int n=0; n<16; n++){ cout<<">>> readandprocess.mask: E["<<n<<"].dstvid: "<<acts_utilobj->UTIL_READFROM_UINT(E[n], 0, 28)<<", E["<<n<<"].incr: "<<acts_utilobj->UTIL_READFROM_UINT(E[4], 28, 4)<<endl; } 
				exit(EXIT_FAILURE); 
			}
			if(ens5 == ON && mask5 > 2){ 
				cout<<"ERROR @ readandprocess.mask5.2. mask5: "<<mask5<<", incr5: "<<incr5<<endl; 
				for(unsigned int n=0; n<MAX_NUM_UNIQ_EDGES_PER_VEC; n++){ cout<<">>> readandprocess.mask: masks["<<n<<"]: "<<masks[n]<<endl; } 
				for(unsigned int n=0; n<16; n++){ cout<<">>> readandprocess.mask: E["<<n<<"].dstvid: "<<acts_utilobj->UTIL_READFROM_UINT(E[n], 0, 28)<<", E["<<n<<"].incr: "<<acts_utilobj->UTIL_READFROM_UINT(E[5], 28, 4)<<endl; } 
				exit(EXIT_FAILURE); 
			}
			if(ens6 == ON && mask6 > 2){ 
				cout<<"ERROR @ readandprocess.mask6.2. mask6: "<<mask6<<", incr6: "<<incr6<<endl; 
				for(unsigned int n=0; n<MAX_NUM_UNIQ_EDGES_PER_VEC; n++){ cout<<">>> readandprocess.mask: masks["<<n<<"]: "<<masks[n]<<endl; } 
				for(unsigned int n=0; n<16; n++){ cout<<">>> readandprocess.mask: E["<<n<<"].dstvid: "<<acts_utilobj->UTIL_READFROM_UINT(E[n], 0, 28)<<", E["<<n<<"].incr: "<<acts_utilobj->UTIL_READFROM_UINT(E[6], 28, 4)<<endl; } 
				exit(EXIT_FAILURE); 
			}
			if(ens7 == ON && mask7 > 2){ 
				cout<<"ERROR @ readandprocess.mask7.2. mask7: "<<mask7<<", incr7: "<<incr7<<endl; 
				for(unsigned int n=0; n<MAX_NUM_UNIQ_EDGES_PER_VEC; n++){ cout<<">>> readandprocess.mask: masks["<<n<<"]: "<<masks[n]<<endl; } 
				for(unsigned int n=0; n<16; n++){ cout<<">>> readandprocess.mask: E["<<n<<"].dstvid: "<<acts_utilobj->UTIL_READFROM_UINT(E[n], 0, 28)<<", E["<<n<<"].incr: "<<acts_utilobj->UTIL_READFROM_UINT(E[7], 28, 4)<<endl; } 
				exit(EXIT_FAILURE); 
			}
			if(ens8 == ON && mask8 > 2){ 
				cout<<"ERROR @ readandprocess.mask8.2. mask8: "<<mask8<<", incr8: "<<incr8<<endl; 
				for(unsigned int n=0; n<MAX_NUM_UNIQ_EDGES_PER_VEC; n++){ cout<<">>> readandprocess.mask: masks["<<n<<"]: "<<masks[n]<<endl; } 
				for(unsigned int n=0; n<16; n++){ cout<<">>> readandprocess.mask: E["<<n<<"].dstvid: "<<acts_utilobj->UTIL_READFROM_UINT(E[n], 0, 28)<<", E["<<n<<"].incr: "<<acts_utilobj->UTIL_READFROM_UINT(E[8], 28, 4)<<endl; } 
				exit(EXIT_FAILURE); 
			}
			if(ens9 == ON && mask9 > 2){ 
				cout<<"ERROR @ readandprocess.mask9.2. mask9: "<<mask9<<", incr9: "<<incr9<<endl; 
				for(unsigned int n=0; n<MAX_NUM_UNIQ_EDGES_PER_VEC; n++){ cout<<">>> readandprocess.mask: masks["<<n<<"]: "<<masks[n]<<endl; } 
				for(unsigned int n=0; n<16; n++){ cout<<">>> readandprocess.mask: E["<<n<<"].dstvid: "<<acts_utilobj->UTIL_READFROM_UINT(E[n], 0, 28)<<", E["<<n<<"].incr: "<<acts_utilobj->UTIL_READFROM_UINT(E[9], 28, 4)<<endl; } 
				exit(EXIT_FAILURE); 
			}
			if(ens10 == ON && mask10 > 2){ 
				cout<<"ERROR @ readandprocess.mask10.2. mask10: "<<mask10<<", incr10: "<<incr10<<endl; 
				for(unsigned int n=0; n<MAX_NUM_UNIQ_EDGES_PER_VEC; n++){ cout<<">>> readandprocess.mask: masks["<<n<<"]: "<<masks[n]<<endl; } 
				for(unsigned int n=0; n<16; n++){ cout<<">>> readandprocess.mask: E["<<n<<"].dstvid: "<<acts_utilobj->UTIL_READFROM_UINT(E[n], 0, 28)<<", E["<<n<<"].incr: "<<acts_utilobj->UTIL_READFROM_UINT(E[10], 28, 4)<<endl; } 
				exit(EXIT_FAILURE); 
			}
			if(ens11 == ON && mask11 > 2){ 
				cout<<"ERROR @ readandprocess.mask11.2. mask11: "<<mask11<<", incr11: "<<incr11<<endl; 
				for(unsigned int n=0; n<MAX_NUM_UNIQ_EDGES_PER_VEC; n++){ cout<<">>> readandprocess.mask: masks["<<n<<"]: "<<masks[n]<<endl; } 
				for(unsigned int n=0; n<16; n++){ cout<<">>> readandprocess.mask: E["<<n<<"].dstvid: "<<acts_utilobj->UTIL_READFROM_UINT(E[n], 0, 28)<<", E["<<n<<"].incr: "<<acts_utilobj->UTIL_READFROM_UINT(E[11], 28, 4)<<endl; } 
				exit(EXIT_FAILURE); 
			}
			if(ens12 == ON && mask12 > 2){ 
				cout<<"ERROR @ readandprocess.mask12.2. mask12: "<<mask12<<", incr12: "<<incr12<<endl; 
				for(unsigned int n=0; n<MAX_NUM_UNIQ_EDGES_PER_VEC; n++){ cout<<">>> readandprocess.mask: masks["<<n<<"]: "<<masks[n]<<endl; } 
				for(unsigned int n=0; n<16; n++){ cout<<">>> readandprocess.mask: E["<<n<<"].dstvid: "<<acts_utilobj->UTIL_READFROM_UINT(E[n], 0, 28)<<", E["<<n<<"].incr: "<<acts_utilobj->UTIL_READFROM_UINT(E[12], 28, 4)<<endl; } 
				exit(EXIT_FAILURE); 
			}
			if(ens13 == ON && mask13 > 2){ 
				cout<<"ERROR @ readandprocess.mask13.2. mask13: "<<mask13<<", incr13: "<<incr13<<endl; 
				for(unsigned int n=0; n<MAX_NUM_UNIQ_EDGES_PER_VEC; n++){ cout<<">>> readandprocess.mask: masks["<<n<<"]: "<<masks[n]<<endl; } 
				for(unsigned int n=0; n<16; n++){ cout<<">>> readandprocess.mask: E["<<n<<"].dstvid: "<<acts_utilobj->UTIL_READFROM_UINT(E[n], 0, 28)<<", E["<<n<<"].incr: "<<acts_utilobj->UTIL_READFROM_UINT(E[13], 28, 4)<<endl; } 
				exit(EXIT_FAILURE); 
			}
			if(ens14 == ON && mask14 > 2){ 
				cout<<"ERROR @ readandprocess.mask14.2. mask14: "<<mask14<<", incr14: "<<incr14<<endl; 
				for(unsigned int n=0; n<MAX_NUM_UNIQ_EDGES_PER_VEC; n++){ cout<<">>> readandprocess.mask: masks["<<n<<"]: "<<masks[n]<<endl; } 
				for(unsigned int n=0; n<16; n++){ cout<<">>> readandprocess.mask: E["<<n<<"].dstvid: "<<acts_utilobj->UTIL_READFROM_UINT(E[n], 0, 28)<<", E["<<n<<"].incr: "<<acts_utilobj->UTIL_READFROM_UINT(E[14], 28, 4)<<endl; } 
				exit(EXIT_FAILURE); 
			}
			if(ens15 == ON && mask15 > 2){ 
				cout<<"ERROR @ readandprocess.mask15.2. mask15: "<<mask15<<", incr15: "<<incr15<<endl; 
				for(unsigned int n=0; n<MAX_NUM_UNIQ_EDGES_PER_VEC; n++){ cout<<">>> readandprocess.mask: masks["<<n<<"]: "<<masks[n]<<endl; } 
				for(unsigned int n=0; n<16; n++){ cout<<">>> readandprocess.mask: E["<<n<<"].dstvid: "<<acts_utilobj->UTIL_READFROM_UINT(E[n], 0, 28)<<", E["<<n<<"].incr: "<<acts_utilobj->UTIL_READFROM_UINT(E[15], 28, 4)<<endl; } 
				exit(EXIT_FAILURE); 
			}
			#endif
