...
readandprocess(bool_type enable, uint512_dt * kvdram, keyvalue_vbuffer_t vbuffer[VBUFFER_VECTOR_SIZE][BLOCKRAM_SIZE], uintNUMPby2_type vmask[BLOCKRAM_SIZE], uintNUMPby2_type vmask_subp[BLOCKRAM_SIZE], keyvalue_buffer_t buffer[VECTOR_SIZE][BLOCKRAM_SIZE], 
	...
	#pragma HLS PIPELINE II=1
		...
		// if(i<4 && srcvid < 4){ 
		// if(srcvid < 4){ 
		// cout<<"readandprocess: i: "<<i<<", mask: "<<mask<<", srcvid: "<<srcvid<<", travstate.i2: "<<travstate.i2<<", lvid: "<<lvid<<", udata: "<<udata<<endl; 
		// for(unsigned int v=0; v<VECTOR_SIZE; v++){  cout<<"readandprocess: udata: "<<udata<<", E[0]["<<v<<"]: "<<E[0][v]<<", E[1]["<<v<<"]: "<<E[1][v]<<endl; }
		// }
		#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3 // CRITICAL REMOVEME.
		if(en == ON && mask == 1){
		...
		#endif
		// if(i>8){ exit(EXIT_SUCCESS); } // CRITICAL REMOVEME.
