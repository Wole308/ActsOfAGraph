...
processit_unsliceddstvtxs( uint512_dt * kvdram, keyvalue_buffer_t sourcebuffer[VECTOR_SIZE][SOURCEBLOCKRAM_SIZE], keyvalue_vbuffer_t vbuffer[VDATA_PACKINGSIZE][BLOCKRAM_SIZE], unitBRAMwidth_type vmask[BLOCKRAM_SIZE], unit1_type vmaskBITS[VDATA_PACKINGSIZE][DOUBLE_BLOCKRAM_SIZE], unitBRAMwidth_type vmask_subp[BLOCKRAM_SIZE], uint32_type vmask_p[BLOCKRAM_SIZE], globalparams_t globalparamsE, globalparams_t globalparamsK,								
	...
	actsutilityobj->print7("### processit_unsliceddstvtxs:: source_p", "upperlimit", "begin", "end", "size", "dest range", "currentLOP", sweepparams.source_partition, sweepparams.upperlimit, avtravstate.begin_kvs * VECTOR_SIZE, avtravstate.end_kvs * VECTOR_SIZE, (avtravstate.end_kvs - avtravstate.begin_kvs) * VECTOR_SIZE, BATCH_RANGE / (1 << (NUM_PARTITIONS_POW * sweepparams.currentLOP)), sweepparams.currentLOP); 							
	#endif
	...
	#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_loop1 avg=analysis_loop1
		...
		cout<<"processit_unsliceddstvtxs: processing source partition "<<source_partition<<" ... "<<endl; // REMOVEME.
		#endif 
		...
		#ifdef _DEBUGMODE_KERNELPRINTS2
		actsutilityobj->print5("### processit_unsliceddstvtxs:: source_partition", "voffset", "vbegin", "vend", "vskip", source_partition, voffset_kvs2 * VECTOR_SIZE, avtravstate.begin_kvs * VECTOR_SIZE, avtravstate.size_kvs * VECTOR_SIZE, SRCBUFFER_SIZE * VECTOR_SIZE);
		#endif
		
		mem_accessobj->MEMACCESS_loadvmasks(ON, kvdram, vmask, vbuffer, globalparamsK.BASEOFFSETKVS_VERTICESDATAMASK + vmaskoffset_kvs, 0, vmaskbuffersz_kvs, globalparamsK); // NOTE: this should come before loadvdata because vbuffer is used as a temp buffer
		mem_accessobj->MEMACCESS_readvdata(ON, kvdram, vdatabaseoffset_kvs + voffset_kvs2, vbuffer, 0, 0, reducebuffersz, globalparamsK);
		mem_accessobj->MEMACCESS_readvdata(ON, kvdram, vdatabaseoffset_kvs + voffset_kvs2 + reducebuffersz, vbuffer, 8, 0, reducebuffersz, globalparamsK);
		...
		tuple_t tup = mem_accessobj->MEMACCESS_getvptrs_opt( kvdram, vptrbaseoffset_kvs, voffset_kvs2 * VECTOR2_SIZE, (voffset_kvs2 + SKIP_KVS) * VECTOR2_SIZE, edgebankID); // CRITICAL NEWCHANGE.
		...
		if(localendvptr < localbeginvptr){ cout<<"processit_unsliceddstvtxs::ERROR: localendvptr < localbeginvptr. localbeginvptr: "<<localbeginvptr<<", localendvptr: "<<localendvptr<<endl; exit(EXIT_FAILURE); }
		if(localendvptr < globalparamsE.SIZE_EDGES){ actsutilityobj->checkptr("processit_unsliceddstvtxs", beginsrcvid, endsrcvid, localbeginvptr, localendvptr, (keyvalue_t *)&kvdram[globalparamsE.BASEOFFSETKVS_EDGESDATA]); }
		#endif
...
void
	...
	top_nusrcv_nudstv::
	#endif 
processit_sliceddstvtxs( uint512_dt * kvdram, keyvalue_buffer_t sourcebuffer[VECTOR_SIZE][SOURCEBLOCKRAM_SIZE], keyvalue_vbuffer_t vbuffer[VDATA_PACKINGSIZE][BLOCKRAM_SIZE], unitBRAMwidth_type vmask[BLOCKRAM_SIZE], unit1_type vmaskBITS[VDATA_PACKINGSIZE][DOUBLE_BLOCKRAM_SIZE], unitBRAMwidth_type vmask_subp[BLOCKRAM_SIZE], uint32_type vmask_p[BLOCKRAM_SIZE], globalparams_t globalparamsE, globalparams_t globalparamsK,								
			unsigned int v_chunkids[EDGESSTATSDRAMSZ], unsigned int v_chunkid, unsigned int edgebankID){
	#pragma HLS INLINE 
	analysis_type analysis_loop1 = 1;
	#if defined(_DEBUGMODE_KERNELPRINTS2) || defined(_DEBUGMODE_CHECKS2)
	...
	unsigned int vreadskipsz_kvs2 = globalparamsK.SIZEKVS2_PROCESSEDGESPARTITION / globalparamsK.NUM_REDUCEPARTITIONS; // TOO EXPENSIVE.
	unsigned int vmreadskipsz_kvs2 = globalparamsK.SIZEKVS_VMASKBUFFER / globalparamsK.NUM_REDUCEPARTITIONS; // TOO EXPENSIVE.
	
	batch_type voffset_kvs2 = 0; // source_superpartition * NUM_PEs * globalparams.SIZEKVS2_REDUCEPARTITION;
	batch_type vmaskoffset_kvs = 0; // source_superpartition * NUM_PEs * SIZEKVS_VMASKBUFFER;
	// batch_type vreadoffset_kvs2 = 0;
	// batch_type vmaskreadoffset_kvs = 0;
	
	#ifdef _DEBUGMODE_KERNELPRINTS2
	actsutilityobj->print7("### processit_sliceddstvtxs:: source_p", "upperlimit", "begin", "end", "size", "dest range", "currentLOP", sweepparams.source_partition, sweepparams.upperlimit, avtravstate.begin_kvs * VECTOR_SIZE, avtravstate.end_kvs * VECTOR_SIZE, (avtravstate.end_kvs - avtravstate.begin_kvs) * VECTOR_SIZE, BATCH_RANGE / (1 << (NUM_PARTITIONS_POW * sweepparams.currentLOP)), sweepparams.currentLOP); 							
	#endif
	PROCESSIT_MAINLOOP1: for(batch_type source_superpartition=0; source_superpartition<globalparamsK.NUM_REDUCEPARTITIONS; source_superpartition+=1){
		// batch_type voffset_kvs2 = 0; // source_superpartition * NUM_PEs * globalparams.SIZEKVS2_REDUCEPARTITION;
		// batch_type vmaskoffset_kvs = 0; // source_superpartition * NUM_PEs * SIZEKVS_VMASKBUFFER;
		batch_type vreadoffset_kvs2 = 0;
		batch_type vmaskreadoffset_kvs = 0;
		#ifdef _DEBUGMODE_KERNELPRINTS3
		cout<<"processit_sliceddstvtxs: processing [voffset: "<<voffset_kvs2 * VECTOR2_SIZE<<", vmaskoffset "<<vmaskoffset_kvs * VECTOR_SIZE<<"] ... "<<endl;
		#endif 
		
		PROCESSIT_MAINLOOP1B: for(batch_type source_subpartition=0; source_subpartition<NUM_PEs; source_subpartition+=1){
		#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_loop1 avg=analysis_loop1
			unsigned int source_partition = (source_superpartition * NUM_PEs) + source_subpartition;
			#ifdef _DEBUGMODE_KERNELPRINTS3
			// cout<<"processit_sliceddstvtxs: processing source_superpartition "<<source_superpartition<<", source_subpartition: "<<source_subpartition<<", source_partition: "<<source_partition<<" ... "<<endl;
			cout<<"processit_sliceddstvtxs: processing [source_superpartition "<<source_superpartition<<", source_subpartition: "<<source_subpartition<<", source_partition: "<<source_partition<<"]: [vreadoffset: "<<vreadoffset_kvs2 * VECTOR2_SIZE<<", vmaskreadoffset: "<<vmaskreadoffset_kvs * VECTOR_SIZE<<"] ... "<<endl;
			#endif 
			
			if(voffset_kvs2 >= avtravstate.end_kvs){ voffset_kvs2 += globalparamsK.SIZEKVS2_PROCESSEDGESPARTITION; vmaskoffset_kvs += globalparamsK.SIZEKVS_VMASKBUFFER; vreadoffset_kvs2 += vreadskipsz_kvs2; vmaskreadoffset_kvs += vmreadskipsz_kvs2; continue; }
			if(GraphAlgo != PAGERANK) { if(vmask_p[source_partition] == 0){ voffset_kvs2 += globalparamsK.SIZEKVS2_PROCESSEDGESPARTITION; vmaskoffset_kvs += globalparamsK.SIZEKVS_VMASKBUFFER; vreadoffset_kvs2 += vreadskipsz_kvs2; vmaskreadoffset_kvs += vmreadskipsz_kvs2; continue; }}
			
			sweepparams.source_partition = source_partition;
			avtravstate.i_kvs = voffset_kvs2;
			
			#ifdef _DEBUGMODE_KERNELPRINTS2
			actsutilityobj->print5("### processit_sliceddstvtxs:: source_partition", "voffset", "vbegin", "vend", "vskip", source_partition, voffset_kvs2 * VECTOR_SIZE, avtravstate.begin_kvs * VECTOR_SIZE, avtravstate.size_kvs * VECTOR_SIZE, SRCBUFFER_SIZE * VECTOR_SIZE);
			#endif
			mem_accessobj->MEMACCESS_loadallvmasks_sliceddstvtxs(ON, kvdram, vmask, vbuffer, globalparamsK.BASEOFFSETKVS_VERTICESDATAMASK, vmaskreadoffset_kvs, vmreadskipsz_kvs2, globalparamsK);
			mem_accessobj->MEMACCESS_readallvdatas_sliceddstvtxs(ON, kvdram, vdatabaseoffset_kvs, vreadoffset_kvs2, vreadskipsz_kvs2, vbuffer, globalparamsK);
	
			voffset_kvs2 += globalparamsK.SIZEKVS2_PROCESSEDGESPARTITION;
			vmaskoffset_kvs += globalparamsK.SIZEKVS_VMASKBUFFER;
			vreadoffset_kvs2 += vreadskipsz_kvs2;
			vmaskreadoffset_kvs += vmreadskipsz_kvs2;
			// exit(EXIT_SUCCESS); ////
			continue; //////////////////// REMOVEME.
			...
			keyy_t numedges_kvs = numedges / VECTOR_SIZE; // NB: this is correct.
			#ifdef _DEBUGMODE_CHECKS2
			if(localendvptr < localbeginvptr){ cout<<"processit_sliceddstvtxs::ERROR: localendvptr < localbeginvptr. localbeginvptr: "<<localbeginvptr<<", localendvptr: "<<localendvptr<<endl; exit(EXIT_FAILURE); }
			if(localendvptr < globalparamsE.SIZE_EDGES){ actsutilityobj->checkptr("processit_sliceddstvtxs", beginsrcvid, endsrcvid, localbeginvptr, localendvptr, (keyvalue_t *)&kvdram[globalparamsE.BASEOFFSETKVS_EDGESDATA]); }
			#endif
 ...
 kvdram, sourcebuffer, vbuffer, vmask, vmaskBITS, vmask_subp, globalstatsbuffer, 
		...
		voffset_kvs2 += globalparamsK.NUM_REDUCEPARTITIONS * globalparamsK.SIZEKVS2_REDUCEPARTITION; // TOO EXPENSIVE.
		vmaskoffset_kvs += globalparamsK.NUM_REDUCEPARTITIONS * globalparamsK.SIZEKVS_VMASKBUFFER; // TOO EXPENSIVE.
	}
...
processit_sliceddstvtxs( uint512_dt * kvdram, keyvalue_buffer_t sourcebuffer[VECTOR_SIZE][SOURCEBLOCKRAM_SIZE], keyvalue_vbuffer_t vbuffer[VDATA_PACKINGSIZE][BLOCKRAM_SIZE], unitBRAMwidth_type vmask[BLOCKRAM_SIZE], unit1_type vmaskBITS[VDATA_PACKINGSIZE][DOUBLE_BLOCKRAM_SIZE], unitBRAMwidth_type vmask_subp[BLOCKRAM_SIZE], uint32_type vmask_p[BLOCKRAM_SIZE], globalparams_t globalparamsE, globalparams_t globalparamsK,								
	...
	*/
	...
	unsigned int vmreadskipsz_kvs2 = globalparamsK.SIZEKVS_VMASKBUFFER / NUM_PEs; // TOO EXPENSIVE.
	...
	PROCESSIT_MAINLOOP1: for(batch_type source_partition=0; source_partition<globalparamsK.NUM_PROCESSEDGESPARTITIONS; source_partition+=1){
		#ifdef _DEBUGMODE_KERNELPRINTS
		cout<<"processit_sliceddstvtxs: processing [source_partition: "<<source_partition<<"]: [voffset: "<<voffset_kvs2 * VECTOR2_SIZE<<", vmaskoffset "<<vmaskoffset_kvs * VECTOR_SIZE<<"]: [vreadoffset: "<<vreadoffset_kvs2 * NUM_PEs * VECTOR2_SIZE<<", vmaskreadoffset: "<<vmaskreadoffset_kvs * NUM_PEs * VECTOR_SIZE<<"] ... "<<endl;
		#endif 
		...
		// mem_accessobj->MEMACCESS_loadvmasks(ON, kvdram, vmask, vbuffer, globalparamsK.BASEOFFSETKVS_VERTICESDATAMASK + vmaskoffset_kvs, 0, globalparamsK.SIZEKVS_VMASKBUFFER, globalparamsK); // NOTE: this should come before loadvdata because vbuffer is used as a temp buffer
		// mem_accessobj->MEMACCESS_readvdata(ON, kvdram, vdatabaseoffset_kvs + voffset_kvs2, vbuffer, 0, 0, reducebuffersz, globalparamsK);
		// mem_accessobj->MEMACCESS_readvdata(ON, kvdram, vdatabaseoffset_kvs + voffset_kvs2 + reducebuffersz, vbuffer, 8, 0, reducebuffersz, globalparamsK);
 ...
 kvdram, sourcebuffer, vbuffer, vmask, vmaskBITS, vmask_subp, globalstatsbuffer, 
		...
		vmaskoffset_kvs += globalparamsK.SIZEKVS_VMASKBUFFER;
		vreadoffset_kvs2 += vreadskipsz_kvs2;
		vmaskreadoffset_kvs += vmreadskipsz_kvs2; ////////////////// NEWCHANGE.
...
void
	...
	top_nusrcv_nudstv::
	#endif 
partitionit( uint512_dt * kvdram, keyvalue_buffer_t sourcebuffer[VECTOR_SIZE][SOURCEBLOCKRAM_SIZE], keyvalue_vbuffer_t vbuffer[VDATA_PACKINGSIZE][BLOCKRAM_SIZE], unitBRAMwidth_type vmask[BLOCKRAM_SIZE], unit1_type vmaskBITS[VDATA_PACKINGSIZE][DOUBLE_BLOCKRAM_SIZE], unitBRAMwidth_type vmask_subp[BLOCKRAM_SIZE], globalparams_t globalparams, unsigned int edgebankID){
	#pragma HLS INLINE
	...
	actsutilityobj->printglobalvars();
	actsutilityobj->printglobalparameters("top_nusrcv_nudstv::acts_utilobj->UTIL_getglobalparams:: printing global parameters", globalparams);
	#endif 
	#if defined(_DEBUGMODE_KERNELPRINTS2) || defined(_DEBUGMODE_CHECKS2)
	actsutilityobj->clearglobalvars();
	#endif
...
void
	...
	top_nusrcv_nudstv::
	#endif 
reduceit( uint512_dt * kvdram, keyvalue_buffer_t sourcebuffer[VECTOR_SIZE][SOURCEBLOCKRAM_SIZE], keyvalue_vbuffer_t vbuffer[VDATA_PACKINGSIZE][BLOCKRAM_SIZE], unitBRAMwidth_type vmask[BLOCKRAM_SIZE], unit1_type vmaskBITS[VDATA_PACKINGSIZE][DOUBLE_BLOCKRAM_SIZE], unitBRAMwidth_type vmask_subp[BLOCKRAM_SIZE], batch_type sourcestatsmarker, batch_type source_partition, globalparams_t globalparams, unsigned int edgebankID){	
	#pragma HLS INLINE
	analysis_type analysis_numllops = 1;
	analysis_type analysis_numsourcepartitions = 1;
...
dispatch(bool_type en_process, bool_type en_partition, bool_type en_reduce,  uint512_dt * kvdram, keyvalue_buffer_t sourcebuffer[VECTOR_SIZE][SOURCEBLOCKRAM_SIZE], keyvalue_vbuffer_t vbuffer[VDATA_PACKINGSIZE][BLOCKRAM_SIZE], unitBRAMwidth_type vmask[BLOCKRAM_SIZE], unit1_type vmaskBITS[VDATA_PACKINGSIZE][DOUBLE_BLOCKRAM_SIZE], unitBRAMwidth_type vmask_subp[BLOCKRAM_SIZE], uint32_type vmask_p[BLOCKRAM_SIZE],
	...
	if(en_process == ON){ processit_sliceddstvtxs( kvdram, sourcebuffer, vbuffer, vmask, vmaskBITS, vmask_subp, vmask_p, globalparamsE, globalparamsK, v_chunkids, v_chunkid, edgebankID); } 
	#else 
	if(en_process == ON){ processit_unsliceddstvtxs( kvdram, sourcebuffer, vbuffer, vmask, vmaskBITS, vmask_subp, vmask_p, globalparamsE, globalparamsK, v_chunkids, v_chunkid, edgebankID); } 	
	#endif 
...
topkernelproc_embedded(
	...
	unit1_type vmaskBITS[VDATA_PACKINGSIZE][DOUBLE_BLOCKRAM_SIZE]; // NEWCHANGE.
