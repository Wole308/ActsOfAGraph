...
processit( uint512_dt * kvdram, keyvalue_buffer_t sourcebuffer[VECTOR_SIZE][SOURCEBLOCKRAM_SIZE], keyvalue_vbuffer_t vbuffer[VDATA_PACKINGSIZE][BLOCKRAM_SIZE], unitBRAMwidth_type vmask[BLOCKRAM_SIZE], unit1_type vmaskBITS[VDATA_PACKINGSIZE][DOUBLE_BLOCKRAM_SIZE], unitBRAMwidth_type vmask_subp[BLOCKRAM_SIZE], uint32_type vmask_p[BLOCKRAM_SIZE], globalparams_t globalparamsE, globalparams_t globalparamsK,								
	...
	batch_type voffsets_kvs[NUM_PEs]; for(unsigned int i=0; i<NUM_PEs; i++){ voffsets_kvs[i] = 0; }
	#endif 
	...
	#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_loop1 avg=analysis_loop1
		// cout<<"processit: source_partition "<<source_partition<<" ... "<<endl; // REMOVEME.
		...
		actsutilityobj->print5("### processit:: source_partition", "voffset", "vbegin", "vend", "vskip", source_partition, voffset_kvs * VECTOR_SIZE, avtravstate.begin_kvs * VECTOR_SIZE, avtravstate.size_kvs * VECTOR_SIZE, SRCBUFFER_SIZE * VECTOR_SIZE);
			...
			// for(unsigned int s=0; s<NUM_PEs / 2; s++){ mem_accessobj->MEMACCESS_readvdata(ON, kvdram, (vdatabaseoffset_kvs + (s * globalparamsK.NUM_REDUCEPARTITIONS * REDUCEPARTITIONSZ_KVS2) + (voffset_kvs / NUM_PEs)), vbuffer, 0, 0, (REDUCEPARTITIONSZ_KVS2 / NUM_PEs), globalparamsK); }
			// for(unsigned int s=NUM_PEs / 2; s<NUM_PEs; s++){ mem_accessobj->MEMACCESS_readvdata(ON, kvdram, (vdatabaseoffset_kvs + (s * globalparamsK.NUM_REDUCEPARTITIONS * REDUCEPARTITIONSZ_KVS2) + (voffset_kvs / NUM_PEs)), vbuffer, 8, 0, (REDUCEPARTITIONSZ_KVS2 / NUM_PEs), globalparamsK); }
			// for(unsigned int s=0; s<NUM_PEs; s++){
				...
				// cout<<"---vofffffset (s="<<s<<"): "<<((s * globalparamsK.NUM_REDUCEPARTITIONS * REDUCEPARTITIONSZ_KVS2) + (voffset_kvs / NUM_PEs)) * VECTOR2_SIZE<<"";
				// cout<<endl;
			// }
			// exit(EXIT_SUCCESS); ////
			
			// mem_accessobj->MEMACCESS_loadallvmasks(ON, kvdram, vmask, vbuffer, globalparamsK.BASEOFFSETKVS_VERTICESDATAMASK, vmaskoffset_kvs, globalparamsK);
			// mem_accessobj->MEMACCESS_readallvdatas(ON, kvdram, vdatabaseoffset_kvs, voffset_kvs, voffsets_kvs, vbuffer, globalparamsK);
...
dispatch_reduce( uint512_dt * kvdram, keyvalue_buffer_t sourcebuffer[VECTOR_SIZE][SOURCEBLOCKRAM_SIZE], keyvalue_vbuffer_t vbuffer[VDATA_PACKINGSIZE][BLOCKRAM_SIZE], unitBRAMwidth_type vmask[BLOCKRAM_SIZE], unit1_type vmaskBITS[VDATA_PACKINGSIZE][DOUBLE_BLOCKRAM_SIZE], unitBRAMwidth_type vmask_subp[BLOCKRAM_SIZE], uint32_type vmask_p[BLOCKRAM_SIZE], globalparams_t globalparamsE, globalparams_t globalparamsK,	
	...
	#pragma HLS LOOP_TRIPCOUNT min=0 max=analysis_loop1 avg=analysis_loop1
		...
		sourcestatsmarker += 1;
		vreadoffset_kvs += globalparamsK.SIZEKVS2_REDUCEPARTITION;
