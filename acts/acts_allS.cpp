		...
		for(unsigned int reduce_partition=0; reduce_partition<globalparamsSRC.NUM_REDUCEPARTITIONS; reduce_partition++){
			...
			#pragma HLS PIPELINE II=1
				...
				vdramDST2[globalparamsDST2.BASEOFFSETKVS_SRCVERTICESDATA + voffset_kvs + k] = vdramSRC[globalparamsSRC.BASEOFFSETKVS_SRCVERTICESDATA + voffset_kvs + k];
					...
					value_t vdata = combo >> 1; 
					// if(mask == 1){ cout<<">>> EXCHANGE:: EXCHANGE VERTICES SEEN @ i: "<<i<<", reduce_partition: "<<reduce_partition<", k: "<<k<<", v: "<<v<<", vdata: "<<vdata<<", mask: "<<mask<<", srcvid: "<<UTILS_GETREALVID((edge_data.value * EDGEDATA_PACKINGSIZE) + v, globalparams.ACTSPARAMS_INSTID)<<", dstvid*: "<<edge_data.key<<endl; }
...
void acts_all::reduce_update(bool enx, unsigned int v, unsigned int loc, keyvalue_t update_data, keyvalue_vbuffer_t vbuffer[MAX_BLOCKRAM_VDESTDATA_SIZE], unsigned int memory[1], globalparams_t globalparams){
	...
	if(en == true && new_vprop != vdata_tmp){
		#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3
		cout<<">>> REDUCE VECTOR:: REDUCE UPDATE SEEN @: v: "<<v<<", loc: "<<loc<<", vdata_tmp: "<<vdata_tmp<<", mask: "<<mask<<", update_data.value: "<<update_data.value<<", new_vprop: "<<new_vprop<<", new combo: "<<((new_vprop << 1) | 0x1)<<", dstvid: "<<UTILS_GETREALVID(update_data.key, globalparams.ACTSPARAMS_INSTID)<<endl; 
		#endif
...
void compute_and_store(hls::stream<uint512_evec_dt >& in, uint512_dt *out, keyvalue_vbuffer_t vbuffer_source[VDATA_PACKINGSIZE][MAX_BLOCKRAM_VSRCDATA_SIZE],
	...
	#pragma HLS PIPELINE II=1
		...
		// store
		UTILS_WriteDatas(out, destbaseaddr_kvs + offset_kvs + i, datas2);
	}
}
#endif 
...
void acts_all::ACTSS_read_process_partition_and_write(uint512_dt *in, uint512_dt *out, keyvalue_vbuffer_t vbuffer_source[VDATA_PACKINGSIZE][MAX_BLOCKRAM_VSRCDATA_SIZE],
	...
	for(int i = 0; i < size; i++){
		...
		// store
		UTILS_WriteDatas(out, destbaseaddr_kvs + destoffset_kvs + i, datas2);
...
void acts_all::ACTSS_read_process_and_reduce(uint512_dt *in, uint512_dt *out, keyvalue_vbuffer_t vbuffer_sourcebits[VDATA_PACKINGSIZE][BLOCKRAM_SIZE], keyvalue_vbuffer_t vbuffer_dest[VDATA_PACKINGSIZE][MAX_BLOCKRAM_VSRCDATA_SIZE],
	...
	READ_PROCESS_REDUCE_LOOP1: for(unsigned int llp_set=0; llp_set<num_LLPset; llp_set++){
		...
		READ_PROCESS_REDUCE_LOOP1B: for(unsigned int n=0; n<num_its; n++){
			...
			#pragma HLS dependence variable=vbuffer_dest inter false
				// get dataset
				UTILS_ReadEdges(in, globalparamsE.BASEOFFSETKVS_EDGESDATA + offset + i, datas);
...
void acts_all::ACTSS_actit(bool_type enable, unsigned int mode,
	...
	if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMID == BFS){ //  globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE){
		#ifdef BIT_TRAVERSAL_ALGORITHM
		ACTSS_read_process_and_reduce(kvdram, kvdram, vbuffer_sourcebits, vbuffer_dest, collections_tmp, sweepparams, globalparamsE, globalparamsK, globalposition); 
		#endif 
		...
		if(mode == ACTSPROCESSMODE || mode == ACTSPARTITIONMODE){
			...
			// #pragma HLS PIPELINE II=20
				workload_t workload_kvs;
				workload_kvs.offset_begin = MEMACCESSS_getdata(kvdram, globalparamsE.BASEOFFSETKVS_EDGESMAP, (sweepparams.source_partition * num_LLPset) + llp_set) / EDGEDATA_PACKINGSIZE;
				workload_kvs.offset_end = MEMACCESSS_getdata(kvdram, globalparamsE.BASEOFFSETKVS_EDGESMAP, (sweepparams.source_partition * num_LLPset) + llp_set + 1) / EDGEDATA_PACKINGSIZE;
				workload_kvs.size = workload_kvs.offset_end - workload_kvs.offset_begin;
				if(workload_kvs.offset_end < workload_kvs.offset_begin){ workload_kvs.size = 0; }
