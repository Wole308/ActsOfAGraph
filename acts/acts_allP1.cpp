...
unsigned int acts_all::MERGEP1_copy(uint512_dt * vdramSRC, uint512_dt * vdramDST1, uint512_dt * vdramDST2, unsigned int voffset_kvs, unsigned int firstinstance_id, unsigned int num_compute_units, 
	...
	MERGEP1_EXCHANGEVS_LOOP1A: for(unsigned int i=0; i<num_compute_units; i++){
		for(unsigned int reduce_partition=0; reduce_partition<globalparamsSRC.NUM_REDUCEPARTITIONS; reduce_partition++){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP1_ReadDatas(vdramSRC, globalparamsSRC.BASEOFFSETKVS_SRCVERTICESDATA + voffset_kvs + k, datas);
				for(unsigned int v=0; v<VECTOR_SIZE; v++){
				#pragma HLS UNROLL 
				...
				#pragma HLS UNROLL 
					...
					unsigned int vid = UTILP1_GETREALVID(lvid, firstinstance_id + i);
					if(mask == 1){ 
...
void acts_all::MERGEP1_exchange(uint512_dt * vdramA, uint512_dt * vdramB, uint512_dt * vdramC){
	...
	total_sync[0] = MERGEP1_copy(vdramA, vdramB, vdramC, voffsetA_kvs, 0, NUMCOMPUTEUNITS_SLR2, actvvs, actvvs_size, globalparamsvA, globalparamsvB, globalparamsvC);
	total_sync[1] = MERGEP1_copy(vdramB, vdramA, vdramC, voffsetB_kvs, NUMCOMPUTEUNITS_SLR2, NUMCOMPUTEUNITS_SLR1, actvvs, actvvs_size, globalparamsvB, globalparamsvA, globalparamsvC);
	total_sync[2] = MERGEP1_copy(vdramC, vdramA, vdramB, voffsetC_kvs, NUMCOMPUTEUNITS_SLR2 + NUMCOMPUTEUNITS_SLR1, NUMCOMPUTEUNITS_SLR0, actvvs, actvvs_size, globalparamsvC, globalparamsvA, globalparamsvB);
		...
		for(unsigned int v_p=0; v_p<globalparamsvA.NUM_PROCESSEDGESPARTITIONS; v_p++){
			...
			for(unsigned int t=0; t<MAXNUM_EDGEBLOCKS_PER_VPARTITION; t++){ 
				if(edgeblock_stats[v_p][t] > 0){ 
					MEMACCESSP1_setdata(vdramA, globalparamsvA.BASEOFFSETKVS_ACTIVEVERTICES, (v_p * CONFIG_HYBRIDGPMODE_HYBRIDVTHRESHOLD_PER_VPARTITION) + index, t);
					MEMACCESSP1_setdata(vdramB, globalparamsvB.BASEOFFSETKVS_ACTIVEVERTICES, (v_p * CONFIG_HYBRIDGPMODE_HYBRIDVTHRESHOLD_PER_VPARTITION) + index, t);
					MEMACCESSP1_setdata(vdramC, globalparamsvC.BASEOFFSETKVS_ACTIVEVERTICES, (v_p * CONFIG_HYBRIDGPMODE_HYBRIDVTHRESHOLD_PER_VPARTITION) + index, t);
					index += 1;
				}
			}
		}
