...
void acts_all::reduce_update(unsigned int mode, bool enx, unsigned int v, unsigned int loc, keyvalue_t update_data, keyvalue_vbuffer_t vbuffer[MAX_BLOCKRAM_VDESTDATA_SIZE], unsigned int stats[BLOCKRAM_SIZE], unsigned int memory[1], globalparams_t globalparams){
	...
	if(en == true){ // REMOVEME.
		...
		if(mode == ACTSREDUCEMODE){ cout<<">>> REDUCE VECTOR:: REDUCE UPDATE SEEN @: v: "<<v<<", loc: "<<loc<<", mask: "<<mask<<", update_data.key: "<<update_data.key<<", update_data.value: "<<update_data.value<<", new_vprop: "<<new_vprop<<", new combo: "<<((new_vprop << 1) | 0x1)<<", dstvid: "<<UTILP1_GETREALVID(update_data.key, globalparams.ACTSPARAMS_INSTID)<<", vdata_tmp: "<<vdata_tmp<<endl; }
		#endif
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
unsigned int acts_all::MERGEP1__broadcastACTVVs(uint512_dt * vdram, uint512_dt * kvdram0, unsigned int source_partition, globalparams_t globalparamsK, globalparams_t globalparamsV){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		UTILP1_ReadDatas(vdram, globalparamsV.BASEOFFSETKVS_ACTIVEVERTICESDATA + v_offset + offset, datas);
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				UTILP1_ReadDatas(vdram, globalparamsV.BASEOFFSETKVS_ACTIVEVERTICESDATA + v_offset + offset + t, datas2);
	
				UTILP1_WriteDatas(kvdram0, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
...
unsigned int acts_all::MERGEP1__broadcastACTVVs(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1, unsigned int source_partition, globalparams_t globalparamsK, globalparams_t globalparamsV){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	cout<< TIMINGRESULTSCOLOR << ">>> broadcastACTVVs:: broadcasting frontier vertices..."<< RESET <<endl; 
	#endif
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP1_WriteDatas(kvdram1, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
...
unsigned int acts_all::MERGEP1__broadcastACTVVs(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2, unsigned int source_partition, globalparams_t globalparamsK, globalparams_t globalparamsV){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	cout<< TIMINGRESULTSCOLOR << ">>> broadcastACTVVs:: broadcasting frontier vertices..."<< RESET <<endl; 
	#endif
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP1_WriteDatas(kvdram2, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
...
unsigned int acts_all::MERGEP1__broadcastACTVVs(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2,uint512_dt * kvdram3, unsigned int source_partition, globalparams_t globalparamsK, globalparams_t globalparamsV){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	cout<< TIMINGRESULTSCOLOR << ">>> broadcastACTVVs:: broadcasting frontier vertices..."<< RESET <<endl; 
	#endif
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP1_WriteDatas(kvdram3, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
...
unsigned int acts_all::MERGEP1__broadcastACTVVs(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2,uint512_dt * kvdram3,uint512_dt * kvdram4, unsigned int source_partition, globalparams_t globalparamsK, globalparams_t globalparamsV){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	cout<< TIMINGRESULTSCOLOR << ">>> broadcastACTVVs:: broadcasting frontier vertices..."<< RESET <<endl; 
	#endif
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP1_WriteDatas(kvdram4, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
...
unsigned int acts_all::MERGEP1__broadcastACTVVs(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2,uint512_dt * kvdram3,uint512_dt * kvdram4,uint512_dt * kvdram5, unsigned int source_partition, globalparams_t globalparamsK, globalparams_t globalparamsV){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	cout<< TIMINGRESULTSCOLOR << ">>> broadcastACTVVs:: broadcasting frontier vertices..."<< RESET <<endl; 
	#endif
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP1_WriteDatas(kvdram5, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
...
unsigned int acts_all::MERGEP1__broadcastACTVVs(uint512_dt * vdram, uint512_dt * kvdram0,uint512_dt * kvdram1,uint512_dt * kvdram2,uint512_dt * kvdram3,uint512_dt * kvdram4,uint512_dt * kvdram5,uint512_dt * kvdram6, unsigned int source_partition, globalparams_t globalparamsK, globalparams_t globalparamsV){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	cout<< TIMINGRESULTSCOLOR << ">>> broadcastACTVVs:: broadcasting frontier vertices..."<< RESET <<endl; 
	#endif
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP1_WriteDatas(kvdram6, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP1_WriteDatas(kvdram7, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP1_WriteDatas(kvdram8, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP1_WriteDatas(kvdram9, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP1_WriteDatas(kvdram10, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
	...
	for(unsigned int s=0; s<NUM_PEs; s++){
		...
		if(max > 0){
			...
			#pragma HLS PIPELINE II=1
				...
				UTILP1_WriteDatas(kvdram11, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + t, datas2);
				transfsz_kvs += 1;
...
unsigned int acts_all::MERGEP1_mergeACTVVs(uint512_dt * kvdram, uint512_dt * vdram){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	...
	MERGEP1_MERGEVSLOOP2: for(unsigned int reduce_partition=0; reduce_partition<globalparams.NUM_REDUCEPARTITIONS; reduce_partition++){
		...
		for(unsigned int p_=0; p_<NUM_ACTVVSPARTITIONS_PER_VPARTITION; p_++){
			...
			UTILP1_ReadDatas(kvdram, globalparams.BASEOFFSETKVS_ACTIVEVERTICESDATA + offset, datas);
			...
			if(max > 0){
				for (buffer_type t=0; t<max+1; t++){
				#pragma HLS PIPELINE II=1
					UTILP1_ReadDatas(kvdram, globalparams.BASEOFFSETKVS_ACTIVEVERTICESDATA + offset + t, datas2);
					UTILP1_WriteDatas(vdram, globalparamsv.BASEOFFSETKVS_ACTIVEVERTICESDATA + v_offset + offset + t, datas2);
					transfsz_kvs += 1;
...
unsigned int acts_all::MERGEP1_copyACTVVs(uint512_dt * vdramA, uint512_dt * vdramB, uint512_dt * vdramC, unsigned int begin_computeunitid, unsigned int num_compute_units){
	#ifdef _DEBUGMODE_KERNELPRINTS4
	...
	globalparams_t globalparamsv = UTILP1_getglobalparams(vdramA, 0);
	#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3 
	...
	for(unsigned int i=begin_computeunitid; i<(begin_computeunitid + num_compute_units); i++){
		for(unsigned int reduce_partition=0; reduce_partition<globalparamsv.NUM_REDUCEPARTITIONS; reduce_partition++){
			...
			for(unsigned int p_=0; p_<NUM_ACTVVSPARTITIONS_PER_VPARTITION; p_++){
				...
				UTILP1_ReadDatas(vdramA, globalparamsv.BASEOFFSETKVS_ACTIVEVERTICESDATA + v_offset + offset, datas);
				...
				if(max > 0){
					for (buffer_type t=0; t<max+1; t++){
					#pragma HLS PIPELINE II=1
						UTILP1_ReadDatas(vdramA, globalparamsv.BASEOFFSETKVS_ACTIVEVERTICESDATA + v_offset + offset + t, datas2);
						UTILP1_WriteDatas(vdramB, globalparamsv.BASEOFFSETKVS_ACTIVEVERTICESDATA + v_offset + offset + t, datas2);
						UTILP1_WriteDatas(vdramC, globalparamsv.BASEOFFSETKVS_ACTIVEVERTICESDATA + v_offset + offset + t, datas2);
						transfsz_kvs += 1;
							
						#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3 
						if(t>0){ for(unsigned int v=0; v<VECTOR2_SIZE; v++){ 
							value_t combo = datas2[v]; value_t lvid = combo >> ACTVVATA_BITSZ; value_t vdata = combo & 0xFFFF;
							unsigned int actvvid = UTILP1_GETREALVID(((offset + t - 1) * VECTOR2_SIZE) + v, i);
							cout<<">>> SYNC:ACTVV BUFFER:: ACTVV SEEN @: v: "<<v<<", lvid: "<<lvid<<", vdata: "<<vdata<<", num items: "<<datas[v]<<", lvid: "<<((offset + t - 1) * VECTOR2_SIZE) + v<<", actvvid: "<<UTILP1_GETREALVID(((offset + t - 1) * VECTOR2_SIZE) + v, i)<<endl; 
...
unsigned int acts_all::MERGEP1_exchangeACTVVs(uint512_dt * vdramA, uint512_dt * vdramB, uint512_dt * vdramC){
	#ifdef _DEBUGMODE_KERNELPRINTS4//3
	cout<< TIMINGRESULTSCOLOR << ">>> exchangeVs:: exchanging vertices across different SLRs..." << RESET <<endl; 
	#endif

	unsigned int transfsz_kvs = 0;
	transfsz_kvs += MERGEP1_copyACTVVs(vdramA, vdramB, vdramC, 0, NUMCOMPUTEUNITS_SLR2);
	transfsz_kvs += MERGEP1_copyACTVVs(vdramB, vdramA, vdramC, NUMCOMPUTEUNITS_SLR2, NUMCOMPUTEUNITS_SLR1);
	transfsz_kvs += MERGEP1_copyACTVVs(vdramC, vdramA, vdramB, NUMCOMPUTEUNITS_SLR2 + NUMCOMPUTEUNITS_SLR1, NUMCOMPUTEUNITS_SLR0);
	return transfsz_kvs;
}
...
#pragma HLS DATA_PACK variable = vdramC
	...
	if(globalparamsvA.ENABLE_EXCHANGECOMMAND == ON){
		MERGEP1_exchangeACTVVs(vdramA, vdramB, vdramC);
	}
	#endif 
	return;
}
	...
	if(globalparamsK.ENABLE_RUNKERNELCOMMAND == ON && en == true){
		...
		if(mode == ACTSPROCESSMODE || mode == ACTSPARTITIONMODE){
			...
			for(unsigned int s=0; s<NUM_PEs; s++){
				...
				UTILP1_ReadDatas(kvdram, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset, datas);
				unsigned int max = 0; for(unsigned int v=0; v<VECTOR2_SIZE; v++){ if(datas[v]>max){ max = datas[v]; } }
				if(max > VDATA_PACKING_PERCHANNEL){ max = 0; }
				#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3 
				...
				if(max > 0){
					...
					#pragma HLS PIPELINE II=1
						UTILP1_ReadDatas(kvdram, globalparamsK.BASEOFFSETKVS_SRCVERTICESDATA + offset + 1 + t, datas2);
		...
		if(mode == ACTSREDUCEMODE){		
			...
			for(unsigned int n=0; n<num_its; n++){
				...
				#pragma HLS PIPELINE II=1
					...
					#pragma HLS UNROLL 
						...
						if(mask == 1){
							...
							#endif
							#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3  // REMOVEME.
							cout<<">>> ACTIT:ACTVV BUFFER:: ACTVV SEEN @: v: "<<v<<", lvid: "<<index_offset + lvid<<", vdata: "<<vdata<<", mask: "<<mask<<", dstvid: "<<UTILP1_GETREALVID((index_offset + lvid), globalparamsK.ACTSPARAMS_INSTID)<<", new combo: "<<(((index_offset + lvid) << ACTVVATA_BITSZ) | vdata)<<endl; 
							#endif
						}
						#ifdef _DEBUGMODE_CHECKS3
				...
				for(unsigned int p_=0; p_<NUM_ACTVVSPARTITIONS_PER_VPARTITION; p_++){
					...
					UTILP1_WriteDatas(kvdram, globalparamsK.BASEOFFSETKVS_ACTIVEVERTICESDATA + offset, datas);
					for (buffer_type t=0; t<max; t++){
						for(unsigned int v=0; v<VECTOR2_SIZE; v++){
						#pragma HLS UNROLL 
							datas[v] = actvvbuffer[v][t].data;
						}
						UTILP1_WriteDatas(kvdram, globalparamsK.BASEOFFSETKVS_ACTIVEVERTICESDATA + offset + 1 + t, datas);
						#ifdef _DEBUGMODE_KERNELPRINTS_TRACE3  // REMOVEME.
						for(unsigned int v=0; v<VECTOR2_SIZE; v++){ 
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL1_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL1_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL1_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP1__broadcastACTVVs(vdram, kvdram0, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP1_mergeACTVVs(kvdram0, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL2_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL2_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL2_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP1__broadcastACTVVs(vdram, kvdram0,kvdram1, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP1_mergeACTVVs(kvdram1, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL3_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL3_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL3_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP1__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP1_mergeACTVVs(kvdram2, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL4_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL4_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL4_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP1__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP1_mergeACTVVs(kvdram3, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL5_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL5_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL5_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP1__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP1_mergeACTVVs(kvdram4, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL6_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL6_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL6_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP1__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP1_mergeACTVVs(kvdram5, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL7_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL7_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL7_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP1__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP1_mergeACTVVs(kvdram6, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL8_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL8_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL8_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP1__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP1_mergeACTVVs(kvdram7, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL9_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL9_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL9_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP1__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7,kvdram8, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP1_mergeACTVVs(kvdram8, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL10_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL10_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL10_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP1__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7,kvdram8,kvdram9, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP1_mergeACTVVs(kvdram9, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL11_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL11_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL11_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP1__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7,kvdram8,kvdram9,kvdram10, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP1_mergeACTVVs(kvdram10, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
...
#pragma HLS DATA_PACK variable = vdram
			...
			TOPKERNEL12_BASELOOP1C: for(step_type stage=0; stage<num_stages; stage++){
				...
				TOPKERNEL12_BASELOOP1D: for(step_type currentLOP=FIRSTLOP; currentLOP<(FIRSTLOP + NUMLOPs); currentLOP+=1){
					...
					TOPKERNEL12_BASELOOP1E: for(batch_type source_partition=FIRSTSOURCEPARTITION; source_partition<LASTSOURCEPARTITIONS; source_partition+=1){
						...
						bool en = true; if(globalparamsK.ALGORITHMINFO_GRAPHALGORITHMCLASS != ALGORITHMCLASS_ALLVERTEXISACTIVE && globalposition.num_active_edgeblocks == 0){ en = false; } // check if vertex partition is active
						if(stage==0 && globalparamsK.ENABLE_PROCESSCOMMAND == ON && en == true){
							unsigned int transfsz_kvs = MERGEP1__broadcastACTVVs(vdram, kvdram0,kvdram1,kvdram2,kvdram3,kvdram4,kvdram5,kvdram6,kvdram7,kvdram8,kvdram9,kvdram10,kvdram11, source_partition, globalparamsK, globalparamsV);
							#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST	
							collections0[BROADCASTPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
							collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
							collections0[NUMREADSFROMDRAM_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	...
	if(globalparamsV.ENABLE_MERGECOMMAND == ON){	
		...
		transfsz_kvs += MERGEP1_mergeACTVVs(kvdram11, vdram); 
	}
	#ifdef MEMACCESS_ENABLE_COLLECTSTATSFORHOST
	collections0[SYNCPHASE_TRANSFSZ_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE; 
	collections0[NUMVERTICESPROCESSED_COLLECTIONID].data1 += transfsz_kvs * VECTOR2_SIZE;
