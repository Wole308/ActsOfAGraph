...
long double host_fpga_async::runapp(action_t action__, std::string binaryFile__[2], HBM_channelAXISW_t * HBM_axichannel[2][NUM_PEs], HBM_channelAXISW_t * HBM_axicenter[2], unsigned int globalparams[1024], universalparams_t universalparams){
	...
	std::cout <<">>> write-to-FPGA time elapsed for iteration "<<iteration<<", burst_compute "<<burst_compute<<" : "<<end_time0<<" ms, "<<(end_time0 * 1000)<<" microsecs, "<<std::endl;
	...
	for (size_t iteration_idx = 0; iteration_idx < 1; iteration_idx++) {
		for(unsigned int burst_compute=0; burst_compute<num_burst_computes; burst_compute++){
			...
			std::cout << TIMINGRESULTSCOLOR << ">>> kernel time elapsed for iteration "<<iteration<<", burst_compute "<<burst_compute<<" : "<<end_time1<<" ms, "<<(end_time1 * 1000)<<" microsecs, "<< RESET <<std::endl;
			...
			std::cout <<">>> read-from-FPGA time elapsed for iteration "<<iteration<<", burst_compute "<<burst_compute<<" : "<<end_time2<<" ms, "<<(end_time2 * 1000)<<" microsecs, "<<std::endl;
			 */
		}
    }
	#endif 
