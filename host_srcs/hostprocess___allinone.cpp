...
int main(int argc, char** argv){
	...
	#ifdef RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR
	...
	for(unsigned int i=0; i<VALID_NUMPEs; i++){ 
		...
		for(unsigned int t=0; t<HBM_CHANNEL_SIZE; t++){ 
			...
			for(unsigned int v=0; v<HBM_AXI_PACK_SIZE; v++){ 
				source_in1[bank_index][t*HBM_AXI_PACK_SIZE + v] = 1;//t*HBM_AXI_PACK_SIZE + v;//HBM_axichannel[0][i][t].data[v]; 
		...
		for(unsigned int t=0; t<HBM_CHANNEL_SIZE; t++){ 
			for(unsigned int v=0; v<HBM_AXI_PACK_SIZE; v++){ 
				source_in1[bank_index][t*HBM_AXI_PACK_SIZE + v] = 1;//t*HBM_AXI_PACK_SIZE + v + 2;//HBM_axichannel[1][i][t].data[v];
	...
	for(unsigned int t=0; t<HBM_CHANNEL_SIZE; t++){ 
		for(unsigned int v=0; v<HBM_AXI_PACK_SIZE; v++){ 
			source_in1[bank_index][t*HBM_AXI_PACK_SIZE + v] = 1;//t*HBM_AXI_PACK_SIZE + v + 3;//HBM_axicenter[0][t].data[v];
	...
	for(unsigned int t=0; t<HBM_CHANNEL_SIZE; t++){ 
		for(unsigned int v=0; v<HBM_AXI_PACK_SIZE; v++){ 
			source_in1[bank_index][t*HBM_AXI_PACK_SIZE + v] = 1;//t*HBM_AXI_PACK_SIZE + v + 4;//HBM_axicenter[1][t].data[v];
	...
	for(unsigned int i=0; i<1; i++){ // VALID_NUMPEs + 2
		for(unsigned int t=0; t<1; t++){ 
			for(unsigned int v=0; v<1; v++){ 
				...
				source_in1[i][26*2] = 8; // GLOBALPARAMSCODE__PARAM__NUM_UPARTITIONS
				source_in1[i][27*2] = 4;
	...
	/* for(unsigned int i=0; i<VALID_NUMPEs + 2; i++){ 
		for(unsigned int t=0; t<1; t++){ 
			for(unsigned int v=0; v<1; v++){ 
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__CSRVPTRS*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__ACTPACKVPTRS*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__CSREDGES*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__ACTPACKEDGES*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__UPDATESPTRS*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__UPDATES*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__VDATAS*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__CFRONTIERSTMP*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__NFRONTIERS*HBM_AXI_PACK_SIZE] = 1;
				
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__CSRVPTRS*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__ACTPACKVPTRS*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__CSREDGES*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__ACTPACKEDGES*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__UPDATESPTRS*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__UPDATES*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__VDATAS*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__CFRONTIERSTMP*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__NFRONTIERS*HBM_AXI_PACK_SIZE] = 1;
				
				source_in1[i][GLOBALPARAMSCODE__PARAM__NUM_VERTICES*HBM_AXI_PACK_SIZE] = 3000000;
				source_in1[i][GLOBALPARAMSCODE__PARAM__NUM_EDGES*HBM_AXI_PACK_SIZE] = 3000000;
				source_in1[i][GLOBALPARAMSCODE__PARAM__NUM_UPARTITIONS*HBM_AXI_PACK_SIZE] = 24;
				source_in1[i][GLOBALPARAMSCODE__PARAM__NUM_APPLYPARTITIONS*HBM_AXI_PACK_SIZE] = 4;
				source_in1[i][GLOBALPARAMSCODE__PARAM__NUM_ITERATIONS*HBM_AXI_PACK_SIZE] = 12; // FIXME_HARDWARE.
				source_in1[i][GLOBALPARAMSCODE__PARAM__THRESHOLD__ACTIVEFRONTIERSFORCONTROLSWITCH*HBM_AXI_PACK_SIZE] = 128;
				source_in1[i][GLOBALPARAMSCODE__PARAM__MAXDEGREE*HBM_AXI_PACK_SIZE] = 12345;
				source_in1[i][GLOBALPARAMSCODE__PARAM__ALGORITHM*HBM_AXI_PACK_SIZE] = PAGERANK;

				source_in1[i][GLOBALPARAMSCODE___ENABLE___RESETBUFFERSATSTART*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE___ENABLE___PROCESSEDGES*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE___ENABLE___READ_FRONTIER_PROPERTIES*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE___ENABLE___VCPROCESSEDGES*HBM_AXI_PACK_SIZE] = 1; 
				source_in1[i][GLOBALPARAMSCODE___ENABLE___ECPROCESSEDGES*HBM_AXI_PACK_SIZE] = 1; 
				source_in1[i][GLOBALPARAMSCODE___ENABLE___SAVEVCUPDATES*HBM_AXI_PACK_SIZE] = 1; 
				source_in1[i][GLOBALPARAMSCODE___ENABLE___COLLECTACTIVEDSTVIDS*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE___ENABLE___APPLYUPDATESMODULE*HBM_AXI_PACK_SIZE] = 1; 
				source_in1[i][GLOBALPARAMSCODE___ENABLE___READ_DEST_PROPERTIES*HBM_AXI_PACK_SIZE] = 1;
				source_in1[i][GLOBALPARAMSCODE___ENABLE___APPLYUPDATES*HBM_AXI_PACK_SIZE] = 1; 
				source_in1[i][GLOBALPARAMSCODE___ENABLE___COLLECT_AND_SAVE_FRONTIER_PROPERTIES*HBM_AXI_PACK_SIZE] = 1; 
				source_in1[i][GLOBALPARAMSCODE___ENABLE___SAVE_DEST_PROPERTIES*HBM_AXI_PACK_SIZE] = 1; 
				source_in1[i][GLOBALPARAMSCODE___ENABLE___EXCHANGEFRONTIERINFOS*HBM_AXI_PACK_SIZE] = 1; 
			}
		}
	} */
	...
	/* for(unsigned int i=0; i<VALID_NUMPEs + 2; i++){ 
		for(unsigned int t=0; t<1; t++){ 
			for(unsigned int v=0; v<1; v++){ 
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__CSRVPTRS*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__BASEOFFSET__CSRVPTRS];
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__ACTPACKVPTRS*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__BASEOFFSET__ACTPACKVPTRS];
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__CSREDGES*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__BASEOFFSET__CSREDGES];
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__ACTPACKEDGES*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__BASEOFFSET__ACTPACKEDGES];
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__UPDATESPTRS*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__BASEOFFSET__UPDATESPTRS];
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__UPDATES*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__BASEOFFSET__UPDATES];
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__VDATAS*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__BASEOFFSET__VDATAS];
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__CFRONTIERSTMP*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__BASEOFFSET__CFRONTIERSTMP];
				source_in1[i][GLOBALPARAMSCODE__BASEOFFSET__NFRONTIERS*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__BASEOFFSET__NFRONTIERS];
				
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__CSRVPTRS*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__WWSIZE__CSRVPTRS];
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__ACTPACKVPTRS*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__WWSIZE__ACTPACKVPTRS];
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__CSREDGES*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__WWSIZE__CSREDGES];
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__ACTPACKEDGES*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__WWSIZE__ACTPACKEDGES];
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__UPDATESPTRS*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__WWSIZE__UPDATESPTRS];
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__UPDATES*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__WWSIZE__UPDATES];
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__VDATAS*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__WWSIZE__VDATAS];
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__CFRONTIERSTMP*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__WWSIZE__CFRONTIERSTMP];
				source_in1[i][GLOBALPARAMSCODE__WWSIZE__NFRONTIERS*HBM_AXI_PACK_SIZE] = globalparams[GLOBALPARAMSCODE__WWSIZE__NFRONTIERS];
				
				source_in1[i][GLOBALPARAMSCODE__PARAM__NUM_VERTICES*HBM_AXI_PACK_SIZE] = universalparams.NUM_VERTICES;
				source_in1[i][GLOBALPARAMSCODE__PARAM__NUM_EDGES*HBM_AXI_PACK_SIZE] = universalparams.NUM_EDGES;
				source_in1[i][GLOBALPARAMSCODE__PARAM__NUM_UPARTITIONS*HBM_AXI_PACK_SIZE] = universalparams.NUM_UPARTITIONS;
				source_in1[i][GLOBALPARAMSCODE__PARAM__NUM_APPLYPARTITIONS*HBM_AXI_PACK_SIZE] = universalparams.NUM_APPLYPARTITIONS;
				source_in1[i][GLOBALPARAMSCODE__PARAM__NUM_ITERATIONS*HBM_AXI_PACK_SIZE] = universalparams.NUM_ITERATIONS; // FIXME_HARDWARE.
				source_in1[i][GLOBALPARAMSCODE__PARAM__THRESHOLD__ACTIVEFRONTIERSFORCONTROLSWITCH*HBM_AXI_PACK_SIZE] = 128;
				source_in1[i][GLOBALPARAMSCODE__PARAM__MAXDEGREE*HBM_AXI_PACK_SIZE] = max_degree;
				source_in1[i][GLOBALPARAMSCODE__PARAM__ALGORITHM*HBM_AXI_PACK_SIZE] = universalparams.ALGORITHM;
    ...
    std::string binaryFile = _binaryFile1; //argv[1];
    cl_int err;
    cl::CommandQueue q;
    std::string krnl_name = "top_function";
    std::vector<cl::Kernel> krnls(NUM_KERNEL);
	...
	// #ifdef TTTTTTTTTTTTTTTTTTTTTTTTTTTT
	...
	for(unsigned int i=0; i<1; i++){ // VALID_NUMPEs + 2
		for(unsigned int t=0; t<1; t++){ 
			for(unsigned int v=0; v<1; v++){ 
				...
				source_in1[24*2] = 3000000;
				source_in1[25*2] = 3000000;
				source_in1[26*2] = 8; // GLOBALPARAMSCODE__PARAM__NUM_UPARTITIONS
				source_in1[27*2] = 4;
				// source_in1[28*2] = 12; // GLOBALPARAMSCODE__PARAM__NUM_ITERATIONS
				// source_in1[29*2] = 128;
				// source_in1[30*2] = 12345;
				// source_in1[31*2] = PAGERANK;
    ...
    std::vector<cl_mem_ext_ptr_t> inBufExt1(14);
    ...
    std::vector<cl::Buffer> buffer_input1(14);
   ...
   /*  for (int i = 0; i < NUM_KERNEL; i++) {
    ...
    // be used to reference the memory locations on the device.
    // Creating Buffers
	std::cout << "------------- SEEN 5. "<<std::endl;
    for (int i = 0; i < 14; i++) { // CL_MEM_READ_WRITE
        OCL_CHECK(err,
                  buffer_input1[i] = cl::Buffer(context, CL_MEM_READ_ONLY | CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR,
    ...
    for (int i = 0; i < NUM_KERNEL; i++) {
		...
		OCL_CHECK(err,
                  err = q.enqueueMigrateMemObjects({buffer_input1[0], buffer_input1[1], buffer_input1[2], buffer_input1[3], buffer_input1[4], buffer_input1[5], buffer_input1[6], buffer_input1[7], buffer_input1[8], buffer_input1[9], buffer_input1[10], buffer_input1[11], buffer_input1[12], buffer_input1[13]}, 0 /* 0 means from host*/));
    }
    q.finish();
    ...
    for (int i = 0; i < NUM_KERNEL; i++) {
        // Setting the k_vadd Arguments
        OCL_CHECK(err, err = krnls[i].setArg(0, buffer_input1[0]));
        OCL_CHECK(err, err = krnls[i].setArg(1, buffer_input1[1]));
		
		OCL_CHECK(err, err = krnls[i].setArg(2, buffer_input1[2]));
		OCL_CHECK(err, err = krnls[i].setArg(3, buffer_input1[3]));
		OCL_CHECK(err, err = krnls[i].setArg(4, buffer_input1[4]));
		OCL_CHECK(err, err = krnls[i].setArg(5, buffer_input1[5]));
		OCL_CHECK(err, err = krnls[i].setArg(6, buffer_input1[6]));
		OCL_CHECK(err, err = krnls[i].setArg(7, buffer_input1[7]));
		OCL_CHECK(err, err = krnls[i].setArg(8, buffer_input1[8]));
		OCL_CHECK(err, err = krnls[i].setArg(9, buffer_input1[9]));
		OCL_CHECK(err, err = krnls[i].setArg(10, buffer_input1[10]));
		OCL_CHECK(err, err = krnls[i].setArg(11, buffer_input1[11]));
		OCL_CHECK(err, err = krnls[i].setArg(12, buffer_input1[12]));
		OCL_CHECK(err, err = krnls[i].setArg(13, buffer_input1[13]));
