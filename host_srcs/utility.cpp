...
unsigned int utility::runsssp_sw(string graphname, unsigned int total_num_vertices, vector<vertex_t> &srcvids, vector<edge_t> &vertexptrbuffer, vector<edge2_type> &edgedatabuffer, unsigned int NumGraphIters, long double edgesprocessed_totals[128], unsigned int * numValidIters, 
		unsigned int * vpmaskbuffer[MAXNUMGRAPHITERATIONS], unsigned int num_edges_processed[MAXNUMGRAPHITERATIONS], 
			uint512_ivec_dt * mdram, uint512_ivec_dt * vdram, uint512_ivec_dt * kvbuffer[NUM_PEs], bool savemasks, bool printactivepartitios){						
	#ifdef _DEBUGMODE_HOSTPRINTS3
	cout<<endl<<"utility::runsssp_sw:: running traditional sssp... "<<endl;
	#endif 
	
	unsigned int maxnumiterations = MAXNUMGRAPHITERATIONS;
	if(universalparams.ALGORITHM != BFS || universalparams.ALGORITHM != SSSP){ maxnumiterations = 1; }
	...
	*numValidIters = 0;
	...
	// defaults
	for(unsigned int iter=0; iter<maxnumiterations; iter++){
	...
	if(actvvs.size() < CONFIG_HYBRIDGPMODE_MAXVTHRESHOLD){ acts_mode = OFF; }
	#if not defined(CONFIG_ALGORITHMTYPE_RANDOMACTIVEVERTICES) || not defined(CONFIG_ACTS_HYBRIDLOGIC)
	acts_mode = ON; // override
	#endif 
	mdram[BASEOFFSET_MESSAGESDATA_KVS + MESSAGES_MAILBOX + 0].data[0] = acts_mode;
	mdram[BASEOFFSET_MESSAGESDATA_KVS + MESSAGES_MAILBOX + 0].data[1] = 1;
	vdram[BASEOFFSET_MESSAGESDATA_KVS + MESSAGES_MAILBOX + 0].data[0] = acts_mode;
	vdram[BASEOFFSET_MESSAGESDATA_KVS + MESSAGES_MAILBOX + 0].data[1] = 1;
	for(unsigned int i=0; i<NUM_PEs; i++){
		kvbuffer[i][BASEOFFSET_MESSAGESDATA_KVS + MESSAGES_MAILBOX + 0].data[0] = acts_mode; // {TRAD=OFF, ACTS=ON} 
		kvbuffer[i][BASEOFFSET_MESSAGESDATA_KVS + MESSAGES_MAILBOX + 0].data[1] = 1;
	}
	
	for(unsigned int iter=0; iter<maxnumiterations; iter++){ num_edges_processed[iter] = 0; }
	
	for(GraphIter=0; GraphIter<NumGraphIters; GraphIter++){
		// cout<<">>> GraphIter: "<<GraphIter<<endl;
		unsigned int actvvcount_nextit = 0;
		// unsigned int vdram_SRC4NextIt_BASE_KVS = (GraphIter + 1) * ((vdram_SIZE_SRCVERTICESDATA / VECTOR2_SIZE) / VDATA_SHRINK_RATIO); // NOTE: must correspond with what is defined in classname__top_usrcv_nudstv.cpp
		// unsigned int vdram_SRC4NextIt_BASE_KVS = (GraphIter + 1) * ((vdram_SIZE_SRCVERTICESDATA / VECTOR2_SIZE));
		unsigned int vdram_SRC4NextIt_BASE_KVS = (GraphIter + 1) * SRC_VOLUME_SIZE_KVS;
		unsigned int actvvs_nextit_basekvs = (GraphIter + 1) * (CONFIG_HYBRIDGPMODE_MDRAMSECTIONSZ / VECTOR2_SIZE);
		cout<<"-------------------------------------------- utility: GraphIter: "<<GraphIter<<", NumGraphIters: "<<NumGraphIters<<", vdram_SRC4NextIt_BASE_KVS: "<<vdram_SRC4NextIt_BASE_KVS<<" ----------------------------------------------------------- "<<endl;
		...
		for(unsigned int i=0; i<actvvs.size(); i++){
			...
			for(unsigned int k=0; k<edges_size; k++){
				...
				if(vtemp != vprop){ 
					...
					checkoutofbounds("utility::runsssp_sw:: ERROR 20", dstvid / universalparams.PROCESSPARTITIONSZ, universalparams.NUMPROCESSEDGESPARTITIONS, dstvid, vid, dstvid);
					
					if(GraphIter+1 < maxnumiterations){ vpmaskbuffer[GraphIter+1][dstvid / universalparams.PROCESSPARTITIONSZ] += 1; }			
					#if defined(CONFIG_PRELOADEDVERTICESMASKS) // && defined(ALGORITHMTYPE_REPRESENTVDATASASBITS)
					if(savemasks == true){
						...
						// checkoutofbounds("utility::getglobalpartition", vdram_BASEOFFSETKVS_SRCVERTICESDATA + vdram_SRC4NextIt_BASE_KVS + depth_kvs2 + ((lvid / VECTOR2_SIZE) / VDATA_SHRINK_RATIO), universalparams.MAXHBMCAPACITY_KVS2, vdram_SRC4NextIt_BASE_KVS, depth_kvs2, s);
						checkoutofbounds("utility::getglobalpartition", vdram_BASEOFFSETKVS_SRCVERTICESDATA + vdram_SRC4NextIt_BASE_KVS + depth_kvs2 + ((lvid / VECTOR2_SIZE) / VDATA_SHRINK_RATIO), universalparams.MAXHBMCAPACITY_KVS2, vdram_BASEOFFSETKVS_SRCVERTICESDATA, vdram_SRC4NextIt_BASE_KVS, s);
						...
						// if(READBITSFROM_UINTV(vdram[vdram_BASEOFFSETKVS_SRCVERTICESDATA + vdram_SRC4NextIt_BASE_KVS + depth_kvs2 + ((lvid / VECTOR2_SIZE) / VDATA_SHRINK_RATIO)].data[(lvid % VECTOR2_SIZE)], OFFSETOF_VMASK, SIZEOF_VMASK) == 0){
							// vpmaskbuffer[GraphIter+1][dstvid / (1 << universalparams.PROCESSPARTITIONSZ_KVS2)] += 1; 
						// }
						
						WRITEBITSTO_UINTV(&vdram[vdram_BASEOFFSETKVS_SRCVERTICESDATA + vdram_SRC4NextIt_BASE_KVS + depth_kvs2 + ((lvid / VECTOR2_SIZE) / VDATA_SHRINK_RATIO)].data[(lvid % VECTOR2_SIZE)], OFFSETOF_VMASK, SIZEOF_VMASK, 1);	
						#endif 
						#if defined(CONFIG_PRELOADEDVERTICESMASKS) && defined(ALGORITHMTYPE_REPRESENTVDATASASBITS)
						
						// if(READBITSFROM_UINTV(vdram[vdram_BASEOFFSETKVS_SRCVERTICESDATA + vdram_SRC4NextIt_BASE_KVS + depth_kvs2 + ((lvid / VECTOR2_SIZE) / VDATA_SHRINK_RATIO)].data[(lvid % VECTOR2_SIZE)], BEGINOFFSETOF_VMASK + (lvid % VDATA_SHRINK_RATIO), SIZEOF_VMASK) == 0){
							// vpmaskbuffer[GraphIter+1][dstvid / (1 << universalparams.PROCESSPARTITIONSZ_KVS2)] += 1; 
						// }
						// WRITEBITSTO_UINTV(&vdram[vdram_BASEOFFSETKVS_SRCVERTICESDATA + vdram_SRC4NextIt_BASE_KVS + depth_kvs2 + ((lvid / VECTOR2_SIZE) / VDATA_SHRINK_RATIO)].data[(lvid % VECTOR2_SIZE)], BEGINOFFSETOF_VMASK + (lvid % VDATA_SHRINK_RATIO), SIZEOF_VMASK, 1);					
						...
						// unsigned int data = vdram[vdram_BASEOFFSETKVS_SRCVERTICESDATA + ((vdram_SRC4NextIt_BASE_KVS + depth_kvs2 + (lvid / VECTOR2_SIZE)) / VDATA_SHRINK_RATIO)].data[(lvid % VECTOR2_SIZE)];
						// unsigned int one = 1;
						// WRITEBITSTO_UINTV(&data, BEGINOFFSETOF_VMASK + (lvid % VDATA_SHRINK_RATIO), SIZEOF_VMASK, one);	
						// vdram[vdram_BASEOFFSETKVS_SRCVERTICESDATA + ((vdram_SRC4NextIt_BASE_KVS + depth_kvs2 + (lvid / VECTOR2_SIZE)) / VDATA_SHRINK_RATIO)].data[(lvid % VECTOR2_SIZE)] = data;
						
						// tempvdram[lvid % VECTOR2_SIZE][vdram_SRC4NextIt_BASE_KVS + depth_kvs2 + (lvid / VECTOR2_SIZE)] = 1;
						// WRITEBITSTO_UINTV(&tempvdram[lvid % VECTOR2_SIZE][(vdram_SRC4NextIt_BASE_KVS + depth_kvs2 + (lvid / VECTOR2_SIZE)) / VDATA_SHRINK_RATIO], BEGINOFFSETOF_VMASK + (lvid % VDATA_SHRINK_RATIO), SIZEOF_VMASK, 1);					
	...
	if(printactivepartitios == true){
		cout<<">>> utility::runsssp_sw: printing active vertex partitions: maxnumiterations: "<<maxnumiterations<<", universalparams.NUMPROCESSEDGESPARTITIONS: "<<universalparams.NUMPROCESSEDGESPARTITIONS<<", PROCESSPARTITIONSZ: "<<(1 << universalparams.PROCESSPARTITIONSZ_KVS2)<<endl;
		for(unsigned int iter=0; iter<maxnumiterations; iter++){
		...
		#ifdef _DEBUGMODE_HOSTPRINTS3
		unsigned int num_actvs = 0;
		for(unsigned int iter=0; iter<maxnumiterations; iter++){
			cout<<">>> utility::runsssp_sw: iter: "<<iter<<endl;	
			num_actvs = 0;
			for(unsigned int t=0; t<universalparams.NUMPROCESSEDGESPARTITIONS; t++){
				// if(vpmaskbuffer[iter][t] > 0){ cout<<t<<": "<<vpmaskbuffer[iter][t]<<endl; num_actvs += vpmaskbuffer[iter][t]; }
				if(vpmaskbuffer[iter][t] > 0){ num_actvs += vpmaskbuffer[iter][t]; }
			}
			cout<<" ("<<num_actvs<<" active vertices processd, "<<total_num_vertices<<" vertices in total)"<<endl;	
		}
		#endif 
	}
	...
	for(unsigned int iter=0; iter<maxnumiterations; iter++){
		cout<<">>> utility:: edgesprocessed_totals["<<iter<<"] "<<(unsigned int)edgesprocessed_totals[iter]<<endl;
	}
	
	// check 
	for(unsigned int GraphIter=0; GraphIter<NumGraphIters; GraphIter++){
		unsigned int numactvvs = 0;
		// unsigned int vdram_SRC4NextIt_BASE_KVS = GraphIter * ((vdram_SIZE_SRCVERTICESDATA / VECTOR2_SIZE));
		unsigned int vdram_SRC4NextIt_BASE_KVS = GraphIter * SRC_VOLUME_SIZE_KVS;
		unsigned int num_vertices = NUM_PEs * (universalparams.NUMREDUCEPARTITIONS * universalparams.REDUCEPARTITIONSZ_KVS2 * VECTOR2_SIZE);
		for(unsigned int lvid=0; lvid<num_vertices; lvid++){
			unsigned int mask = 0;
			#if defined(CONFIG_PRELOADEDVERTICESMASKS) && not defined(ALGORITHMTYPE_REPRESENTVDATASASBITS)
			mask = READBITSFROM_UINTV(vdram[vdram_BASEOFFSETKVS_SRCVERTICESDATA + vdram_SRC4NextIt_BASE_KVS + ((lvid / VECTOR2_SIZE) / VDATA_SHRINK_RATIO)].data[(lvid % VECTOR2_SIZE)], OFFSETOF_VMASK, SIZEOF_VMASK);	
			#endif 
			#if defined(CONFIG_PRELOADEDVERTICESMASKS) && defined(ALGORITHMTYPE_REPRESENTVDATASASBITS)
			// unsigned int mask = tempvdram[(lvid % VECTOR2_SIZE)][vdram_SRC4NextIt_BASE_KVS + (lvid / VECTOR2_SIZE)];	
			mask = READBITSFROM_UINTV(tempvdram[(lvid % VECTOR2_SIZE)][(vdram_SRC4NextIt_BASE_KVS + (lvid / VECTOR2_SIZE)) / VDATA_SHRINK_RATIO], BEGINOFFSETOF_VMASK + (lvid % VDATA_SHRINK_RATIO), SIZEOF_VMASK);

			// WRITEBITSTO_UINTV(&tempvdram[lvid % VECTOR2_SIZE][vdram_SRC4NextIt_BASE_KVS + ((depth_kvs2 + (lvid / VECTOR2_SIZE)) / VDATA_SHRINK_RATIO)], BEGINOFFSETOF_VMASK + (lvid % VDATA_SHRINK_RATIO), SIZEOF_VMASK, 1);					
			
			#endif 
			if(mask == 1){ numactvvs += 1; }
		}
		cout<<">>> utility:: check:: GraphIter: "<<GraphIter<<", number of active vertices: "<<numactvvs<<", num_vertices: "<<num_vertices<<endl;
		cout<<">>> utility:: checkkkk:: NUM_PEs: "<<NUM_PEs<<", universalparams.NUMREDUCEPARTITIONS: "<<universalparams.NUMREDUCEPARTITIONS<<", universalparams.REDUCEPARTITIONSZ_KVS2: "<<universalparams.REDUCEPARTITIONSZ_KVS2<<", VECTOR2_SIZE: "<<VECTOR2_SIZE<<endl;
	}
	
	/* for(unsigned int v=0; v<16; v++){
		for(unsigned int GraphIter=0; GraphIter<NumGraphIters; GraphIter++){
			unsigned int vdram_SRC4NextIt_BASE_KVS = GraphIter * ((vdram_SIZE_SRCVERTICESDATA / VECTOR2_SIZE));
			for(unsigned int row=0; row<vdram_SIZE_SRCVERTICESDATA / VECTOR2_SIZE; row+=VDATA_SHRINK_RATIO){
				unsigned int data = 0;
				for(unsigned int k=0; k<VDATA_SHRINK_RATIO; k++){	
					WRITEBITSTO_UINTV(&data, BEGINOFFSETOF_VMASK + k, SIZEOF_VMASK, tempvdram[v][vdram_SRC4NextIt_BASE_KVS + row + k]);			
				}
				vdram[vdram_BASEOFFSETKVS_SRCVERTICESDATA + (vdram_SRC4NextIt_BASE_KVS + (row / VDATA_SHRINK_RATIO))].data[v] = data;
			}
		}
	} */
	...
	/* for(unsigned int GraphIter=0; GraphIter<NumGraphIters; GraphIter++){
		...
		for(unsigned int t=0; t<num_vertices; t++){
			#if defined(CONFIG_PRELOADEDVERTICESMASKS) && not defined(ALGORITHMTYPE_REPRESENTVDATASASBITS)
			unsigned int mask = READBITSFROM_UINTV(vdram[vdram_BASEOFFSETKVS_SRCVERTICESDATA + ((vdram_SRC4NextIt_BASE_KVS + (t / VECTOR2_SIZE)) / VDATA_SHRINK_RATIO)].data[(t % VECTOR2_SIZE)], OFFSETOF_VMASK, SIZEOF_VMASK);	
			#endif 
			#if defined(CONFIG_PRELOADEDVERTICESMASKS) && defined(ALGORITHMTYPE_REPRESENTVDATASASBITS)
			unsigned int mask = READBITSFROM_UINTV(vdram[vdram_BASEOFFSETKVS_SRCVERTICESDATA + ((vdram_SRC4NextIt_BASE_KVS + (t / VECTOR2_SIZE)) / VDATA_SHRINK_RATIO)].data[(t % VECTOR2_SIZE)], BEGINOFFSETOF_VMASK + (t % VDATA_SHRINK_RATIO), SIZEOF_VMASK);	
			#endif 
			if(mask == 1){ numactvvs += 1; }
		}
		cout<<">>> utility:: check(2):: GraphIter: "<<GraphIter<<", number of active vertices: "<<numactvvs<<", num_vertices: "<<num_vertices<<endl;
		cout<<">>> utility:: check(2):: NUM_PEs: "<<NUM_PEs<<", universalparams.NUMREDUCEPARTITIONS: "<<universalparams.NUMREDUCEPARTITIONS<<", universalparams.REDUCEPARTITIONSZ_KVS2: "<<universalparams.REDUCEPARTITIONSZ_KVS2<<", VECTOR2_SIZE: "<<VECTOR2_SIZE<<endl;
	} */
