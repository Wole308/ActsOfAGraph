	...
	for(unsigned int llp_id=0; llp_id<EDGE_PACK_SIZE; llp_id++){	
		...
		#pragma HLS PIPELINE II=1
			...
			#pragma HLS UNROLL
				...
				// if(t < count[v][p_[v]].size){ edge_update[v].srcvid = edge_update_.srcvid % MAX_UPARTITION_VECSIZE; edge_update[v].dstvid = edge_update_.dstvid % MAX_UPARTITION_VECSIZE; }  // FIXME. 
				// if(t < count[v][p_[v]].size){ edge_update[v].srcvid = edge_update_.srcvid % 8192; edge_update[v].dstvid = edge_update_.dstvid % 8192; }  // FIXME. 
...
#pragma HLS resource variable=edgeupdates_buffer core=XPM_MEMORY uram 
...
for(unsigned int i=0; i<MAXNUMGRAPHITERATIONS; i++){ for(unsigned int t=0; t<MAX_NUM_UPARTITIONS; t++){ hybrid_map[i][t] = 1; }}
	...
	#ifdef RRRRRRRRRRRRRRRRRRRRR
	// import (keep the PCIe channels busy always)
	import_frontiers(0, HBM_centerA, HBM_import, action, _NUMCLOCKCYCLES_);
	
	// export (keep the PCIe channels busy always)
	export_frontiers(0, HBM_centerA, HBM_export, action, _NUMCLOCKCYCLES_);
	#endif 
...
#pragma HLS resource variable=edgeupdates_buffer core=XPM_MEMORY URAM_map 
		...
		EC_PROCESS_EDGEUPDATES_LOOP1B: for(unsigned int llp_id=0; llp_id<EDGE_PACK_SIZE; llp_id++){ // REMOVEME.
			...
			APPLY_EDGEUPDATES_MAINLOOP1: for(unsigned int c=0; c<1; c++){
				...
				// for(unsigned int t=0; t<EDGE_UPDATES_DRAMBUFFER_SIZE; t++){ 	
					// URAM_map[t].offset = 0; URAM_map[t].size = 0;
				// } 				
				MY_LOOP1580: for(unsigned int t=0; t<EDGE_UPDATES_DRAMBUFFER_SIZE; t++){ 
					...
					#pragma HLS UNROLL
						for(unsigned int v=0; v<EDGE_PACK_SIZE; v++){
						#pragma HLS UNROLL		
							URAM_map[inst][v][t].offset = 0; URAM_map[inst][v][t].size = 0;	
						}
					}
				}
				...
				for(unsigned int t=0; t<max_sz_eu; t++){ // sz_ // EDGE_UPDATES_DRAMBUFFER_SIZE // FIXME // FIXHANG
				...
				// for(unsigned int t=0; t<EDGE_UPDATES_DRAMBUFFER_SIZE; t++){ 
					// for(unsigned int inst=0; inst<1; inst++){
						// for(unsigned int v=0; v<1; v++){
							// if(URAM_map[inst][v][t].size > 0){ cout<<"------ URAM_map["<<inst<<"]["<<v<<"]["<<t<<"].size: "<<URAM_map[inst][v][t].size<<endl; tots_ += 1; tots2_ += URAM_map[inst][v][t].size; }
						// }
					// }
				// }
				// cout<<"------ summary: used "<<tots_<<" slots (of 8192 slots). tots2: "<<tots2_<<endl;
				
				// for(unsigned int t=1; t<EDGE_UPDATES_DRAMBUFFER_SIZE; t++){ URAM_map[inst][v][t].offset = URAM_map[t-1].offset + URAM_map[t-1].size; }	
				unsigned int padding_space = 16; // 8;
				for(unsigned int t=1; t<EDGE_UPDATES_DRAMBUFFER_SIZE; t++){ 
					for(unsigned int inst=0; inst<NUM_VALID_PEs; inst++){
					#pragma HLS UNROLL
						for(unsigned int v=0; v<EDGE_PACK_SIZE; v++){
						#pragma HLS UNROLL
							URAM_map[inst][v][t].offset = URAM_map[inst][v][t-1].offset + (((URAM_map[inst][v][t-1].size + (padding_space-1)) / padding_space) * padding_space); 
							// URAM_map[inst][v][t].offset = URAM_map[t-1].offset + (((URAM_map[t-1].size + (fac-1)) / fac) * fac); 
						}
					}
				}	
				...
				for(unsigned int t=0; t<EDGE_UPDATES_DRAMBUFFER_SIZE; t++){ 
					for(unsigned int inst=0; inst<1; inst++){
						for(unsigned int v=0; v<1; v++){
							if(URAM_map[inst][v][t].size > 0){ cout<<"------ URAM_map["<<inst<<"]["<<v<<"]["<<t<<"].offset: "<<URAM_map[inst][v][t].offset<<", URAM_map["<<inst<<"]["<<v<<"]["<<t<<"].size: "<<URAM_map[inst][v][t].size<<endl; tots_ += 1; tots2_ += URAM_map[inst][v][t].size; }
						}
					}
				}
				cout<<"------ summary: used "<<tots_<<" slots (of 8192 slots). tots2: "<<tots2_<<endl;
						...
						#pragma HLS UNROLL
							...
							#pragma HLS UNROLL
								...
								if(en == true){
									...
									// if(v==0 && (edge_update.srcvid==507 || edge_update.srcvid==510)){ cout<<"$$$ load-edge-updates:: [edge_update-update: srcvid: "<<edge_update.srcvid<<", dstvid: "<<edge_update.dstvid<<"]---[edge_hashid: "<<edge_hashid<<"]"<<endl; }								
									
									#ifdef _____DEBUG_____
									// if(v==0){ edge_hashid = URAM_map[edge_update.srcvid].offset + (   ((edge_update.dstvid / 16) % 16)  +  (edge_update.dstvid % 16)   ); } //***
									// if(v==0){ edge_hashid = URAM_map[edge_update.srcvid].offset + ((edge_update.srcvid / 16) * 14) + (((edge_update.dstvid / 16) % 16) + (edge_update.dstvid % 16)); }
									unsigned int hash_val = URAM_map[inst][v][edge_update.srcvid].size * padding_space; edge_hashid = URAM_map[inst][v][edge_update.srcvid].offset + ((edge_update.srcvid / hash_val) * (hash_val-2)) + (((edge_update.dstvid / hash_val) % hash_val) + (edge_update.dstvid % hash_val)); 
									#ifdef _DEBUGMODE_CHECKS3
									checkoutofbounds("utility::ERROR 776688::", edge_hashid, EDGE_UPDATES_DRAMBUFFER_SIZE, edge_update.srcvid, edge_update.dstvid, NAp);
									#endif
									#endif
...
unsigned int offset_c = 0 * 0; // FIXME
...
EC_PROCESS_EDGES_LOOP1: for(unsigned int llp_set=0; llp_set<__NUM_ACTIVE_LLPSETS; llp_set++){	// __NUM_APPLYPARTITIONS
	EC_PROCESS_EDGES_LOOP1B: for(unsigned int llp_id=0; llp_id<NUM_LLP_PER_LLPSET; llp_id++){ 
		...
		#pragma HLS PIPELINE II=1
			...
			// export (FPGA -> Host)
			HBM_export[t].data[0] = HBM_centerB[offset_c + t].data[0];
			HBM_export[t].data[1] = HBM_centerB[offset_c + t].data[1];
			HBM_export[t].data[2] = HBM_centerB[offset_c + t].data[2];
			HBM_export[t].data[3] = HBM_centerB[offset_c + t].data[3];
			HBM_export[t].data[4] = HBM_centerB[offset_c + t].data[4];
			HBM_export[t].data[5] = HBM_centerB[offset_c + t].data[5];
			HBM_export[t].data[6] = HBM_centerB[offset_c + t].data[6];
			HBM_export[t].data[7] = HBM_centerB[offset_c + t].data[7];
			HBM_export[t].data[8] = HBM_centerB[offset_c + t].data[8];
			HBM_export[t].data[9] = HBM_centerB[offset_c + t].data[9];
			HBM_export[t].data[10] = HBM_centerB[offset_c + t].data[10];
			HBM_export[t].data[11] = HBM_centerB[offset_c + t].data[11];
			HBM_export[t].data[12] = HBM_centerB[offset_c + t].data[12];
			HBM_export[t].data[13] = HBM_centerB[offset_c + t].data[13];
			HBM_export[t].data[14] = HBM_centerB[offset_c + t].data[14];
			HBM_export[t].data[15] = HBM_centerB[offset_c + t].data[15];
			// }
		}
...
#pragma HLS PIPELINE II=1
// #pragma HLS dependence variable=vdata_buffer type=inter false
...
#pragma HLS UNROLL
	...
	#pragma HLS UNROLL
		MY_LOOP237: for(unsigned int local_subpartitionID=0; local_subpartitionID<NUM_SUBPARTITION_PER_PARTITION; local_subpartitionID++){ nfrontier_buffer___size[n][v][local_subpartitionID] = 0; }
	}
}
...
COLLECT_AND_SAVE_FRONTIER_INFO_LOOP1: for(unsigned int local_subpartitionID=0; local_subpartitionID<NUM_SUBPARTITION_PER_PARTITION; local_subpartitionID++){
	...
	#pragma HLS UNROLL
		...
		#pragma HLS UNROLL
			temp_[n][v] = nfrontier_buffer___size[n][v][local_subpartitionID];
		}
		temp_2_[n] = nfrontier_dram___size[n][upartitionID];
	}
	...
	unsigned int baseoffset0_aup = local_subpartitionID * VDATA_SUBPARTITION_VECSIZE;
	...
	COLLECT_FRONTIER_INFO_LOOP1A: for(unsigned int au_p=0; au_p<num_its; au_p++){
		...
		#ifdef _DEBUGMODE_CHECKS3
		checkoutofbounds("acts_kernel::ERROR 772::", ((local_subpartitionID * VDATA_SUBPARTITION_VECSIZE) + (au_p * VDATA_SUBSUBPARTITION_VECSIZE)), MAX_APPLYPARTITION_VECSIZE, local_subpartitionID, au_p, VDATA_SUBPARTITION_VECSIZE);
		checkoutofbounds("acts_kernel::ERROR 773::", offset_aup, BLOCKRAM_SIZE, local_subpartitionID, au_p, VDATA_SUBPARTITION_VECSIZE);
		#endif 
			...
			#pragma HLS PIPELINE II=1
				...
				#pragma HLS UNROLL
					...
					#pragma HLS UNROLL
						if(en==true){ 
							#ifdef _DEBUGMODE_CHECKS3
							checkoutofbounds("acts_kernel::ERROR 713::", dstvid_lpv, MAX_APPLYPARTITION_VECSIZE, local_subpartitionID, au_p, t); 
							#endif	
							vprop_t vprop =  retrieve_vdatabuffer(v, dstvid_lpv, vdata_buffer[inst]);
	...
	unsigned int offset = globalparams[GLOBALPARAMSCODE__BASEOFFSET__NFRONTIERS] + (upartitionID * MAX_APPLYPARTITION_VECSIZE);
	...
	#pragma HLS PIPELINE II=1
		...
		dinsertmany_nfrontierdram(offset, upartitionID, offsets2, t, actvvs, ens,  HBM_channelA0, HBM_channelB0);
	...
	#pragma HLS UNROLL
		...
		#pragma HLS UNROLL
			nfrontier_buffer___size[n][v][local_subpartitionID] = temp_[n][v];
		}
		nfrontier_dram___size[n][upartitionID] = temp_2_[n];
	}
}
