...
EdgeProcess<K,V>::Start() {
	printf( "\t\t++ sortreduce-master/examples/graph/src/EdgeProcess -> EdgeProcess<K,V>::Start() %d Threads \n", m_ep_count );
...
/* float processedgefunc(unsigned int Uprop, unsigned int edgeweight){
	value_t ret = 0;
	#ifdef PR_ALGORITHM
	ret = Uprop;
...
template <class K, class V>
inline void
EdgeProcess<K,V>::LoadKVs(K key, V val, keyvalue_t * kvdram, unsigned int kvoffset, unsigned int * kvdram_weof){
	// If all other threads are busy, do it here
	unsigned int firstvptr = vertexptrs[srcvoffset];
	
	for(unsigned int i=0; i<actvvsize; i++){
		unsigned int srcvid = actvvs[i];
		...
		for(unsigned int j=beginvptr; j<(beginvptr + numedges); j++){
			...
			vertexupdate.value = processedgefunc(sourcedata, 1);
