...
SortReduce<K,V>::SortReduce(SortReduceTypes::Config<K,V> *config) {
	...
	#else 
	// buffer_manager_io->Init(1024*1024*2, 64*64); //FIXME set to maximum of ... very large GBs. 
	buffer_manager_io->Init(1024*1024*2, 64*16); //FIXME set to maximum of ... very large GBs. 
	#endif 
...
SortReduce<K,V>::ManagerThread() {
	...
	printf( "\t\t++ sortreduce-master/src/sortreduce.cpp -> SortReduce<K,V>::ManagerThread() \n" );
	printf( "\t\t++ sortreduce-master/src/sortreduce.cpp -> SortReduce<K,V>::ManagerThread(): m_config->max_storage_allocatd_bytes: %d \n", m_config->max_storage_allocatd_bytes );
	printf( "\t\t++ sortreduce-master/src/sortreduce.cpp -> SortReduce<K,V>::ManagerThread(): m_config->temporary_directory: %c \n", m_config->temporary_directory.c_str() );
	...
	printf("sortreduce-master/src/sortreduce.cpp -> SortReduce<K,V>::ManagerThread():: statvfs_ret: %d \n", statvfs_ret );
	printf("sortreduce-master/src/sortreduce.cpp -> SortReduce<K,V>::ManagerThread():: m_config->max_storage_allocatd_bytes: %d \n", m_config->max_storage_allocatd_bytes );
	printf("sortreduce-master/src/sortreduce.cpp -> SortReduce<K,V>::ManagerThread():: fs_stat.f_bavail: %ld \n", fs_stat.f_bavail );
	printf("sortreduce-master/src/sortreduce.cpp -> SortReduce<K,V>::ManagerThread():: fs_stat.f_bsize: %ld \n", fs_stat.f_bsize );
	printf("sortreduce-master/src/sortreduce.cpp -> SortReduce<K,V>::ManagerThread():: fs_avail_bytes: %ld \n", fs_avail_bytes );
	
	if (m_config->max_storage_allocatd_bytes == 0 && ( statvfs_ret != 0 || fs_avail_bytes == 0 ) ) {
		fprintf(stderr, "sortreduce-master/src/sortreduce.cpp -> SortReduce<K,V>::ManagerThread():: statvfs returns invalid storage capacity! Set storage usage manually via Config\n" );		
		exit(1);
	}
