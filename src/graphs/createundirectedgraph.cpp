...
void createundirectedgraph::start(){
	...
	unsigned int totaldegrees = 0;
	unsigned int alltotaldegrees = 0;
	std::vector<keyvalue_t> greaterthan_1000_vids[64];
	for(unsigned int j=0; j<64; j++){ greaterthan_1000_vids[j].clear(); }
	for(unsigned int k=0; k<KVDATA_RANGE; k++){
		keyvalue_t kv; kv.key = k; kv.value = inoutdegree_dup[k];
		if(inoutdegree_dup[k] >= 0){ greaterthan_1000_vids[0].push_back(kv); } 
		if(inoutdegree_dup[k] >= 100){ greaterthan_1000_vids[1].push_back(kv); } 
		if(inoutdegree_dup[k] >= 200){ greaterthan_1000_vids[2].push_back(kv); } 
		if(inoutdegree_dup[k] >= 500){ greaterthan_1000_vids[3].push_back(kv); } 
		if(inoutdegree_dup[k] >= 1000){ greaterthan_1000_vids[4].push_back(kv); } 
		alltotaldegrees += inoutdegree_dup[k];
	}
	for(unsigned int t=0; t<64; t++){
		totaldegrees = 0;
		unsigned int sizz; if(greaterthan_1000_vids[t].size() > 16){ sizz = 16; } else { sizz = greaterthan_1000_vids[t].size(); }
		for(unsigned int k=0; k<greaterthan_1000_vids[t].size(); k++){
			if(k<sizz){ cout<<"vid: "<<greaterthan_1000_vids[t][k].key<<", degree: "<<greaterthan_1000_vids[t][k].value<<endl; }
			totaldegrees += greaterthan_1000_vids[t][k].value;
		}
		cout<<"makeundirectedgraph_orderbyindegree: number of vertices with degrees greater than t("<<t<<"); 0/100/200/500/1000: "<<greaterthan_1000_vids[t].size()<<",totaldegrees: "<<totaldegrees<<", alltotaldegrees: "<<alltotaldegrees<<endl<<endl;
	}
	exit(EXIT_SUCCESS);
	////////////////////////////////////////////////////////////////////////////////// */
	/* //////////////////////////////////////////////////////////////////////////////////
	// what vertices have the highest degrees? (location of graphs: /localtmp/oj2zf/dataset/)
	unsigned int totaldegrees = 0;
	unsigned int alltotaldegrees = 0;
	std::vector<keyvalue_t> greaterthan_1000_vids[64];
	for(unsigned int j=0; j<64; j++){ greaterthan_1000_vids[j].clear(); }
	for(unsigned int k=0; k<KVDATA_RANGE; k++){
		...
		for(unsigned int m=0; m<32; m++){
			if(inoutdegree_dup[k] >= m*100){ greaterthan_1000_vids[m].push_back(kv); } 
		}
		alltotaldegrees += inoutdegree_dup[k];
	}
	for(unsigned int t=0; t<64; t++){
		totaldegrees = 0;
		unsigned int sizz; if(greaterthan_1000_vids[t].size() > 8){ sizz = 8; } else { sizz = greaterthan_1000_vids[t].size(); }
		for(unsigned int k=0; k<greaterthan_1000_vids[t].size(); k++){
			if(k<sizz){ cout<<"vid: "<<greaterthan_1000_vids[t][k].key<<", degree: "<<greaterthan_1000_vids[t][k].value<<endl; }
			totaldegrees += greaterthan_1000_vids[t][k].value;
		}
		cout<<"makeundirectedgraph_orderbyindegree: number of vertices with degrees greater than [t("<<t<<"); degree above("<<t*100<<")]: "<<greaterthan_1000_vids[t].size()<<",totaldegrees: "<<totaldegrees<<", alltotaldegrees: "<<alltotaldegrees<<endl<<endl;
	}
	
	//which range has for 1024
	unsigned int ts[8];
	unsigned int first = OFF; unsigned int second = OFF; unsigned int third = OFF;
	for(unsigned int t=32; t>0; t--){
		if(greaterthan_1000_vids[t].size() > 1024 && first == OFF){ cout<<"1024 => CHECK FOR T("<<t+1<<")"<<endl; first = ON; ts[0] = t-1; }
		if(greaterthan_1000_vids[t].size() > 16384 && second == OFF){ cout<<"16384 => CHECK FOR T("<<t+1<<")"<<endl; second = ON; ts[1] = t-1; }
		// if(greaterthan_1000_vids[t].size() > 262144 && third == OFF){ cout<<"262144 => CHECK FOR T("<<t-1<<")"<<endl; third = ON; }
	}
	exit(EXIT_SUCCESS);
	
	// rename vertices according to these three steps
	// for(unsigned int i=0; i<greaterthan_1000_vids[ts[0]].size(); i++){
		// unsigned int vid = greaterthan_1000_vids[i];
		// newvid_oldvid.key = i; newvid_oldvid.value = vid; //  = {i, vid};
		// oldvid_newvid.key = vid; oldvid_newvid.value = i;
	// }
	////////////////////////////////////////////////////////////////////////////////// */
