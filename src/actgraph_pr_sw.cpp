...
void actgraph_pr_sw::launchkernel(int threadidx, uint512_dt ** kvsourcedram, uint512_dt ** kvdestdram, keyvalue_t ** kvstats){
	...
	#else
	for (int iy = 0; iy < NUMCPUTHREADS; iy+=NUMDRAMBANKS){
		#ifdef LOCKE
		for (int ddr = 0; ddr < NUMDRAMBANKS; ddr++){ kernelobjs[threadidx][ddr]->topkernel(kvsourcedram[iy + ddr], kvdestdram[ddr], kvstats[iy + ddr]); }
		#else 
		for (int ddr = 0; ddr < NUMDRAMBANKS; ddr++){ runacts_thread[threadidx][ddr] = std::thread(&actgraph_pr_sw::topkernel, this, ddr, threadidx, kvsourcedram[iy + ddr], kvdestdram[ddr], kvstats[iy + ddr]); }
		for (int ddr = 0; ddr < NUMDRAMBANKS; ddr++){ runacts_thread[threadidx][ddr].join(); }
		#endif
	}
	#endif
	return;
