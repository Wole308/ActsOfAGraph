...
void actgraph_pr_sw::WorkerThread2(int threadidx, int threadidxoffset){
	...
	#ifndef PR_ALGORITHM // NEWLYADDED.
...
void actgraph_pr_sw::launchkernel(int threadidx, uint512_dt ** kvsourcedram, uint512_dt ** kvdestdram, keyvalue_t ** kvstats){
	...
	#else 
	for (int i = 0; i < NUMDRAMBANKS; i++){ runacts_thread[threadidx][i] = std::thread(&actgraph_pr_sw::topkernel, this, i, threadidx, kvsourcedram[i], kvdestdram[i], kvstats[i]); }
