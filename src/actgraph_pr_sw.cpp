...
actgraph_pr_sw::actgraph_pr_sw(graph * _graphobj){
	...
	#ifdef SW
	for(unsigned int i=0; i<NUMCPUTHREADSY; i++){
		for(unsigned int flag=0; flag<NUMFLAGS; flag++){ for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ kvsourcedram[i][flag][ddr] = new uint512_vec_dt[PADDEDKVSOURCEDRAMSZ_KVS]; }}
		for(unsigned int flag=0; flag<NUMFLAGS; flag++){ for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ kvdestdram[i][flag][ddr] = new uint512_vec_dt[KVDATA_RANGE_PERSSDPARTITION_KVS]; }}
		for(unsigned int flag=0; flag<NUMFLAGS; flag++){ for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ kvstats[i][flag][ddr] = new keyvalue_t[KVSTATSDRAMSZ]; }}
	}
	#endif
	#ifdef FPGA_IMPL
	for(unsigned int i=0; i<NUMCPUTHREADSY; i++){
		for(unsigned int flag=0; flag<NUMFLAGS; flag++){ for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ kvsourcedram[i][flag][ddr] = (uint512_vec_dt *) aligned_alloc(4096, (KVDATA_BATCHSIZE_KVS * sizeof(uint512_vec_dt))); }}
		for(unsigned int flag=0; flag<NUMFLAGS; flag++){ for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ kvdestdram[i][flag][ddr] = (uint512_vec_dt *) aligned_alloc(4096, (KVDATA_RANGE_PERSSDPARTITION_KVS * sizeof(uint512_vec_dt))); }}
		for(unsigned int flag=0; flag<NUMFLAGS; flag++){ for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ kvstats[i][flag][ddr] = (keyvalue_t *) aligned_alloc(4096, (KVSTATSDRAMSZ * sizeof(keyvalue_t))); }}
	}
	#endif
	...
	#ifdef SW 
	for(unsigned int i=0; i<NUMCPUTHREADSY; i++){ for(unsigned int j=0; j<NUMCPUTHREADS; j++){ kernelobjs[i][j] = new acts(); }}
	#endif 
	...
	#ifndef PR_ALGORITHM
	unsigned int isactivevertexinfosz = (KVDATA_RANGE / graphobj->getnumvertexbanks()) / NUMBITSINUNSIGNEDINT;
	for(unsigned int i=0; i<graphobj->getnumvertexbanks(); i++){ isactivevertexinfobuffer_source[i] = new unsigned int[isactivevertexinfosz]; }
	for(unsigned int i=0; i<graphobj->getnumvertexbanks(); i++){ 
		isactivevertexinfobuffer_dest[i] = new unsigned int[isactivevertexinfosz]; 
		for(unsigned int k=0; k<isactivevertexinfosz; k++){ isactivevertexinfobuffer_dest[i][k] = 0; }}
	#endif
...
actgraph_pr_sw::~actgraph_pr_sw(){
	...
	#endif 
	for(unsigned int i=0; i<NUMCPUTHREADSY; i++){		
		for(unsigned int flag=0; flag<NUMFLAGS; flag++){ for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ delete [] kvsourcedram[i][flag][ddr]; }}
		for(unsigned int flag=0; flag<NUMFLAGS; flag++){ for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ delete [] kvdestdram[i][flag][ddr]; }}
		for(unsigned int flag=0; flag<NUMFLAGS; flag++){ for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ delete [] kvstats[i][flag][ddr]; }}
	}
}
...
runsummary_t actgraph_pr_sw::start2(unsigned int graph_iterationidx) {
	...
	std::chrono::steady_clock::time_point begintime = std::chrono::steady_clock::now();
	for(unsigned int i_batch=0; i_batch<graphobj->getnumvertexbanks(); i_batch += NUMCPUTHREADSY){
		cout<<">>> actgraph_pr_sw::start2: super iteration: [i_batch: "<<i_batch<<"][size: "<<graphobj->getnumvertexbanks()<<"][step: "<<NUMCPUTHREADSY<<"]"<<endl;
		for (int i = 0; i < NUMCPUTHREADSY; i++) { WorkerThread2(i, i_batch); }
...
void actgraph_pr_sw::WorkerThread2(int threadidx, int threadidxoffset){
	...
	replicateverticesdata((keyvalue_t **)kvdestdram[threadidx][0], 0, KVDATA_RANGE_PERSSDPARTITION);
	#ifdef FPGA_IMPL
	writeVstokernel(threadidx);
	#endif
	
	for(unsigned int lbankoffset = 0; lbankoffset < graphobj->getnumedgebanks(); lbankoffset += NUMCPUTHREADS){
		edge_t edgepropertyfilesize[NUMCPUTHREADS];
		unsigned int _iteration_size[NUMCPUTHREADS];	
		...
		loadvertexpropertiesfromfile(threadidx, lbankoffset);
		#ifndef PR_ALGORITHM
		loadvertexisactiveinfosfromfile(threadidx, threadidxoffset);
		#endif
		for(unsigned int i=0; i<NUMCPUTHREADS; i++){
			edgepropertyfilesize[i] = lseek(graphobj->getnvmeFd_edgeproperties_r2()[lbankoffset + i][threadidxoffset + threadidx], 0, SEEK_END) / sizeofedgeprop;			
			_iteration_size[i] = utilityobj[threadidx]->hceildiv((lseek(graphobj->getnvmeFd_edgeproperties_r2()[lbankoffset + i][threadidxoffset + threadidx], 0, SEEK_END) / sizeofedgeprop), KVDATA_BATCHSIZE);
		}
		
		unsigned int iteration_size = 0;
		for(unsigned int i=0; i<NUMCPUTHREADS; i++){ iteration_size = utilityobj[threadidx]->hmax(iteration_size, _iteration_size[i]); }			
		#ifdef _DEBUGMODE_HOSTPRINTS
		for(unsigned int i=0; i<NUMCPUTHREADS; i++){ cout<<"actgraph_pr_sw::WorkerThread: edgepropertyfilesize["<<i<<"]: "<<edgepropertyfilesize[i]<<", numverticespervertexbank: "<<KVDATA_RANGE_PERSSDPARTITION<<", threadidx: "<<(threadidxoffset + threadidx)<<endl; }			
		#endif
		
		unsigned int runningvertexid[NUMCPUTHREADS];
		unsigned int kvcount[NUMCPUTHREADS]; for (int i = 0; i < NUMCPUTHREADS; i++){ kvcount[i] = 0; }
		unsigned int keyvaluecount[NUMCPUTHREADS]; for (int i = 0; i < NUMCPUTHREADS; i++){ keyvaluecount[i] = 0; }
		for(unsigned int i=0; i<NUMCPUTHREADS; i++){ runningvertexid[i] = utilityobj[threadidx]->GETKVDATA_RANGEOFFSET_FORSSDPARTITION_((threadidxoffset + threadidx)); }
		for(int flag = 0; flag < NUMFLAGS; flag++){ for(int ddr = 0; ddr < NUMCPUTHREADS; ddr++){ utilityobj[threadidx]->resetkeyvalues("actgraph_pr_sw:: resetting messages", &kvstats[threadidx][flag][ddr][BASEOFFSET_MESSAGESDRAM], MESSAGES_SIZE); }} 
		
		for (unsigned int iteration_idx = 0; iteration_idx < iteration_size; iteration_idx++){
			#ifdef _DEBUGMODE_HOSTPRINTS2
			cout<<"PP&A:: [threadidx:"<<(threadidxoffset + threadidx)<<"][size:"<<graphobj->getnumvertexbanks()<<"][step:"<<NUMCPUTHREADS<<"], [lbankoffset:"<<lbankoffset<<"][size:"<<graphobj->getnumedgebanks()<<"][step:"<<NUMCPUTHREADS<<"], [iteration_idx:"<<iteration_idx<<"][size:"<<iteration_size<<"][step:1]"<<endl;		
			#endif 
			#ifdef _DEBUGMODE_TIMERS
	...
	#ifndef PR_ALGORITHM
	graphobj->savevertexisactiveinfostofile(graphobj->getnvmeFd_vertexisactive_w2(), utilityobj[0]->GETKVDATA_RANGEOFFSET_FORSSDPARTITION_((threadidxoffset + threadidx)), isactivevertexinfobuffer_dest[threadidx], 0, KVDATA_RANGE_PERSSDPARTITION, (graph_iterationidx + 1)); 
	#endif
	#ifdef _DEBUGMODE_TIMERS
...
void actgraph_pr_sw::generatekvs(int threadidx, unsigned int flag, unsigned int threadidxoffset, unsigned int subthreadidx, unsigned int gbankoffset, unsigned int lbankoffset, edge_t * edgepropertyfilesize, unsigned int * runningvertexid, unsigned int iteration_idx, unsigned int iteration_size, unsigned int kvcount[NUMCPUTHREADS], unsigned int keyvaluecount[NUMCPUTHREADS]){
	#if (defined(_DEBUGMODE_TIMERS) || defined(LOCKE))
	std::chrono::steady_clock::time_point begintime_populatekvdram = std::chrono::steady_clock::now();
	#endif
	
	for (int i = 0; i < NUMCPUTHREADS; i++){ kvcount[i] = 0; }
	#ifdef LOCKE
	for (int i = 0; i < NUMCPUTHREADS; i++){ actgraph_pr_sw::workerthread_generatekvs(i, flag, threadidx, threadidxoffset, subthreadidx, gbankoffset, lbankoffset, edgepropertyfilesize[i], runningvertexid, iteration_idx, iteration_size, kvcount, keyvaluecount); }
	#else
	for (int i = 0; i < NUMCPUTHREADS; i++){ genw_thread[threadidx][i] = std::thread(&actgraph_pr_sw::workerthread_generatekvs, this, i, flag, threadidx, threadidxoffset, subthreadidx, gbankoffset, lbankoffset, edgepropertyfilesize[i], runningvertexid, iteration_idx, iteration_size, kvcount, keyvaluecount); }
	for (int i = 0; i < NUMCPUTHREADS; i++){ genw_thread[threadidx][i].join(); }
	#endif
	for (int i = 0; i < NUMCPUTHREADS; i++){ totalnumkvsread[threadidx] += kvcount[i]; }
	#ifdef _DEBUGMODE_STATS
	for (int i = 0; i < NUMCPUTHREADS; i++){ globaldebugger_totalkeyvaluesstransferredtokernel += kvcount[i]; }
	#endif
...
void actgraph_pr_sw::workerthread_generatekvs(unsigned int ddr, unsigned int flag, int threadidx, unsigned int threadidxoffset, unsigned int subthreadidx, unsigned int gbankoffset, unsigned int lbankoffset, edge_t edgepropertyfilesize, unsigned int * runningvertexid, unsigned int iteration_idx, unsigned int iteration_size, unsigned int kvcount[NUMCPUTHREADS], unsigned int keyvaluecount[NUMCPUTHREADS]){			
	vertex_t edgepropsz = utilityobj[ddr]->hmin(KVDATA_BATCHSIZE, utilityobj[ddr]->hsub((size_t)edgepropertyfilesize, (size_t)((size_t)((size_t)iteration_idx * (size_t)KVDATA_BATCHSIZE))));
	unsigned int returnkvcount = edge_process_obj[ddr]->generateupdates_stream((iteration_idx * KVDATA_BATCHSIZE), utilityobj[ddr]->GETKVDATA_RANGEOFFSET_FORSSDPARTITION_((threadidxoffset + threadidx)), &runningvertexid[ddr], 
	...
	if((utilityobj[ddr]->runActs(iteration_idx) == 1) || (iteration_idx == (iteration_size-1))){
		#ifdef FPGA_IMPL 
...
void actgraph_pr_sw::settleupdates(int threadidx, unsigned int flag, unsigned int iteration_idx, unsigned int iteration_size){
	if((utilityobj[threadidx]->runActs(iteration_idx) == 1) || (iteration_idx == (iteration_size-1))){
		for(int ddr = 0; ddr < NUMCPUTHREADS; ddr++){
...
void actgraph_pr_sw::printstructures(int threadidx, unsigned int flag){
	for (int ddr = 0; ddr < NUMCPUTHREADS; ddr++){ utilityobj[threadidx]->printkvdrams(kvsourcedram[threadidx][flag][ddr]); }	
	// for (int ddr = 0; ddr < NUMCPUTHREADS; ddr++){ utilityobj[threadidx]->printstats(16, kvstats[threadidx][flag][ddr]); }	
	return;
}
...
void actgraph_pr_sw::loadvertexpropertiesfromfile(int threadidx, int lbankoffset){
	#ifdef LOCKE
	for(unsigned int i=0; i<NUMCPUTHREADS; i++){ workerthread_loadvertexpropertiesfromfile(i, graphobj->getnvmeFd_vertexproperties_r2()[lbankoffset + i], 0, vertexpropertiesbuffer[i], 0, KVDATA_RANGE_PERSSDPARTITION); }
	#else
	for (int i = 0; i < NUMCPUTHREADS; i++){ genw_thread[threadidx][i] = std::thread(&actgraph_pr_sw::workerthread_loadvertexpropertiesfromfile, this, i, graphobj->getnvmeFd_vertexproperties_r2()[lbankoffset + i], 0, vertexpropertiesbuffer[i], 0, KVDATA_RANGE_PERSSDPARTITION); }
	for (int i = 0; i < NUMCPUTHREADS; i++){ genw_thread[threadidx][i].join(); }
	#endif 
}
void actgraph_pr_sw::workerthread_loadvertexpropertiesfromfile(int ithreadidx, int fd, unsigned int offset, vertexprop_t * buffer, vertex_t bufferoffset, vertex_t size){
	#ifdef _DEBUGMODE_HOSTPRINTS
	std::cout << "actgraph_pr_sw::workerthread_loadvertexpropertiesfromfile. loading vertices from file... vertexprop offset: "<<bufferoffset<<", size: "<<size<<", offset: "<<offset<<endl;
	#endif 
	...
	#ifdef CONFIG_FACTOROUTSSDLATENCY
	edge_process_obj[ithreadidx]->appendtime_ssdaccesses_ms(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - begintime_ssdaccess).count());
	unlockmutex(&mutex_pr_ssdaccess);
	#endif
	
	#ifdef _DEBUGMODE_HOSTPRINTS
	std::cout <<"actgraph_pr_sw::workerthread_loadvertexpropertiesfromfile. buffer[0].outdegree: "<<buffer[0].outdegree<<endl;
	std::cout <<"actgraph_pr_sw::workerthread_loadvertexpropertiesfromfile. buffer[1].outdegree: "<<buffer[1].outdegree<<endl;
	if(size > 0){ std::cout <<"actgraph_pr_sw::workerthread_loadvertexpropertiesfromfile. buffer["<<(size-1)<<"].outdegree: "<<buffer[size-1].outdegree<<endl; }
	#endif
	return;
}
...
void actgraph_pr_sw::replicateverticesdata(keyvalue_t * kvdram[NUMCPUTHREADS], unsigned int offset, unsigned int size){
	#ifdef _DEBUGMODE_HOSTPRINTS
	std::cout << endl << "actgraph_pr_sw::replicateverticesdata. replicating verticesdata across DRAMs. offset: "<<offset<<", size: "<<size<<endl;
	#endif
	for(unsigned int i=0; i<NUMCPUTHREADS; i++){ memcpy(&kvdram[i][offset], &kvdram[0][offset], (size * sizeof(keyvalue_t))); }
	return;
}
void actgraph_pr_sw::loadvertexisactiveinfosfromfile(int threadidx, int threadidxoffset){
	#ifdef LOCKE
	for(unsigned int i=0; i<NUMCPUTHREADS; i++){ workerthread_loadvertexisactiveinfosfromfile(i, graphobj->getnvmeFd_vertexisactive_r2(), utilityobj[0]->GETKVDATA_RANGEOFFSET_FORSSDPARTITION_((threadidxoffset + threadidx)), isactivevertexinfobuffer_source[i], 0, KVDATA_RANGE_PERSSDPARTITION); }
	#else
	for (int i = 0; i < NUMCPUTHREADS; i++){ genw_thread[threadidx][i] = std::thread(&actgraph_pr_sw::workerthread_loadvertexisactiveinfosfromfile, this, i, graphobj->getnvmeFd_vertexisactive_r2(), utilityobj[0]->GETKVDATA_RANGEOFFSET_FORSSDPARTITION_((threadidxoffset + threadidx)), isactivevertexinfobuffer_source[i], 0, KVDATA_RANGE_PERSSDPARTITION); }
	for (int i = 0; i < NUMCPUTHREADS; i++){ genw_thread[threadidx][i].join(); }
	#endif 
}
void actgraph_pr_sw::workerthread_loadvertexisactiveinfosfromfile(int ithreadidx, int fd, unsigned int offset, unsigned int * buffer, vertex_t bufferoffset, vertex_t size){
	#ifdef _DEBUGMODE_HOSTPRINTS
	std::cout << "actgraph_pr_sw::workerthread_loadvertexisactiveinfosfromfile. loading vertices from file... vertexprop offset: "<<bufferoffset<<", size: "<<size<<", offset: "<<offset<<endl;
	#endif 
	...
	#ifdef _DEBUGMODE_HOSTPRINTS
	std::cout <<"actgraph_pr_sw::workerthread_loadvertexisactiveinfosfromfile. buffer[0]: "<<buffer[0]<<endl;
	std::cout <<"actgraph_pr_sw::workerthread_loadvertexisactiveinfosfromfile. buffer[1]: "<<buffer[1]<<endl;
	if(size > 0){ std::cout <<"actgraph_pr_sw::workerthread_loadvertexisactiveinfosfromfile. buffer["<<(size-1)<<"]: "<<buffer[size-1]<<endl; }
	#endif
	return;
}
...
void actgraph_pr_sw::loadOCLstructures(std::string binaryFile){
	...
	for (int threadidx = 0; threadidx < NUMCPUTHREADSY; threadidx++) {
...
void actgraph_pr_sw::finishOCL(){
	...
	for (int threadidx = 0; threadidx < NUMINSTANCES; threadidx++) {
		for(int flag=0; flag<2; flag++){
...
#ifdef SW
void actgraph_pr_sw::topkernel(unsigned int ddr, int threadidx, uint512_dt * kvsourcedram, uint512_dt * kvdestdram, keyvalue_t * kvstats){
	return kernelobjs[threadidx][ddr]->topkernel(kvsourcedram, kvdestdram, kvstats);
}
void actgraph_pr_sw::launchkernel(int threadidx, uint512_dt ** kvsourcedram, uint512_dt ** kvdestdram, keyvalue_t ** kvstats){
	// return; // REMOVEME.
