...
actgraph_pr_sw::actgraph_pr_sw(graph * _graphobj){
	...
	#ifdef SW 
	// for(unsigned int j=0; j<NUMDRAMBANKS; j++){ kernelobjs[j] = new acts(); } // NEWLYADDED.
	...
	// for(unsigned int i=0; i<NUMCPUTHREADS; i++){ kernel_process[i] = new kernelprocess(kernelobjs[i]); }	
	#endif 
...
void actgraph_pr_sw::launchkernel(int threadidx, uint512_dt ** kvsourcedram, uint512_dt ** kvdestdram, keyvalue_t ** kvstats){
	...
	#else 
	for (int i = 0; i < NUMDRAMBANKS; i++){ acts_thread[i] = std::thread(&actgraph_pr_sw::topkernel, this, i, threadidx, kvsourcedram[i], kvdestdram[i], kvstats[i]); }
