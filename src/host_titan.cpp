...
host_titan::host_titan(graph * _graphobj){
	graphobj = _graphobj;
...
host_titan::~host_titan(){
	cout<<"host_titan::~host_titan:: finish destroying memory structures... "<<endl;
	for(unsigned int i=0; i<NUMCPUTHREADS; i++){		
...
void host_titan::start() {
	WorkerThread(0);
}
void host_titan::WorkerThread(int threadidx){
	loaddrams(threadidx);
	#ifdef SW
...
void host_titan::finish(){
	#if (defined(FPGA_IMPL) && defined(PR_ALGORITHM))
	finishOCL();
	#endif
}

void host_titan::loaddrams(int threadidx){
	#ifdef SW
	...
	#ifdef SW
	cout<<endl<<"=== host_titan:: loaddrams ENDED. printing summary and out messages === "<<endl;
	kernelobj->printglobalvars();	
	cout<<"host_titan::loaddrams: test ended."<<endl;
	kernelobj->printparameters();
	#endif 
	return;
}
void host_titan::loadkvdram(uint512_vec_dt * kvdram, unsigned int baseoffset_kvs, unsigned int kvsize_kvs){
	...
	// printkeyvalues("host_titan::loadkvdram", (keyvalue_t *)kvdram, 8);
	// exit(EXIT_SUCCESS);
	return;
}
/* void host_titan::loadcapsulestats(uint512_vec_dt * capsules, unsigned int baseoffset_kvs, unsigned int kvsize_kvs){
	cout<<"host_titan:: loadcapsulestats started."<<endl;
...
void host_titan::loadstats(uint512_vec_dt * kvdram, keyvalue_t * kvstats, vertex_t kvdramoffset, vertex_t kvdramsz, vertex_t kvrangeoffset, unsigned int LLOPnumpartitions, unsigned int LLOPrangepartitions){
	...
	cout<<"host_titan::loadstats:: LLOPnumpartitions: "<<LLOPnumpartitions<<", LLOPrangepartitions: "<<LLOPrangepartitions<<endl;
...
void host_titan::loadmessages(int * messages){
	...
	messages[DRAM_TREEDEPTH] = 1;//TREE_DEPTH;
...
void host_titan::checkoutofbounds(string message, unsigned int data, unsigned int upper_bound){
	if(data > upper_bound){ cout<<"host_titan::checkoutofbounds: ERROR. out of bounds. message: "<<message<<", data: "<<data<<", upper_bound: "<<upper_bound<<endl; exit(EXIT_FAILURE); }
}
void host_titan::printkeyvalues(string message, keyvalue_t * keyvalues, unsigned int size){
...
#ifdef SW
void host_titan::launchswkernel(int threadidx){
	kernelobj->topkernel(
...
#endif 
#ifdef FPGA_IMPL
void host_titan::loadOCLstructures(std::string binaryFile){
	cl_int err;
	global = 1; local = 1;
...
void host_titan::launchkernel(unsigned int flag){
	#ifdef LAUNCHKERNEL
...
void host_titan::finishOCL(){
	cout<<"host_titan::finish destroying memory structures... "<<endl;
	for(unsigned int flag=0; flag<2; flag++){ clWaitForEvents(1, &kernel_events[flag]); }
	
	cout<<"host_titan::finish: releasing and destroying all OCL structures... "<<endl;
	#ifdef LAUNCHKERNEL
	...
	cout<<"host_titan::finish: released and destroyed all OCL structures"<<endl;
	#endif 
}
#endif 
