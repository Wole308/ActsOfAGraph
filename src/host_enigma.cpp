...
host_enigma::host_enigma(graph * _graphobj){
	...
	for(unsigned int i=0; i<NUMCPUTHREADSY; i++){
		for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ kvsourcedram[i][ddr] = new uint512_vec_dt[PADDEDKVSOURCEDRAMSZ_KVS]; }
		for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ kvdestdram[i][ddr] = new uint512_vec_dt[KVDATA_RANGE_PERSSDPARTITION_KVS]; }
		for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ kvstats[i][ddr] = new keyvalue_t[KVSTATSDRAMSZ]; }
	}
	#else
	for(unsigned int i=0; i<NUMCPUTHREADSY; i++){
		for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ kvsourcedram[i][ddr] = (uint512_vec_dt *) aligned_alloc(4096, (KVDATA_BATCHSIZE_KVS * sizeof(uint512_vec_dt))); }
		for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ kvdestdram[i][ddr] = (uint512_vec_dt *) aligned_alloc(4096, (KVDATA_RANGE_PERSSDPARTITION_KVS * sizeof(uint512_vec_dt))); }
		for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ kvstats[i][ddr] = (keyvalue_t *) aligned_alloc(4096, (MESSAGESDRAMSZ * sizeof(keyvalue_t))); } // NEWCHAGE.
	}
	#endif 
	...
	#endif 
	for(unsigned int i=0; i<NUMCPUTHREADSY; i++){ utilityobj[i] = new utility(); }
	for(unsigned int i=0; i<NUMCPUTHREADSY; i++){ edgeprocessobj[i] = new edge_process(graphobj); }
	srand (0);
	// srand(time(0)); 
}
host_enigma::~host_enigma(){
	...
	for(unsigned int i=0; i<NUMCPUTHREADSY; i++){		
		for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ delete [] kvsourcedram[i][ddr]; }
		for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ delete [] kvdestdram[i][ddr]; }
		for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ delete [] kvstats[i][ddr]; }
	}
	#ifdef FPGA_IMPL
	finishOCL();
	#endif 
...
void host_enigma::WorkerThread(int threadidx){
	...
	for(unsigned int IterCount=0; IterCount<iteration_size; IterCount++){
		...
		for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ loadmessages(kvstats[threadidx][ddr], BASEOFFSET_MESSAGESDRAM, IterCount); }
		if((runActs(IterCount) == 1) || (IterCount == (iteration_size-1))){ 
			calculatealloffsets(threadidx); 
			for(unsigned int ddr=0; ddr<NUMCPUTHREADS; ddr++){ 
		...
		#endif 
		utilityobj[threadidx]->stopTIME("Total time elapsed (Entire TopKernel + OCL Transfer time elapsed): ", begintime_topkernel, NAp);
	...
	cout<< TIMINGRESULTSCOLOR <<">>> host_enigma::summary Total Size : "<<(unsigned long)((unsigned long)iteration_size * (unsigned long)KVDATA_BATCHSIZE * NUMCPUTHREADS)<<" keyvalues"<< RESET <<endl;
	cout<< TIMINGRESULTSCOLOR <<">>> host_enigma::summary Total Time Elapsed (ms) : "<<(long double)(totaltime_topkernel_ms)<<" milliseconds"<< RESET <<endl;
	cout<< TIMINGRESULTSCOLOR <<">>> host_enigma::summary Total Time Elapsed (s) : "<<(long double)(totaltime_topkernel_ms / 1000)<<" seconds"<< RESET <<endl;
	cout<< TIMINGRESULTSCOLOR <<">>> host_enigma::summary Throughput : "<<(unsigned long)((unsigned long)iteration_size * (unsigned long)KVDATA_BATCHSIZE * NUMCPUTHREADS) / (long double)(totaltime_topkernel_ms / 1000)<<" keyvalues per second"<< RESET <<endl;
	cout<< TIMINGRESULTSCOLOR <<">>> host_enigma::summary Throughput : "<<(unsigned long)(((unsigned long)iteration_size * (unsigned long)KVDATA_BATCHSIZE * NUMCPUTHREADS) / 1000000) / (long double)(totaltime_topkernel_ms / 1000)<<" million keyvalues per second"<< RESET <<endl;
	return;
}
...
#ifdef SW
void host_enigma::topkernel(unsigned int ddr, int threadidx, uint512_dt * kvsourcedram, uint512_dt * kvdestdram, keyvalue_t * kvstats){
...
void host_enigma::loadOCLstructures(std::string binaryFile){
	...
	#ifdef TESTKERNEL
	kvdest_size_bytes = ((1 << 26) / 8) * sizeof(uint512_vec_dt); // NEWCHAGE. don't need this for parameters evaluation
	#else 
	...
	for(int flag=0; flag<NUMCPUTHREADSY; flag++){
...
void host_enigma::launchkernel(unsigned int flag){
	...
	for (int ddr = 0; ddr < NUMINSTANCES; ddr++){
		OCL_CHECK(clEnqueueWriteBuffer(world.command_queue, buffer_kvsourcedram[flag][ddr], CL_TRUE, 0, kvsource_size_bytes, kvsourcedram[flag][ddr], 0, NULL, &write_events[ddr] ));
	}
	for (int ddr = 0; ddr < NUMINSTANCES; ddr++){
		OCL_CHECK(clEnqueueWriteBuffer(world.command_queue, buffer_kvdestdram[flag][ddr], CL_TRUE, 0, kvdest_size_bytes, kvdestdram[flag][ddr], 0, NULL, &write_events[1 + ddr] ));
	}
	for (int ddr = 0; ddr < NUMINSTANCES; ddr++){
		OCL_CHECK(clEnqueueWriteBuffer(world.command_queue, buffer_kvstatsdram[flag][ddr], CL_TRUE, 0, kvstats_size_bytes, kvstats[flag][ddr], 0, NULL, &write_events[2 + ddr] ));
	}
...
void host_enigma::finishOCL(){
	...
	for(unsigned int flag=0; flag<NUMCPUTHREADSY; flag++){ clWaitForEvents(1, &kernel_events[flag]); }
