...
#define BFS_ALGORITHM // PR_ALGORITHM, BFS_ALGORITHM, BC_ALGORITHM
...
#define KVBUFFERSIZE_KVS (KVBUFFER_SIZE / VECTOR_SIZE)
...
#if defined(FPGA_IMPL) || defined(TESTKERNEL)
#define DRAMBATCHFACTOR 4
#else
...
#define SRCBUFFER_SIZEE (512 - (2 * NUM_PARTITIONS)) // FIXME.
#define NFACTORR 4
#define KVDRAMPADDING (((DRAMBATCHFACTOR * KVDATA_BATCHSIZE_KVS) / (SRCBUFFER_SIZEE * NFACTORR)) * (NUM_PARTITIONS * VECTOR_SIZE) * (NUM_PARTITIONS * 2))			
...
#define CAPSULESZ_KVS (KVDRAMSZ_KVS / KVBUFFERSIZE_KVS)
...
typedef struct {
    ...
    float totaltime_SSDtransfers_ms;
	float totaltime_OCLtransfers_ms;
	float totaltime_ms;
	float totaltime_overallexcludingOCLandSSDtransfers_ms;
} runsummary_t;
