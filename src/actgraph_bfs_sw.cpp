...
unsigned int actgraph_bfs_sw::run(){
	sizetime_t totalsizetime; totalsizetime.size = 0; totalsizetime.time_ms = 0;
	...
	for(graph_iterationidx=0; graph_iterationidx<64; graph_iterationidx++){
		...
		if(edges_access_type.type == STREAMING_ACCESS_TYPE){
			...
			sizetime_t sizetime = actgraph_pr_sw_obj->start2();
			totalsizetime.size += sizetime.size;
			totalsizetime.time_ms += sizetime.time_ms;
		} if(edges_access_type.type == PARTIAL_ACCESS_TYPE){
			cout<<"=== PARTIAL_ACCESS_TYPE ==="<<endl;
			sizetime_t sizetime = start();
			totalsizetime.size += sizetime.size;
			totalsizetime.time_ms += sizetime.time_ms;
		}
	...
	cout<< TIMINGRESULTSCOLOR <<">>> actgraph_bfs_sw::summary Total number of kvReads for all threads (1): "<<totalsizetime.size<< RESET <<endl;
	cout<< TIMINGRESULTSCOLOR <<">>> actgraph_bfs_sw::summary Throughput: "<<(unsigned long)totalsizetime.size / (float)(totaltime_ms / 1000)<<" keyvalues per second"<< RESET <<endl;
	
	// finish();
	return graph_iterationidx;
}
sizetime_t actgraph_bfs_sw::start() {
	std::chrono::steady_clock::time_point begintime = std::chrono::steady_clock::now();
	...
	cout<<">>> actgraph_bfs_sw::summary Total number of kvReads for all threads (1): "<<gettotalsize1()<<endl;
	cout<<">>> actgraph_bfs_sw::summary Total number of kvReads for all threads (2): "<<gettotalsize2()<<endl;
	cout<< TIMINGRESULTSCOLOR <<">>> actgraph_bfs_sw::summary Throughput: "<<(float)gettotalsize2() / (float)(totaltime_ms / 1000)<<" keyvalues per second"<< RESET <<endl;
	
	sizetime_t sizetime; sizetime.size = gettotalsize2(); sizetime.time_ms = totaltime_ms;
	return sizetime;
}
...
void actgraph_bfs_sw::launchkernel(unsigned int threadidx, unsigned int flag){
	...
	for (int ddr = 0; ddr < NUMINSTANCES; ddr++){
		// OCL_CHECK(clEnqueueWriteBuffer(world.command_queue, buffer_kvdestdram[threadidx][flag][ddr], CL_FALSE, 0, kvdest_size_bytes, kvdestdram[threadidx][flag][ddr], 0, NULL, &write_events[4 + ddr] ));
