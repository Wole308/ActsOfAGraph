...
public:
	...
	void loadvertexpropertiesfromfile(int threadidx, int lbankoffset);
	...
	void loadvertexisactiveinfosfromfile(int threadidx, int threadidxoffset);
	void workerthread_loadvertexisactiveinfosfromfile(int ithreadidx, int fd, unsigned int offset, unsigned int * buffer, vertex_t bufferoffset, vertex_t size);
	
	void replicateverticesdata(keyvalue_t * kvdram[NUMCPUTHREADS], unsigned int offset, unsigned int size);
...
private:
	uint512_vec_dt * kvsourcedram[NUMCPUTHREADSY][NUMFLAGS][NUMCPUTHREADS];
	uint512_vec_dt * kvdestdram[NUMCPUTHREADSY][NUMFLAGS][NUMCPUTHREADS];
	keyvalue_t * kvstats[NUMCPUTHREADSY][NUMFLAGS][NUMCPUTHREADS];
	...
	std::thread genw_thread[NUMCPUTHREADSY][NUMCPUTHREADS];
	std::thread runacts_thread[NUMCPUTHREADS][NUMCPUTHREADS];
	...
	cl_mem buffer_kvsourcedram[NUMCPUTHREADSY][NUMFLAGS][NUMINSTANCES];
	cl_mem buffer_kvdestdram[NUMCPUTHREADSY][NUMFLAGS][NUMINSTANCES];
	cl_mem buffer_kvstatsdram[NUMCPUTHREADSY][NUMFLAGS][NUMINSTANCES];
	#endif
};
#endif
