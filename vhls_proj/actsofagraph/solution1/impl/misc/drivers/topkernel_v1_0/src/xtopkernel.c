...
void XTopkernel_Set_kvsetdramA_V(XTopkernel *InstancePtr, u32 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XTopkernel_WriteReg(InstancePtr->Control_BaseAddress, XTOPKERNEL_CONTROL_ADDR_KVSETDRAMA_V_DATA, Data);
}

u32 XTopkernel_Get_kvsetdramA_V(XTopkernel *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XTopkernel_ReadReg(InstancePtr->Control_BaseAddress, XTOPKERNEL_CONTROL_ADDR_KVSETDRAMA_V_DATA);
    return Data;
}

void XTopkernel_Set_kvdramA(XTopkernel *InstancePtr, u32 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XTopkernel_WriteReg(InstancePtr->Control_BaseAddress, XTOPKERNEL_CONTROL_ADDR_KVDRAMA_DATA, Data);
}

u32 XTopkernel_Get_kvdramA(XTopkernel *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XTopkernel_ReadReg(InstancePtr->Control_BaseAddress, XTOPKERNEL_CONTROL_ADDR_KVDRAMA_DATA);
    return Data;
}
