...
public:
	...
	#define NUMACTCUSPERAPP (NUMCPUTHREADS * NUMSUBCPUTHREADS)
	#define NUMHBMSPERKERNEL 4
	#define NUMACTSCUSPERKERNEL NUMINSTANCES
	
	#if NUMSUBCPUTHREADS<NUMINSTANCES
	#define NUMACTSCUSPERKERNEL NUMSUBCPUTHREADS
	#else 
	#if NUMPIPELINES==3
	#define NUMACTSCUSPERKERNEL NUMINSTANCES
	#endif 
	
	#define NUMKERNELS ((NUMACTCUSPERAPP + (NUMACTSCUSPERKERNEL - 1)) / NUMACTSCUSPERKERNEL)
...
private:
	...
	#ifdef FPGA_IMPL 
	cl_mem_ext_ptr_t inoutBufExt[NUMACTCUSPERAPP];

    cl::Buffer buffer_kvsourcedram[NUMACTCUSPERAPP];
	
	cl::Kernel krnls[NUMACTCUSPERAPP];
