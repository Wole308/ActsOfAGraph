...
long double goclkernel::runapp2(std::string binaryFile[2], uint512_vec_dt * vdram, uint512_vec_dt * kvsourcedram[NUMSUBCPUTHREADS]){		
	...
	for(unsigned int GraphIter=0; GraphIter<numIters; GraphIter++){
		...
		#if defined(ENABLE_ACTSSYNC) && defined(ALLVERTEXISACTIVE_ALGORITHM)
		{
			#ifdef GOCLKERNEL_DEBUGMODE_HOSTPRINTS
			cout<<"--------------- goclkernel[actssync]:: running ACTS SYNCHRONZE (Iteration "<<GraphIter<<") ---------------"<<endl;
			#endif 
		...
		#if defined(ENABLE_ACTSSYNC) && not defined(ALLVERTEXISACTIVE_ALGORITHM)
		{
			...
			unsigned int num_rounds = 32 / NUMSYNCBUFFERSINKERNEL; // CRITICAL REMOVEME.
			for(unsigned int round=0; round<num_rounds; round++){ // 2
				for(unsigned int inst=0; inst<NUMSUBCPUTHREADS; inst+=NUMSYNCBUFFERSINKERNEL){
					...
					/* for(unsigned int k=0; k<KVDATA_RANGE / VECTOR2_SIZE; k++){ // PADDEDKVSOURCEDRAMSZ_KVS, KVDATA_RANGE
						for(unsigned int v=0; v<VECTOR_SIZE; v++){
							if(round == (num_rounds-1)){ tempvdram_sync[_BASEOFFSETKVS_VERTICESDATA + k].data[v] = vdram[_BASEOFFSETKVS_VERTICESDATA + k].data[v]; }
							// else{ tempvdram_sync[k].data[v] = kvsourcedram[0][k].data[v]; }
							else {
								tempvdram_sync[_BASEOFFSETKVS_VERTICESDATA + k].data[v].key = 0xFFFFFFFF;
								tempvdram_sync[_BASEOFFSETKVS_VERTICESDATA + k].data[v].value = 0xFFFFFFFF;
							}
					...
					for(unsigned int k=0; k<KVDATA_RANGE / VECTOR2_SIZE; k++){ // PADDEDKVSOURCEDRAMSZ_KVS, KVDATA_RANGE
						for(unsigned int v=0; v<VECTOR_SIZE; v++){
							if(round == (num_rounds-1)){ vdram[_BASEOFFSETKVS_VERTICESDATA + k].data[v] = tempvdram_sync[_BASEOFFSETKVS_VERTICESDATA + k].data[v]; }
							// else{ kvsourcedram[0][k].data[v] = tempvdram_sync[k].data[v]; } // NOT NEEDED.
						}
					} */
